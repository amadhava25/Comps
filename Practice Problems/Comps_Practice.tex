...
\end{prb}
\begin{solutions}
	Let $M$ be a smooth compact manifold, and let $F: M \to S^{1}$ be a smooth map such that $dF_{p}: T_{p}M \to T_{F(p)}S^{1}$ is non-zero at each $p \in M$. Since $H^{1}(S^{1}) \cong \mathbb{R}$, $H^{1}(S^{1})$ is generated by some nowhere vanishing 1-form $\omega$. Consider the pullback $\alpha = F^{\ast}\omega$, which is a 1-form on $M$. We observe that 
		\begin{equation}
			d\alpha = d(F^{\ast}\omega) = F^{\ast}d\omega = 0, 
		\end{equation}
	since $d\omega$ is a 2-form on $S^{1}$ and all $k$-forms are identically zero for $k > 1$. Therefore, $\alpha$ is a closed 1-form on $M$. In particular, since the derivative of $F$ is identically non-zero on $M$ and $\omega$ is nowhere vanishing, $\alpha$ is a nowhere vanishing 1-form on $M$. Now suppose that $\alpha$ is exact; i.e., let $f$ be a 0-form on $M$ (i.e., a smooth function from $M$ to $\mathbb{R}$) such that $\alpha = df$. Since $\alpha$ is nowhere vanishing, the derivative of $f$ is nonzero everywhere on $M$. But this is impossible since every smooth function must attain an extreme value on a compact domain, which implies that its differential must vanish where it attains its maximum or minimum. Hence, $\alpha$ cannot be an exact form. Since $Z^{1}(M) \neq B^{1}(M)$, we conclude that $\h{1}(M)$ is non-zero. 
\end{solutions}
\begin{prb}{2002-J-II-5}
	Let $\theta$ be a closed 1-form on a compact manifold $M$ without boundary. Further suppose that $\theta \neq 0$ at each point $p \in M$. Prove that $\h{1}(M) \neq 0$. 
\end{prb}
\begin{solutions}
	Let $\theta$ be a closed 1-form on a compact manifold $M$ without boundary. Further suppose that $\theta \neq 0$ at each point $p \in M$. Assume to the contrary that $\theta$ is exact; i.e., let $f: M \to \mathbb{R}$ be a smooth function such that $\theta = df$. Since $\theta$ is nowhere vanishing, the differential of $f$ must also be nowhere vanishing. But this is impossible since $f$, being a smooth function on the \textit{compact} manifold $M$, must have an extreme value on $M$ which forces its differential to vanish where it attains its maximum or minimum. Hence, by contradiction, $\theta$ cannot be an exact 1-form, which means $Z^{1}(M) \neq B^{1}(M)$. Therefore, $\h{1}(M) = Z^{1}(M)/B^{1}(M) \neq 0$. 
\end{solutions}
\begin{prb}{2015-J-I-5}
	Let $X$ be a connected smooth manifold, and let $U$ and $V$ be connected open subsets such that $X = U \cup V$. Show that if $U \cap V$ is not connected, then the de Rham cohomology group $H^{1}(X, \mathbb{R})$ must be nonzero. 
\end{prb}
\textcolor{red}{[!! Complete Later !!]}


\begin{prb}{2005-A-II-5}
	Let $A \subset \mathbb{R}^{2}$ be the compact planar region bounded by a regular hexagon. Let $X$ be the surface-with-boundary obtained from $A$ by identifying four opposite sides of the hexagon in pairs, according to the following diagram: 
	\begin{center}
		\includegraphics[width =0.2\linewidth]{"./Graphics/2005-A-II-5"}
	\end{center}
	Compute the fundamental group of $X$. 
\end{prb}
\begin{solutions}
	We will use the Seifert-van-Kampen Theorem to compute the fundamental group of $X$.
\end{solutions}
