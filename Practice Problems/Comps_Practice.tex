...
\end{prb}
\begin{solutions}
	Let $E \subset \mathbb{R}^{n}$ be a set of positive Lebesgue measure. Then by the Lebesgue Differentiation Theorem, almost every point of $E$ is a Lebesgue point, which is to say that there exists at least one point $x \in E$ such that for all $\epsilon > 0$, there exists a $r > 0$ so that 
		\begin{equation}
			\frac{m(E \cap B(r, x))}{m(B(r,x))} \geq (1 - \epsilon) \implies m(E \cap B(r, x)) \geq (1 - \epsilon)m(B(r, x)). 
		\end{equation}
	Suppose for contradiction that $E - E$ does not contain a neighborhood of the origin. Then there exists a sequence $x_{n} \to 0$ such that $x_{n} \notin E - E$ for any $n$. Equivalently, $(E + x_{n}) \cap E = \varnothing$ for all $n$. Taking $n$ large enough so that $\abs{x_{n}} < \epsilon r$, 
		\begin{equation}
			m(B(r, x) \cap (B(r, x) - x_{n})) \geq (1 - \epsilon)m(B(r, x)).  
		\end{equation}
	By translation invariance of the Lebesgue measure, 
		\begin{equation}
			m((E + x_{n}) \cap B(r,x)) = m(E \cap (B(r, x) - x_{n})). 
		\end{equation}
	Therefore, 
		\begin{align}
			\begin{split}
				m((E + x_{n}) \cap B(r, x)) &\geq m(B(r, x) \cap (B(r,x) - x_{n})) - m(B(r, x)\setminus E) - m((B(r, x) - x_{n})\setminus E) \\
				&\geq (1 - \epsilon) m(B(r, x)) - 2\epsilon m(B(r, x)) \\
				&\geq 0
			\end{split}
		\end{align}
	for $\epsilon$ small enough. Therefore, $(E + x_{n}) \cap E \cap B(r, x) \neq \varnothing$, contradicting $(E + x_{n}) \cap E_{n} = \varnothing$ for all $n$. Hence, $E - E$ contains a neighborhood of the origin. 
\end{solutions}
