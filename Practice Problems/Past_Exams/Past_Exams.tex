...
\subsection{Some Helpful Resources}
\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
	\item \href{https://en.wikipedia.org/wiki/Main\_Page}{https://en.wikipedia.org/wiki/Main\_Page}
	\item \href{https://encyclopediaofmath.org/wiki/Main\_Page}{https://encyclopediaofmath.org/wiki/Main\_Page}
	\item \href{https://www.math3ma.com/}{https://www.math3ma.com/}
	\item \href{https://mathworld.wolfram.com/}{https://mathworld.wolfram.com/}
	\item \href{https://math.stackexchange.com/}{https://math.stackexchange.com/}
	\item \href{https://www.amazon.com/Real-Analysis-Modern-Techniques-Applications/dp/0471317160}{Folland, \textit{Real Analysis}}
	\item \href{https://www.amazon.com/Abstract-Algebra-3rd-David-Dummit/dp/0471433349/ref=sr\_1\_1?crid=2QCEP6X7QBOXZ\&dib=eyJ2IjoiMSJ9.4lpFqXqi4gfno3aacXf0MjatThq5tLEJhAp-fh-ZjoEcpVe6BepfcaakfakkUP11RZ0suA8TjaFWP839kuylJN6HFv0BVFTsPnjGhtLspwbcq18NH\_QG7IPgfLW09sHPIP89\_aZC4YkgtkH8VF4ti3-uwBq2qtNHbb7urgKZAOSRB9HHcJ0TmqsqzWqhb-RqqMi48Npdgh9Xa15HMlw95FCH67WPMr9zUPn9DyEZhAA.58K0BCGVFKYHckU-lNQ\_Xjh1YVuOBflo8ho9tPPGUqo\&dib\_tag=se\&keywords=dummit+and+foote\%2C+abstract+algebra+3rd+edition\&qid=1768622868\&sprefix=dummit+and+foot\%2Caps\%2C124\&sr=8-1}{Dummit \& Foote, \textit{Abstract Algebra}}
	\item Comps solutions for 2014-2018 by Mads Bach Villadsen
\end{enumerate}
...
\subsection{Fundamental Group of the Projective Planes}
\begin{prb}{(Fundamental Group of the Projective Planes)}
	Compute the fundamental group of $\mathbb{RP}^{n}$ for all $n \geq 1$. 
\end{prb}
\begin{solutions}
	We recall the definition of $\mathbb{RP}^{n}$ for $n \geq 1$. 
		\begin{quote}
			(\textbf{Real Projective Space}) $\mathbb{RP}^{n}$ is the quotient space obtained from $S^{n}$ by identifying each point $x$ of $S^{2}$ with its antipodal point $-x$. 
		\end{quote} 
	Now we prove the following: 
		\begin{quote}
			(\textbf{Theorem}) For all $n\geq 1$, $\mathbb{RP}^{n}$ is compact, and the quotient map $p: S^{n} \to \mathbb{RP}^{n}$ is a covering map. 
			
			\begin{proof}
				First, we will show that $\mathbb{RP}^{n}$ is compact. Let $U \subset S^{2}$ be open. The antipodal map $a: S^{n} \to S^{n}$ given by $a(x) = -x$ is a homeomorphism of $S^{n}$, which means that $a(U)$ is open in $S^{n}$. Then since $p$ is a quotient map,
					\begin{equation}
						p^{-1}(p(U)) = U \cup a(U), 
					\end{equation}
				the set is also open in $S^{n}$. Therefore, $p(U)$ is open in $\mathbb{RP}^{n}$. Hence, $p$ is an open map. Likewise, we can show that $p$ is a closed map. Since $\mathbb{RP}^{n}$ is connected, $S^{n}$ is compact, and $p$ is surjective, we conclude that $p(S^{n}) = \mathbb{RP}^{n}$, and so $\mathbb{RP}^{n}$ is compact. 
				
				Now let $y \in \mathbb{RP}^{n}$, and choose $x \in p^{-1}(y)$. Then choose an $\epsilon$-neighborhood $U$ of $x$ in $S^{n}$ for some $\epsilon < 1$, using the euclidean metric of $\mathbb{R}^{n}$. Then $U$ contains no pair $\{z, a(z)\}$ of antipodal points of $S^{n}$, since $d(z, a(z)) = 2$. As a result, the map $$p: U\to p(U)$$ is bijective. Since $p$ is continuous and open, $p$ is a homeomorphism. Likewise, $p: a(U) \to p(a(U)) = p(U)$ is a homeomorphism. The set $p^{-1}(p(U))$ is thus the union of the two disjoint open sets $U$ and $a(U)$, each of which is homeomorphically mapped by $p$ onto $p(U)$. Hence, $p(U)$ is a neighborhood of $p(x) =y$ that is evenly covered by $p$. 
			\end{proof} 
		\end{quote}
		Now we have the following theorem 
		\begin{quote}
			(\textbf{Theorem}) Let $p: E \to B$ be a covering map; let $p(e_{0}) = b_{0}$. If $E$ is path connected, then the lifting correspondence $$\phi: \pi_{1}(B, b_{0})\to p^{-1}(b_{0})$$ is surjective. If $E$ is simply connected, it is bijective. 
			
			\begin{proof}
				Let $E$ be path connected. Then given $e_{1} \in p^{-1}(b_{0})$, there is a path $\ti{f}$ in $E$ from $e_{0}$ to $e_{1}$. Then $f = p \circ \ti{f}$ is a loop in $B$ at $b_{0}$, and $\phi([f]) = e_{1}$ by definition. Therefore, $\phi$ is surjective. 
				
				Now suppose $E$ is simply connected. Let $[f], [g]$ be two elements of $\pi_{1}(B, b_{0})$ such that $\phi([f]) = \phi([g])$. Let $\ti{f}$ and $\ti{g}$ be the liftings of $f$ and $g$, respectively, to paths in $E$ that begin at $e_{0}$; then $\ti{f}(1) = \ti{g}(1)$. Since $E$ is simply connected, there is a path homotopy $\ti{F}$ in $E$ between $\ti{f}$ and $\ti{g}$. Hence, $F = p \circ \ti{F}$ is a path homotopy between $f$ and $g$, which proves injectivity. 
			\end{proof}
		\end{quote}
	Now, suppose $n \geq 2$. Then since $S^{n}$ is simply connected, $\phi: \pi_{1}(\mathbb{RP}^{n}, b_{0}) \to p^{-1}(b_{0})$ is bijective. But since $|p^{-1}(b_{0})| = 2$, $|\pi_{1}(\mathbb{RP}^{n},b_{0})| = 2$ for all $b_{0} \in \mathbb{RP}^{n}$. Therefore, $\pi_{1}(\mathbb{RP}^{n},b_{0}) \cong \mathbb{Z}/2\mathbb{Z}$. Now suppose $n = 1$. Since $\mathbb{RP}^{1}$ is homeomorphic to $S^{1}$, their fundamental groups must be isomorphic. Therefore, $\pi_{1}(\mathbb{RP}^{1}, b_{0}) \cong \pi_{1}(S^{1}, p_{0}) \cong \mathbb{Z}$. Hence, we conclude that 
		\begin{equation}
			\pi_{1}(\mathbb{RP}^{n}) \cong 
			\begin{cases}
				\mathbb{Z}, & n = 1, \\
...
\begin{prb}{(Lifting Criterion)}
	Let $f: (Y, y_{0}) \to (X, x_{0})$ be a map with $Y$ path-connected and locally path-connected. Then a lift of $f$ exists if and only if 
		\begin{equation*}
			f_{\ast}(\pi_{1}(Y, y_{0})) \subseteq p_{\ast}(\pi_{1}(\ti{X},\ti{x}_{0})). 
		\end{equation*}
\end{prb}
...
\end{solutions}
\begin{prb}{2025-A-II-1 (Geometry/Topology)}
	For $n\geq 2$, \vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\roman{*})]
		\item Calculate the fundamental group of the real $n$-dimensional projective space $\mathbb{P}^{n}$; 
		\item Show that any continuous map $f: \mathbb{P}^{n} \to \mathbb{T}^{n}$ from $\mathbb{P}^{n}$ to the $n$-dimensional torus is homotopic to a constant map. 
	\end{enumerate}
\end{prb}
\begin{solutions}
	Let $n \geq 2$. \vspace{-0.35cm}
	
	\begin{enumerate}[itemsep =-2pt,label = (\roman{*})]
		\item Let $\sim$ be an equivalence relation on $S^{n}$ that identifies each $x \in S^{n}$ with its antipodal point $-x$, and let $\mathbb{RP}^{n} = S^{n}/\sim$ with quotient map $p: S^{n} \to \mathbb{RP}^{n}$. We begin by proving that $p: S^{n} \to \mathbb{RP}^{n}$ is a covering map. 
		
		Let $y \in \mathbb{RP}^{n}$. For each $x \in p^{-1}(y)$, choose an $\epsilon$-neighborhood $U$ in $S^{n}$ for $\epsilon < 1$ using the euclidean metric on $\mathbb{R}^{n}$. Let $a: S^{n} \to S^{n}$ be the antipodal map given by $a(x)= -x$, which is clearly seen to be a homeomorphism of $S^{n}$; this means that $a(U)$ is open in $S^{n}$. By construction of $U$, $U$ does not contain an pair $\{z, a(z)\}$ of antipodal points of $S^{n}$ since $d(z, a(z)) = 2$. Therefore, the map $p: U\to p(U)$ is bijective. Since $p$ is continuous and open, $p$ is a homeomorphism. Likewise, $p: a(U) \to p(a(U)) = p(U)$ is a homeomorphism. The set $p^{-1}(p(U)) = U \sqcup a(U)$, each of which is homeomorphically mapped by $p$ onto $p(U)$. Therefore, $p(U)$ is a neighborhood of $p(x) = y$ that is evenly covered by $p$. Hence, $p$ is a covering map. 
		
		Now we recall that if $p: E \to B$ is a covering map, $b_{0} \in B$, $e_{0} \in p^{-1}(b_{0})$, and $E$ is simply connected, then the map $\phi: \pi_{1}(B, b_{0}) \to p^{-1}(b_{0})$ defined by $\phi([f]) = \ti{f}(1)$, where $\ti{f}$ is a lift of $f$ to a path in $E$ beginning at $e_{0}$, is bijective. Since $p^{-1}(b_{0})$ contains only two elements for every $b_{0} \in \mathbb{RP}^{n}$ and $S^{n}$ is simply connected, $\pi_{1}(\mathbb{RP}^{n}, b_{0})$ is a group of two elements. Therefore, the fundamental group of $\mathbb{RP}^{n}$ is simply $\mathbb{Z}_{2}$. 
		\item Now let $f: \mathbb{RP}^{n} \to \mathbb{T}^{n}$ be a continuous map; $f$ induces a group homomorphism $f_{\ast}: \pi_{1}(\mathbb{RP}^{n}) \longrightarrow \pi_{1}(\mathbb{T}^{n})$. We observe that $\pi_{1}(\mathbb{RP}^{n}) \cong \mathbb{Z}_{2}$, while $\pi_{1}(\mathbb{T}^{n}) \cong \mathbb{Z}^{n}$. Since $\mathbb{Z}^{n}$ has no subgroup of order 2, we conclude that $f_{\ast}$ be the trivial homomorphism. By the lifting criterion, this then implies that $f$ lifts to a path $\ti{f}: \mathbb{RP}^{n} \to \mathbb{R}^{n}$, where $\mathbb{R}^{n}$ is the universal cover of $S^{n}$. But since $\mathbb{R}^{n}$ is contractible, $\ti{f}$ is homotopic to a constant map. Hence, projecting down, we conclude that $f$ is homotopic to a constant map. 
	\end{enumerate}
\end{solutions}
...
\begin{solutions}
	...
	\end{enumerate}
\end{solutions}
\begin{prb}{2025-A-II-4 (Real Analysis)}
	$ $\newline \vspace{-0.65cm}
	\begin{enumerate}[itemsep =-2pt,label = (\roman{*})]
...
\begin{solutions}
	\vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\roman{*})]
		\item Let $f \in L^{1}(\mathbb{R})$ such that $f \ast f = f$. By equality, $f \ast f \in L^{1}$. So taking the Fourier transform of both sides, $\hat{f}^{2} = \hat{f}$, which means that $\hat{f}(\xi) = 0$ or $\hat{f}(\xi) = 1$. In other words, $\hat{f}$ must be the characteristic function $\chi_{E}$ of some measurable subset $E$. Since the Fourier transform of a $L^{1}$ function must necessarily be continuous, either $E = \varnothing$ or $E = \mathbb{R}$. Moreover, by the Riemann-Lebesgue Lemma, $\hat{f}$ must vanish at infinity. Since $\chi_{\mathbb{R}}$ does not vanish at infinity, $\hat{f} = \chi_{\varnothing}$. This means that $\hat{f} \equiv 0$. Therefore, by injectivity of the Fourier transform on $L^{1}$, we conclude that the only $L^{1}$-function for which $f \ast f = f$ is the zero function $f \equiv 0$. 
	\end{enumerate}
\end{solutions}
...
\end{solutions}
\begin{prb}{2024-J-II-2 (Real Analysis)}
...
\end{solutions}
\begin{prb}{https://mycourses.stonybrook.edu/d2l/le/content/2489452/viewContent/60041176/View (Real Analysis)}
...
\begin{prb}{2023-J-I-3 (Geometry/Topology)}
	Show that if $M$ is a closed manifold that has an even dimensional sphere $S^{2n}$ as its universal cover, then its fundamental group is either trivial or $\mathbb{Z}_{2}$. 
\end{prb}
\begin{solutions}
	Let $M$ be a closed manifold that has an even dimensional sphere $S^{2n}$ as its universal cover. Then $M = S^{2n}/G$, where $G$ is the group of deck transformations. Since $S^{2n}$ is path connected, each nonidentity deck transformation $g$ is a homeomorphism $S^{2n} \to S^{2n}$ with no fixed points. Then 
		\begin{equation}
			\deg{g} = (-1)^{2n + 1} =-1. 
		\end{equation}
	If $G$ contains two nontrivial elements $f$ and $g$, this implies $\deg(f \circ g) = \deg{f}\deg{g}= 1$. Hence, by our observation $f \circ g = \operatorname{id}_{S^{2n}}$, and so $f = g^{-1}$. Likewise, $g^{2} = \operatorname{id}_{S^{2n}}$ so that $f = g$. Since $S^{2n}$ is simply connected, we have $\pi_{1}(M) \cong G$, and therefore $\pi_{1}(M)$ is either trivial or $\mathbb{Z}_{2}$. 
\end{solutions}
...
\begin{prb}{2023-J-I-6 (Algebra)}
	Let $V$ be a real vector space of finite dimension $d > 2$. Let $T: V \to V$ be a finite order automorphism. Prove that the associated linear operator 
		\begin{equation*}
			\Lambda^{2}(T): \Lambda^{2}(V) \to \Lambda^{2}(V), \quad v_{1} \wedge v_{2} \mapsto T(v_{1}) \wedge T(v_{2})
		\end{equation*}
	fixes a nonzero vector. 
\end{prb}
\begin{solutions}
	Let $T: V \to V$ be a finite order automorphism, which means that there exists some positive integer $m$ such that $T^{m} = \operatorname{id}_{V}$. This means that $T$ is diagonalizable over $\mathbb{C}$, with eigenvalues that are $m$th roots of unity. If $T$ has a nonreal eigenvalue $z$, then in the complexification $V_{\mathbb{C}}$, there is a nonzero eigenvector $v$ of $T_{\mathbb{C}}$ with eigenvector $z$. Since $T$ is real, its characteristic polynomial is real, and so complex eigenvalues come in conjugate pairs. This means that the conjugate vector $v^{\ast}$ has eigenvalue $z^{\ast}$. Since $z \neq z^{\ast}$, $v$ and $v^{\ast}$ are $\mathbb{C}$-linearly independent. Therefore, $w = iv\wedge v^{\ast}$ is a nonzero element of $\Lambda^{2}_{\mathbb{C}}V_{\mathbb{C}}$ with eigenvalue $z^{\ast}z$ under $\Lambda^{2}(T)$ and which is fixed by complex conjugation. Hence, it is the image of a nonzero element $w$ of $\Lambda^{2}(V)$, which also has eigenvalue $z^{\ast}z$. Since $T$ has finite order, every eigenvalue of $z$ is a root of unity, and so $z^{\ast}z = 1$. 
	
	On the other hand, if every eigenvalue of $T$ was real $\pm 1$, since $d > 2$, $T$ has at least three real eigenvalues, of which at least two must have the same sign. I.e., there exist two linearly independent nonzero eigenvectors $v$ and $w$ with eigenvalues both $+ 1$ or both $-1$. Then 
		\begin{equation}
			\Lambda^{2}(T)(v \wedge w) = (\pm 1)^{2}(v \wedge w) = v \wedge w, 
		\end{equation}
	so that $T$ fixes the nonzero vector $v \wedge w$. Hence, the proof concludes.
\end{solutions}
...
\end{prb}
\begin{prb}{2023-A-I-3 (Algebra)}
	Let $A$ be a square matrix with integer entries. Suppose that for some odd prime number $p$, all eigenvalues of $A$ are $p$th roots of unity, and that $A$ is congruent to the identity matrix modulo $p$. Show that all the eigenvalues of $A$ are equal to $1$. 
\end{prb}
\begin{solutions}
	Let $A$ be a square matrix with integer entries. \textcolor{red}{[!! Complete Later !!]}
\end{solutions}
...
\end{solutions} 
\begin{prb}{2022-J-I-2 (Geometry/Topology)}
	Let $\mathbb{T}^{2} = \mathbb{R}^{2}/\mathbb{Z}^{2}$, where $\mathbb{Z}^{2} \subset \mathbb{R}^{2}$ is the standard integer lattice. Is it true that (a) every continuous map $f: S^{2} \to \mathbb{T}^{2}$ is nulhomotopic? (b) every continuous map $g: \mathbb{T}^{2} \to S^{2}$ is nulhomotopic? 
\end{prb}
\begin{solutions}
	\vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item Let $f: S^{2} \to \mathbb{T}^{2}$ be continuous, and consider the induced group homomorphism $f_{\ast}: \pi_{1}(S^{2}) \to \pi_{1}(\mathbb{T}^{2})$. Recall that $\pi_{1}(S^{2}) = \{e\}$ and $\pi_{1}(\mathbb{T}^{2}) = \mathbb{Z} \times \mathbb{Z}$. This means that $f_{\ast}$ must be the trivial homomorphism. Hence, since $f_{\ast}\pi_{1}(S^{2}) = \{(e,e)\} \subset \pi_{1}(\mathbb{T}^{2})$, by the General Lifting Lemma, $f$ lifts to a continuous map $\ti{f}: S^{2} \to \mathbb{R}^{2}$, where $\mathbb{R}^{2} = \ti{\mathbb{T}^{2}}$. Since $\mathbb{R}^{2}$ is contractible, $\ti{f}$ must be nulhomotopic; let $\ti{H}$ be a corresponding nulhomotopy. Hence, if $q: \mathbb{R}^{2} \to \mathbb{T}^{2}$ is a covering map, then $H \coloneqq q\circ \ti{H}$ would be a homotopy between $f$ and a constant map. Hence, $f$ is nulhomotopic. This concludes that every continuous map $f: S^{2} \to \mathbb{T}^{2}$ is nulhomotopic. 
		\item Let $g: \mathbb{T}^{2} \to S^{2}$ be continuous, and let $q: S^{2} \to S^{2}$ be a universal covering space of $S^{2}$. It is not necessarily true that $g$ is nulhomotopic. Consider $X = (S^{1} \cup \{p\}) \cup (\{p\}\times S^{1})$, and consider the projection map $\pi: T^{2} \to S^{2}/X \cong S^{2}$. Then $\pi$ is not nulhomotopic (has degree one). Hence, this statement is not generally true.  
	\end{enumerate}
\end{solutions} 
...
\end{solutions}
\begin{prb}{2022-J-II-6 (Algebra)}
	Construct a Galois field extension $E \supset \mathbb{Q}$ with $\operatorname{Gal}(E/\mathbb{Q}) = \mathbb{Z}_{3} \times \mathbb{Z}_{9}$. 
\end{prb}
\begin{solutions}
	We observe that $\mathbb{Z}_{3} \subset \mathbb{Z}_{2}\times \mathbb{Z}_{3} = \mathbb{Z}_{6} \approx \mathbb{Z}_{7}^{\ast}$ and $\mathbb{Z}_{9} \subset \mathbb{Z}_{2} \times \mathbb{Z}_{9} = \mathbb{Z}_{18} \approx \mathbb{Z}_{19}^{\ast}$. Hence, by the Chinese Remainder Theorem, 
		\begin{equation}
			\mathbb{Z}_{133}^{\ast} \approx \mathbb{Z}_{7}^{\ast} \times \mathbb{Z}_{19}^{\ast} \approx (\mathbb{Z}_{3} \times\mathbb{Z}_{9}) \times (\mathbb{Z}_{2}\times \mathbb{Z}_{2}). 
		\end{equation}
	Here, we note that $\mathbb{Z}_{133}^{\ast}$ is the Galois group of $\mathbb{Q}(\zeta)$, where $\zeta$ is a primitive root of unit of degree 133. Take 
		\begin{equation}
			E = \mathbb{Q}(\zeta)^{\mathbb{Z}_{2} \times \mathbb{Z}_{2}} \subset \mathbb{Q}(\zeta). 
		\end{equation}
\end{solutions}
\begin{prb}{2022-J-II-6 (Algebra I)}
	Construct a Galois field extension $E \supset \mathbb{Q}$ with $\operatorname{Gal}(E/\mathbb{Q}) = \mathbb{Z}_{4} \times \mathbb{Z}_{6}$. 
\end{prb}
\begin{solutions}
	We observe the following: 
		\begin{equation}
			\mathbb{Z}_{5}^{\ast} \cong \mathbb{Z}_{4} \qquad \text{ and } \mathbb{Z}_{13}^{\ast} \cong \mathbb{Z}_{12}. 
		\end{equation}
	Since $\mathbb{Q}(\zeta_{5})$ is Galois over $\mathbb{Q}$, let $L = \mathbb{Q}(\zeta_{5})$. Likewise, since a group of order 12 has a unique subgroup of index 2, there exists a unique subfield $K \subset \mathbb{Q}(\zeta_{13})$ such that $[K:\mathbb{Q}] = 6$ so that $\operatorname{Gal}(K/\mathbb{Q}) \cong \mathbb{Z}_{6}$. Since $9$ and $13$ are relatively prime, $K$ and $L$ are linearly disjoint over $\mathbb{Q}$. Since $K \cap L = \mathbb{Q}$, we conclude that 
		\begin{equation}
			\operatorname{Gal}(KL/\mathbb{Q}) \cong \operatorname{Gal}(K/\mathbb{Q}) \times \operatorname{Gal}(L/\mathbb{Q}) \cong \mathbb{Z}_{6} \times \mathbb{Z}_{4}. 
		\end{equation}
\end{solutions}
\begin{prb}{2022-J-II-6 (Algebra II)}
		Construct a Galois field extension $E \supset \mathbb{Q}$ with $\operatorname{Gal}(E/\mathbb{Q}) = \mathbb{Z}_{4} \times \mathbb{Z}_{12}$. 
\end{prb}
\begin{solutions}
	Let $E = KL$, where $K = \mathbb{Q}(\zeta_{5}) \cong \mathbb{Z}_{4}$ and $L = \mathbb{Q}(\zeta_{13}) \cong (\mathbb{Z}/13\mathbb{Z})^{\ast} \cong \mathbb{Z}_{12}$.
\end{solutions}
\subsection{August 2022}
\begin{prb}{2022-A-I-1 (Geometry/Topology)}
	Let $Y \subset \mathbb{R}^{3}$ be a surface of genus two. 
		\begin{center}
			\includegraphics[width=0.3\linewidth]{../Graphics/2022AI1.png}
		\end{center}
		
	Let $Z$ be the closure of the bounded component in $\mathbb{R}^{3}\setminus Y$. (Thus, $Z$ is the compact domain-with-boundary whose interior is the region inside of $Y$.) Prove that the inclusion homomorphism $\pi_{1}(Y, \text{pt}) \to \pi_{1}(Z, \text{pt})$ is surjective. Then use this to show that the fundamental group $\pi_{1}(Y, \text{pt})$ of the surface $Y$ is non-abelian. 
\end{prb}
\begin{solutions}
	Let $X \subset Y$ be a figure-eight along the top of the surface. Then $Z$ is a deformation retract of $Z$ and $X \hookrightarrow Z$ induces an isomorphism of fundamental groups. Hence, the image of $\pi_{1}(X)$ in $\pi_{1}(Y)$ maps isomorphically to $\pi_{1}(Z)$. In particular, $\pi_{1}(X) \to \pi_{1}(Z)$ is surjective. But since $\pi_{1}(Z) \cong \pi_{1}(X) \cong \mathbb{Z} \ast \mathbb{Z}$ is non-abelian. Since any homomorphic image of an abelian group is abelian, it follows that $\pi_{1}(Y)$ cannot be abelian. 
\end{solutions}
\begin{prb}{2022-A-I-2 (Algebraa)}
	Let $f(x) \in \mathbb{Q}[x]$ be an irreducible polynomial of odd degree greater than 1 and having only one real root $u \in \mathbb{R}$. Show that $\mathbb{Q}(u)$ is not a normal extension. 
\end{prb}
\begin{solutions}
	This problem requires the definition of a normal extension, which is the following: 
		\begin{quote}
			(\textbf{Normal Extension}) An algebraic field extension $L/K$ for which every irreducible polynomial over $K$ that has a root in $L$ splits into linear factors over $L$. 
		\end{quote}
	By hypothesis, $f$ is an irreducible polynomial of odd-degree greater than 1 that has a real root $u$ trivially contained in $\mathbb{Q}(u)$. If $v \in \mathbb{C}$ is a complex-valued root of $f$ (which $f$ must have since the number of complex solutions of a polynomial is equal to its degree), then $v \notin \mathbb{Q}(u)$ since $\mathbb{Q}(u) \subset \mathbb{R}$. This means that $f$ cannot split over $\mathbb{Q}(u)$. And hence, $\mathbb{Q}(u)/\mathbb{Q}$ cannot be a normal extension. 
\end{solutions}
...
\end{prb}
\begin{solutions}
	Let $X$ be the space of unordered pairs of distinct points of $S^{n}$. We claim that $X$ is homotopy equivalent to the real projective space $\mathbb{RP}^{n}$. Indeed the projective space of dimension $n$ can be identified to the space of unordered pairs of antipodal points of $S^{n}$, and there is a deformation retraction of the whole space to this subspace: draw through the pair of distinct points a line and take the parallel line through the center of $S^{n}$. This means that $\pi_{1}(X) \cong \pi_{1}(\mathbb{RP}^{n}) \cong \mathbb{Z}/2\mathbb{Z}$ for $n\geq 2$, $\cong \mathbb{Z}$ for $n = 1$, and $\cong \{0\}$ for $n =0$. 
\end{solutions}
...
\end{solutions}
\begin{prb}{2022-A-I-6 (Algebra)}
	Denote by $\operatorname{Mat}(m, \mathbb{C})$ and $\operatorname{Mat}(m,n,\mathbb{C})$ the space of $m\times m$ and $m \times n$ matrices with complex coefficients. Let $A \in \operatorname{Mat}(n, \mathbb{C})$, $B \in \operatorname{Mat}(m, \mathbb{C})$ be given and suppose $A$ and $B$ have no common eigenvalues. Show that the only solution of 
		\begin{equation*}
			AX = XB, \qquad X \in \operatorname{Mat}(m,n, \mathbb{C})
		\end{equation*}
...
\begin{solutions}
	Assume all of the given hypotheses. Observe that $A^{2}X = A(AX) = (AX)B = XB^{2}$. Hence, by induction, it follows that for all $k$, 
		\begin{equation}
			A^{k}X = XB^{k}. 
		\end{equation}
	In particular, this means that $p(A)X = Xp(B)$ for any $p \in \mathbb{C}[z]$. Let $p$ be the characteristic polynomial $p_{B}(z)$ of $B$. Hence, we obtain 
		\begin{equation}
			p_{B}(A)X = Xp_{B}(B) = 0. 
		\end{equation}
	We can write 
		...
		\end{equation}
	By the hypothesis, $\lambda_{k}$ is not an eigenvalue of $A$. Hence, $(A - I\lambda_{k})^{m_{k}}$ is invertible. Therefore, $p_{B}(A)$ is invertible. Therefore, 
		\begin{equation}
...
\begin{prb}{2021-J-I-2 (Geometry/Topology)}
...
\begin{prb}{2021-J-II-2 (Real Analysis)}
	A real-valued function $f: [0,1] \to \mathbb{R}$ is said to be H\"older continuous of order $\alpha$ if there is a constant $C > 0$ such that $\abs{f(x) - f(y)} \leq C|x - y|^{\alpha}$ for all $x, y \in [0,1]$. Define 
		\begin{equation*}
			\norm{f}_{\alpha} = \max_{x \in[0,1]}\abs{f(x)} + \sup_{x, y \in [0,1]}\frac{|f(x) - f(y)|}{|x - y|^{\alpha}}. 
		\end{equation*}
	Show that for $\alpha \in (0,1)$, the set of functions 
		\begin{equation*}
			S_{\alpha} = \brac*{f \in C([0,1]): \norm{f}_{\alpha} \leq 1}
		\end{equation*}
	is a compact subset of $C([0,1])$ in the topology of uniform convergence. 
\end{prb}
\begin{solutions}
	By Arz{\`e}la-Ascoli, a subset of $C([0,1])$ is compact if and only if it is closed, bounded, and equicontinuous. Hence, it suffices to prove that for any $\alpha \in (0,1)$, $S_{\alpha}$ is closed, bounded, and equicontinuous in the topology of uniform convergence. \vspace{-0.1cm}
		\begin{enumerate}[itemsep =-2pt,label = (\roman{*})]
			\item First, we show that $S_{\alpha}$ is uniformly bounded. Since for all $f \in S_{\alpha}$, $\norm{f}_{\alpha} \leq 1$, and $\norm{f}_{\alpha} \geq \max_{x \in [0,1]}|f(x)|$, it follows that for all $f \in S_{\alpha}$ and $x \in [0,1]$, $\abs{f(x)} \leq 1$. Hence, $S_{\alpha}$ is uniformly bounded. 
			\item Now we will show that $S_{\alpha}$ is closed. Suppose $\{f_{n}\}\subset S_{\alpha}$ converges uniformly to a function $f \in C([0,1])$. We must show that $f \in S_{\alpha}$. First, we observe that 
				\begin{equation}
					\max_{x \in [0,1]}|f(x)| \leq \max_{x \in [0,1]}\abs{f_{n}(x)} + \norm{f - f_{n}}_{\infty}. 
				\end{equation}
			Next, we observe that 
				\begin{align}
					\begin{split}
						\sup_{x, y \in [0,1]}\frac{\abs{f(x) - f(y)}}{\abs{x -y}^{\alpha}} &\leq \sup_{x, y \in [0,1]}\brac*{\frac{\abs{f(x) - f_{n}(x)}}{\abs{x - y}^{\alpha}} + \frac{\abs{f_{n}(x) - f_{n}(y)}}{\abs{x - y}^{\alpha}} + \frac{\abs{f_{n}(y) - f(y)}}{\abs{x - y}^{\alpha}}} \\
						&\leq 2\norm{f - f_{n}}_{\infty} + \sup_{x, y \in [0,1]}\frac{\abs{f_{n}(x) - f_{n}(y)}}{\abs{x - y}^{\alpha}}. 
					\end{split}
				\end{align}
			Therefore, 
				\begin{align}
					\begin{split}
						\norm{f}_{\alpha} &= \max_{x \in [0,1]}\abs{f(x)} + \sup_{x, y \in [0,1]}\frac{\abs{f(x) - f(y)}}{\abs{x - y}^{\alpha}} \\
						&\leq 3\norm{f - f_{n}}_{\infty} + \norm{f_{n}}_{\alpha} \leq \lim_{n \to \infty}\norm{f_{n}}_{\alpha} \leq 1. 
					\end{split}
				\end{align}
			Hence, $f \in S_{\alpha}$, which proves closedness. 
			\item Finally, we will show equicontinuity. By definition of $S_{\alpha}$, for any $f \in S_{\alpha}$, 
				\begin{equation}
					\frac{\abs{f(x) - f(y)}}{\abs{x - y}^{\alpha}} \leq 1 \implies \abs{f(x) - f(y)} \leq \abs{x -y}^{\alpha}. 
				\end{equation}
			Let $\epsilon > 0$, and choose $\delta = \epsilon^{1/\alpha}$. Then for any $d(x, y) < \delta$, 
				\begin{equation}
					\abs{f(x) - f(y)} \leq \abs{x - y}^{\alpha} < \delta^{\alpha} = \epsilon. 
				\end{equation}
			Hence, $S_{\alpha}$ is equicontinuous. 
		\end{enumerate}
\end{solutions}

\begin{prb}{2021-J-II-6 (Real Analysis)}
	Using Lebesgue measure on $\mathbb{R}$, prove that for almost every $\lambda > 1$, the set $\brac*{\lambda^{n}\mod{1}: n \in \mathbb{Z}^{+}}$ is dense in $[0,1]$. 
\end{prb}
...
\end{prb}
\begin{solutions}
	Let $E \subset \mathbb{R}$ be Lebesgue measurable, and $f: \mathbb{R} \to \mathbb{R}$ be Lipschitz. 
\end{solutions}


\begin{prb}{2021-J-I-4 (Algebra)}
	Let $X$ denote a metric space and $C(X, \mathbb{R})$ denote the ring of continuous real valued functions defined on $X$. Show that if $C(X, \mathbb{R})$ is a Noetherian ring, then $X$ is a finite set. 
\end{prb}
\begin{solutions}
	\textcolor{red}{[!! Complete Later !!]}
\end{solutions}
...
\begin{prb}{2021-A-I-2 (Complex Analysis)}
	Let $P(z) = z^{n} + a_{1}z^{n-1} + \dotsm + a_{n}$. Prove that either there is $|z| = 1$ such that $|P(z)| > 1$ or $P(z) = z^{n}$. 
\end{prb}
\begin{solutions}
	This problem requires the Maximum Modulus Principle, which states the following 
		\begin{quote}
			(\textbf{Maximum Modulus Principle}) Let $f$ be a holomorphic function on some bounded and connected open subset $D$ of the complex plane $\mathbb{C}$ and taking complex values. If $z_{0} \in D$ such that $\abs{f(z_{0})} \geq \abs{f(z)}$ for all $z$ in some neighborhood of $z_{0}$, then $f$ is constant on $D$. 
		\end{quote} 
	So assume that for all $|z| = 1$, $\abs{P(z)} \leq 1$. Define the function 
		\begin{equation}
			g(z) = z^{n}P(1/z) = 1 + a_{1}z + \dotsm + a_{n}z^{n}. 
		\end{equation}
	Then for all $|z| = 1$, $|g(z)| = |z|^{n}|P(1/z)| \leq 1$ and $g(0) = 0$. Hence, by the Maximum Modulus Principle, $g(z) = 1$ for all $z$. In particular, this means that $P(z^{-1}) = z^{-n} \implies P(z) = z^{n}$. 
\end{solutions}
\begin{prb}{2021-A-I-4 (Algebra)}
	If $\beta$ is a nonzero algebraic number, find coefficients of the minimal polynomial over $\mathbb{Q}$ for $\beta^{-1}$ in terms of coefficients of the minimal polynomial over $\mathbb{Q}$ for $\beta$. 
\end{prb}
\begin{solutions}
	Let $\beta$ be a nonzero algebraic number, and let $q(x) = x^{n} + a_{n}x^{n- 1} + \dotsm + a_{1}$ be the minimal polynomial of $\beta$ over $\mathbb{Q}$. I.e., 
		\begin{equation}
			\beta^{n} + a_{n}\beta^{n-1} + \dotsm + a_{1} = 0. 
		\end{equation}
	From this, we observe that 
		\begin{equation}
			a_{1}(\beta^{-1})^{n} + a_{2}(\beta^{-1})^{n-1} + \dotsm + 1 = 0. 
		\end{equation}
	If $a_{1} \neq 0$, then $\beta = 0$ is a root, which contradicts our hypothesis that $\beta$ is nonzero. Hence multiplying both sides by $a_{1}^{-1}$ gets us: 
		\begin{equation}
			\beta^{-n} + \frac{a_{2}}{a_{1}}\beta^{1 - n} + \dotsm + \frac{1}{a_{1}} = 0. 
		\end{equation}
	We claim that 
		\begin{equation}
			p(x) = x^{n} + \frac{a_{2}}{a_{1}}x^{n - 1} + \dotsm + \frac{1}{a_{1}} 
		\end{equation}
	is the minimal polynomial of $\beta^{-1}$ over $\mathbb{Q}$. We will show that $p$ is irreducible over $\mathbb{Q}$. Assume to the contrary. Then since 
		\begin{equation}
			p(x) = x^{n}q(1/x) \implies q(1/x) = x^{-n}p(x) \implies q(x) = x^{n}p(1/x). 
		\end{equation}
	If $p(x)$ is reducible, which means it can be factored into the product of polynomials of degree less than $n$, then it follows that $q(x)$ can also be factorized into the product of polynomials degree less than $n$, contradicting irreducibility of $q$. Hence, $p$ is irreducible. Likewise, it is straightforward to see that $p$ has minimal degree among all polynomials in $\mathbb{Q}[x]$ that vanish at $\beta$ since if otherwise, $q$ cannot be the minimal polynomial of $\beta$. 
\end{solutions} 
\begin{prb}{2021-A-I-5 (Algebra)}
	Let $R$ be an integral domain with field of fractions $Q$. Show that $Q/R \mathop{\otimes}\limits_{R} Q/R = 0$. 
\end{prb}
\begin{solutions}
	In a break with tradition, we will use short exact sequences to answer this problem. Taking $\iota: R \hookrightarrow Q$ and $\pi: Q \to Q/R$ to be the inclusion and quotient maps, respectively, we obtain the short exact sequence 
		\begin{equation}
			0 \longrightarrow R \xrightarrow{\iota} Q \xrightarrow{\pi} Q/R \longrightarrow 0. 
		\end{equation}
	Tensoring the short exact sequence with $Q/R$: 
		\begin{equation}
			Q/R \mathop{\otimes}_{R} R \longrightarrow Q/R \mathop{\otimes}_{R} Q \longrightarrow Q/R \mathop{\otimes}_{R} Q/R \longrightarrow 0. 
		\end{equation}
	First, we note that since $Q/R$ is an $R$-module, $Q/R \mathop{\otimes}_{R} R \cong Q/R$ via the module isomorphism $\varphi: Q/R \mathop{\otimes}_{R}R \to Q/R$ defined by $\varphi(m \otimes r) = mr$. Now consider $Q/R \mathop{\otimes}_{R} Q$. A typical element of this module is a finite linear combination 
		\begin{equation}
			\sum_{i}(x_{i} + R) \otimes q_{i}, \quad q_{i} \in Q, x_{i} \in Q. 
		\end{equation}
	Consider just the simple tensor $(x + R) \otimes q$. Since $x \in Q$, let $x = a/b$, where $b \neq 0$. Then we observe that 
		\begin{equation}
			b(x + R) \otimes q) = b(a/b + R) \otimes q = (a + R) \otimes q = 0 \otimes q = 0 \in Q/R \mathop{\otimes}_{R} Q,
		\end{equation}
	where the third equality stems from the fact that $a \in R$. Hence, since $b$ is invertible in $Q$, we obtain $(x + R) \otimes q = 0$. Since this was an arbitrary simple tensor, we conclude that $Q/R \mathop{\otimes}_{R} Q = 0$. Hence, we obtain the short exact sequence 
		\begin{equation}
			Q/R \longrightarrow 0 \longrightarrow Q/R \mathop{\otimes}_{R} Q/R \longrightarrow 0. 
		\end{equation}
	This implies that $Q/R \mathop{\otimes} Q/R = 0$. Hence, the proof concludes. 
\end{solutions}
...
\end{prb}
\begin{solutions}
	Let $M$ be a compact manifold (without boundary) and $\pi: M \to S^{1}$ a submersion onto the circle. Assume to the contrary that $\h{1}(M) = 0$ which means that every closed form on $M$ is an exact form. Since $\h{1}(S^{1}) \cong \mathbb{R}$, let $[\omega]$ be a generator of this cohomology group, where $\omega$ is a nowhere vanishing closed 1-form on $S^{1}$. Since $\pi$ is a submersion, the 1-form $\pi^{\ast}\omega$ must also be a nowhere vanishing closed form on $M$. By our hypothesis on the de Rham cohomology group in degree one of $M$, $\pi^{\ast}\omega$ is exact, which means there exists a smooth function $f$ such that $\pi^{\ast}\omega = df$. Since $M$ is compact and $f$ is smooth, $f$ must attain either a maximum or minimum value at some $p_{0} \in M$. This means that $df_{p_{0}} = 0$. But this contradicts our claim that $\pi^{\ast}\omega$ is nowhere vanishing. Hence, by contradiction, $\h{1}(M) \neq 0$. 
\end{solutions}
\begin{prb}{2021-A-I-3 (Real Analysis)}
	Characterize all functions $f \in L^{2}(\mathbb{R}^{n})$ such that $f \ast f = f$. Here $\ast$ means convolution. 
\end{prb}
\begin{solutions}
	The key ingredients for this problem are Plancherel's Theorem, properties of the Fourier transform, and the Fourier Inversion Theorem: 
		\begin{quote}
			(\textbf{Plancherel's Theorem}) Suppose $f$ and $\hat{f}$ are both continuous and improperly Riemann integrable on $\mathbb{R}^{n}$. Then 
				\begin{equation}
					\int \abs{f(x)}^{2}\;dx = \int \abs{\hat{f}(\xi)}^{2}\;d\xi. 
				\end{equation}
				
			(\textbf{Fourier Transforms and Convolutions}) If $h(x) = (f \ast g)(x)$, then $\hat{h}(\xi) = \hat{f}(\xi)\hat{g}(\xi)$. 
		\end{quote}
	Suppose $f \in L^{2}(\mathbb{R}^{n})$; by equality, $f \ast f \in L^{2}(\mathbb{R}^{n})$. Hence, taking the Fourier transform of both sides, $\hat{f}^{2} = \hat{f}$. This means that either $\hat{f} = 0$ or $\hat{f} = 1$. Hence, $\hat{f}(\xi) = \chi_{E}$ for some measurable set $E$. Applying Plancherel's Theorem, we find that $\hat{f} \in L^{2}$. Therefore, $E$ must have finite measure. Applying the inversion theorem, we can recover $f$ by taking the inverse Fourier transform. Therefore, $f$ must be of the form 
		\begin{equation}
			f(x) = \int_{E}\hat{f}(\xi)e^{2\pi i \xi x}\;dx. 
		\end{equation}
\end{solutions}
\begin{prb}{2021-A-II-2 (Real Analysis)}
	Suppose $X$ is a compact metric space and $\rho: C(X) \to C$ is an algebra homomorphism ($\rho$ respects addition and multiplication of functions). Prove that $\rho$ satisfies $\abs{\rho(f)} \leq \norm{f}$. ($C(X)$ is the Banach space of continuous complex-valued functions on $X$ has its usual supremum norm.)
\end{prb}
\begin{solutions}
	If $\rho$ is the zero algebra homomorphism (i.e., $\rho(f) = 0$ for all $f \in C(X)$), then the claim is trivially true. So suppose $\rho$ is not the trivial homomorphism. Since for any $f \in C(X)$, 
		\begin{equation}
			\rho(f) = \rho(1f) =\rho(1)\rho(f), 
		\end{equation}
	it follows that $\rho(1) = 1$. If $f$ never vanishes, then $1 = \rho(1) = \rho(f)\rho(1/f)$ so that $\rho(f) \neq 0$. If $\abs{\rho(f)} > \norm{f}$, then $g = f/\rho(f)$ satisfies $\norm{g} < 1$. Hence, $1 - g$ never vanishes, and is therefore invertible. This means that 
		\begin{equation}
			\rho(1 - g) = \rho(1) - \rho(g) = 1 - \rho(f)/\rho(f) = 0, 
		\end{equation}
	which is a contradiction. Hence, $\abs{\rho(f)} \leq \norm{f}$ holds. 
\end{solutions}
\begin{prb}{2021-A-II-5 (Algebra)}
...
\begin{solutions}
	First, observe that 
		...
		\end{equation}
	so that $\mathbb{Q}(\alpha, \omega) = \mathbb{Q}(\alpha, \sqrt{-3})$. Since $[\mathbb{Q}(\alpha, \sqrt{-3}):\mathbb{Q}] = 3 \cdot 2 = 6$, a primitive element of $K$ must be of degree 6. Consider the sequence $\{\beta_{n}\}= \{i\sqrt{3}\alpha n\}_{1}^{\infty}$. $\beta_{n}$ is a root of 
		\begin{equation}
			x^{6} + 6n^{2}x^{4} + 9n^{4}x^{2} + 27n^{6}. 
		\end{equation}
	Moreover, 
		\begin{equation}
			\frac{\beta_{n}^{3} + 3n^{2}\beta_{n}}{2n^{3}} =-i\sqrt{3}, \qquad \alpha = \frac{\beta_{n}}{i\sqrt{3}n}. 
		\end{equation}
	Hence, $K = \mathbb{Q}(\alpha,\sqrt{-3}) \subset \mathbb{Q}(\beta_{n})$. On the other hand, $\beta_{n} \in K$ so that $\mathbb{Q}(\beta_{n}) \subset K$. This means $\mathbb{Q}(\beta_{n}) = K$, and so $\{\beta_{n}\}$ is an infinite series of primitive elements. 
\end{solutions}
\begin{prb}{2021-J-I-1 (Algebra)}
	Let $G$ denote a finite group, $P \subset G$ a Sylow $p$-subgroup of $G$, and $Z(P)$ the center of $P$. If $x, y \in Z(P)$ are conjugate in $G$, then show that $x, y$ are also conjugate in $N(P)$, the normalizer of $P$ in $G$. 
\end{prb}
\begin{solutions}
	\textcolor{red}{[!! Complete Later !!]}
\end{solutions}
...
\end{solutions}
\begin{prb}{2020-J-I-3 (Real Analysis)}
	Let $p > 1$ be a real number, and set $q = \frac{p}{p - 1}$. Suppose that $f \in L^{p}(\mathbb{R})$ and $g \in L^{q}(\mathbb{R})$. Prove that $\lim_{n \to \infty}\int_{\mathbb{R}}f(x)g(nx)\;d\mu =0$, where $\mu$ denotes the Lebesgue measure on $\mathbb{R}$. 
\end{prb}
\begin{solutions}
	Let $p > 1$ be a real number, and $q$ the conjugate exponent of $p$. Further suppose that $f \in L^{p}(\mathbb{R})$ and $g \in L^{q}(\mathbb{R})$. By H\"older's inequality, we observe that 
		\begin{equation}
			\abs{\int_{\mathbb{R}}f(x)g(nx)\;d\mu} \leq \int_{\mathbb{R}}\abs{f(x)g(nx)}\;d\mu \leq \norm{f}_{p}\norm{g(n\cdot)}_{q}. 
		\end{equation}
	Hence, it suffices to show that $\norm{g(n\cdot)}_{q} \to 0$. Let $y = nx \implies dy =ndx \implies dx = n^{-1}dy$. Then 
		\begin{equation}
			\norm{g(n\cdot)}_{q}^{q} \coloneqq \int_{\mathbb{R}}|g(nx)|^{q}\;dx = n^{-1}\int_{\mathbb{R}}|g(y)|^{q}\;dy = n^{-1}\norm{g}_{q}^{q}. 
		\end{equation}
	Hence, $\norm{g(n \cdot)}_{q} = \frac{1}{n^{1/q}} \norm{g}_{q}$. From this, we conclude that $\norm{g(n \cdot )}_{q} \longrightarrow 0$ since $q > 1$, which then concludes the proof. 
\end{solutions}
...
\end{solutions}
\begin{prb}{2020-J-I-6 (Real Analysis)}
	Let $E \subset \mathbb{R}^{n}$ be a measurable set of positive Lebesgue measure, and let $S^{n - 1}$ denote the unit sphere $\brac*{w \in \mathbb{R}^{n}: \norm{w} = 1}$. Given any $w \in S^{n - 1}$, prove there exist $x, y \in E$ such that $$w = \frac{x - y}{\norm{x - y}}.$$
\end{prb}
\begin{solutions}
	Let $E \subset \mathbb{R}^{n}$ be a measurable set of positive Lebesgue measure, and select some $w \in S^{n - 1}$. By inner regularity of the Lebesgue measure, there exists a compact set $K_{1} \subset E$ such that $m(K_{1}) > 0$, where $m$ denotes the Lebesgue measure on $\mathbb{R}^{n}$. On the other hand, given $\epsilon \in (0,1)$, by outer regularity of the Lebesgue measure, there exists an open subset $U \subset \mathbb{R}^{n}$ such that $K_{1} \subset U$ and $m(U) \leq (1 + \epsilon)m(K_{1})$. Since $K_{1}$ is compact, $d_{1} = d(K_{1}, U^{c}) > 0$. So choose an arbitrary $r \in (0, d_{1})$, and consider the set $K_{1} + rw$. We claim that $K_{1} + rw \subset U$. If not, then 
		\begin{equation}
			d(K_{1}, U^{c}) < \norm{rw} = r < d_{1}, 
		\end{equation}
	which is a contradiction. Therefore, the set $K_{1} \cup (K_{1} + rw) \subset U$, which then implies that 	
		\begin{equation}
			m(U) \geq m(K_{1} \cup (K_{1} + rw)) = m(K_{1}) + m(K_{1} + rw) - m(K_{1} \cap (K_{1} + rw)). 
		\end{equation}
	Since the Lebesgue measure is translation invariant, 
		\begin{equation}
			m(K_{1} \cap (K_{1} + rw)) \geq 2m(K_{1}) - m(U) \geq 2m(K_{1}) - (1 + \epsilon)m(K_{1}) = (1 - \epsilon)m(K_{1}) > 0. 
		\end{equation}
	Hence, $K_{1} \cap (K_{1} + rw)$ is nonempty. This gives us a nested sequence of nonempty sets $\varnothing \neq K_{1} \cap (K_{1} + rw) \subset K_{1} \subset E$. Let $x \in K_{1} \cap (K_{1} + rw) \subset E$. Then the element $y = x - rw$ is contained in $K_{1} \subset E$. We observe that 
		\begin{equation}
			\frac{x - y}{\norm{x - y}} = \frac{x - (x - rw)}{\norm{x - (x - rw)}} = \frac{rw}{\norm{rw}} = w. 
		\end{equation}
\end{solutions}
\begin{prb}{2020-J-I-2 (Geometry/Topology)}
	Let $M$ and $N$ be smooth compact connected oriented $n$-manifolds without boundary. Suppose that $\pi_{1}(M)$ is finite, but that $\pi_{1}(N)$ is infinite. Prove that every smooth map $\Psi: M \to N$ has degree zero. 
\end{prb}
...
\end{prb}
\begin{prb}{2020-A-II-6 (Real Analysis)}
...
\end{solutions}
\begin{prb}{2019-J-I-3 (Geometry/Topology)}
	Let $M$ and $N$ be connected, compact, oriented $n$-manifolds. Prove that if $\h{k}(N) \neq 0$ and $\h{k}(M) = 0$ for some $k \neq 0$, then any smooth map $f: M \to N$ has degree $0$.  
\end{prb}
\begin{solutions}
	Assume the given hypotheses. The map $f$ induces a smooth pullback map $f^{\ast}: \h{k}(N) \to \h{k}(M)$; since $\h{k}(M)$ is zero, it follows that $f^{\ast}$ is the zero map. 
	
	Since $\h{k}(N) \neq 0$, there exists a closed $k$-form $\alpha$ such that $[\alpha] \neq 0$. Since $f^{ast}$ is the zero map, $[f^{\ast}\alpha] = 0 \in \h{k}(M)$. Therefore, $f^{\ast}\alpha$ is an exact form on $M$, which means there exists a $(k - 1)$-form $\beta$ on $M$ such that $f^{\ast}\alpha = d\beta$. Now choose a $(n - k)$-form $\gamma$ on $N$ such that 
		\begin{equation}
			\int_{N}\alpha \wedge \gamma \neq 0; 
		\end{equation} 
	this is possible due to the \textit{Poincar{\'e} Duality}: if $N$ is a compact oriented $n$-manifold, then there exists a map $\alpha \smile \beta = \int_{N} \alpha \wedge \beta$ that descends to a nondegenerate bilinear pairing in cohomology:
		\begin{equation}
			H^{k}(M, \mathbb{R}) \otimes H^{n - k}(M, \mathbb{R}) \to \mathbb{R}. 
		\end{equation}
	Then 
		\begin{equation}
			\int_{M}f^{\ast}(\alpha \wedge \gamma) = \int_{M}f^{\ast}\alpha \wedge f^{\ast}\gamma = \int_{M}d\beta \wedge f^{\ast}\gamma. 
		\end{equation}
	Since $f^{\ast}\gamma$ is closed, 
		\begin{equation}
			d(\beta \wedge f^{\ast}\gamma) = d\beta \wedge f^{\ast}\gamma. 
		\end{equation}
	Therefore, 
		\begin{equation}
			\int_{M}f^{\ast}(\alpha \wedge \gamma) = \int_{M}d(\beta \wedge f^{\ast}\gamma) = \int_{\partial M}\beta \wedge f^{\ast}\gamma = 0, 
		\end{equation} 
	since $M$ is without boundary. But $\alpha \wedge \gamma$ is a top form on $N$. Hence, we conclude that $\operatorname{deg}(f) = 0$. 
\end{solutions}


\begin{prb}{2019-J-II-4 (Real Analysis)}
...
\begin{prb}{2019-A-II-4 (Geometry/Topology)}
...
\end{prb}
\begin{solutions}
	Let $M^{n}$ be a compact, connected, oriented $n$-manifold, where $n$ is odd. We recall the following two facts:
		\begin{quote}
			(\textbf{Euler Characteristic}) The Euler characteristic is defined to be the constant: 
				\begin{equation}
					\chi(M^{n}) \coloneqq \sum_{1}^{n}(-1)^{n}\operatorname{rank}(H_{i}(M^{n})).
				\end{equation}
				
			(\textbf{Euler Characteristic of Disjoint Union}) The Euler characteristic is additive over disjoint unions. 
		\end{quote}
	Let $U = M^{n}\setminus \{\text{pt}\}$ and $V$ be an open ball around $p$ so that $U \cap V$ is homeomorphic to $\mathbb{R}^{n}\setminus \{\text{pt}\}$. Hence, we obtain 
		\begin{equation}
			\chi(U) + \chi(V) = \chi(M) + \chi(\mathbb{R}^{n}\setminus \{\text{pt}\}). 
		\end{equation}
	Since $V$ is contractible, $\chi(V) = 0$; since $M$ is a compact, odd-dimensional manifold, $\chi(M) = 0$; since $\mathbb{R}^{n}\setminus \{\text{pt}\}$ deformation retracts onto $S^{n - 1}$, which is an even-dimensional sphere, $\chi(\mathbb{R}^{n}\setminus \{\text{pt}\}) = 2$. Hence, 
		\begin{equation}
			\chi(U) = 2 - 1= 1. 
		\end{equation}
\end{solutions}

\begin{prb}{2019-A-I-3 (Real Analysis)}
	For every positive integer $n$, let $T_{n}: \mathbb{X} \to \mathbb{Y}$ be a linear transformation with $\mathbb{X}, \mathbb{Y}$ normed vector spaces. Suppose that $\norm{T_{n}} \leq 1$. Let $\{x_{\alpha}\}$ be a dense subset of $\mathbb{X}$. Suppose that for any $\alpha$, 
		\begin{equation*}
			\lim_{n} T_{n}(x_{\alpha}) = y_{\alpha}
		\end{equation*}
	for some $y_{\alpha} \in \mathbb{Y}$. Suppose further that if $x = \lim_{i}x_{\alpha_{i}}$, then $y_{\alpha_{i}}$ converges to some $y_{x} \in \mathbb{Y}$. Show that $$\lim_{n}T_{n}(x) = y_{x}.$$ 
\end{prb}
\begin{solutions}
	Assume all of the given hypotheses of the problem. Then 
		\begin{align}
			\begin{split} 
				\norm{T_{n}(x) - y_{x}} &\leq \norm{T_{n}(x) - T_{n}(x_{\alpha_{i}})}  + \norm{T_{n}(x_{\alpha_{i}}) - y_{\alpha_{i}}} + \norm{y_{\alpha_{i}} - y_{x}}  \\
				&\leq \norm{T_{n}}\norm{x - x_{\alpha_{i}}} + \norm{T_{n}(x_{\alpha_{i}}) - y_{\alpha_{i}}} +  \norm{y_{\alpha_{i}} - y_{x}} \\
				&\leq \norm{x - x_{\alpha_{i}}} + \norm{T_{n}(x_{\alpha_{i}}) - y_{\alpha_{i}}} + \norm{y_{\alpha_{i}} - y_{x}}. 
			\end{split} 
		\end{align}
	Let $\epsilon > 0$. Choose $I$ large enough so that for all $i \geq I$, $\norm{x - x_{\alpha_{i}}}, \norm{y_{x} - y_{\alpha_{i}}} < \epsilon/3$. Now choose $N$ large enough so that for all $n\geq N$, $\norm{T_{n}(x_{\alpha_{i}}) - y_{\alpha_{i}}} < \epsilon/3$.  Hence, it follows that $T_{n}(x) \to y_{x}$. 
\end{solutions}
\begin{prb}{2019-A-II-1 (Complex Analysis)}
	Suppose $f: \mathbb{C} \to \mathbb{C}$ is continuous on the closed disk $\brac*{z: \abs{z}\leq 1} \subset \mathbb{C}$ and holomorphic on its interior. If $f$ restricted to the boundary of this disk only takes values on the curve $xy = 1$ (where a point $z \in \mathbb{C}$ is written as $z = x + iy$, with $x, y \in \mathbb{R}$), show that $f$ is constant on the whole disk. 
\end{prb}
\begin{solutions}
	\textcolor{red}{[!! Complete Later !!]}
\end{solutions}

\begin{prb}{2019-A-I-4 (Geometry/Topology)}
	Let $f: \mathbb{RP}^{3} \to \mathbb{T}^{3} = S^{1}\times S^{1}\times S^{1}$ be a smooth map. Show that $f$ is not a immersion. 
\end{prb}
\begin{solutions}
	Let $f: \mathbb{RP}^{3} \to \mathbb{T}^{3} = S^{1}\times S^{1}\times S^{1}$ be a smooth map. Assume to the contrary that $f$ is an immersion. Since $\mathbb{RP}^{3}$ is smooth, compact, connected, and nonempty, it follows from the Comps Lemma that $f$ is a covering map. In particular, this forces the induced group homomorphism, $f_{\ast}: \pi_{1}(\mathbb{RP}^{3}) \to \pi_{1}(\mathbb{T}^{3})$ to be injective. We recall that 
		\begin{equation}
			\pi_{1}(\mathbb{RP}^{3}) \cong \mathbb{Z}/2\mathbb{Z} \qquad \text{ and } \qquad \pi_{1}(\mathbb{T}^{3}) \cong \bigtimes_{j = 1}^{3}\pi_{1}(S^{1}) \cong \mathbb{Z}^{3}. 
		\end{equation}
	Since $f_{\ast}$ is injective, it cannot be the trivial homomorphism. Therefore, $f_{\ast}$ must map the equivalence class $[\gamma] \in \pi_{1}(\mathbb{RP}^{3})$ of order two to a nonzero equivalence class $[\zeta]$ with order dividing $2$ in $\pi_{1}(\mathbb{T}^{3})$. However, since $\mathbb{Z}\times \dotsm \times \mathbb{Z}$ is torsion-free, there exists no nonzero element of finite order in $\mathbb{Z} \times \mathbb{Z} \times \mathbb{Z} \cong \pi_{1}(\mathbb{T}^{3})$. This leads to a contradiction. Therefore, $f$ cannot be an immersion. 
\end{solutions}

\subsection{August 2018}
\begin{prb}{2018-A-I-2 (Geometry/Topology)}
	Let $M^{n}$ be a compact, connected, oriented $n$-manifold, and assume that $n$ is odd. Compute the Euler characteristic $\chi(M\setminus \{\text{pt}\})$. 
\end{prb}
\begin{solutions}
	Let $U = M^{n}\setminus \{\text{pt}\}$ and $V$ be an open ball around $p$ so that $U \cap V$ is homeomorphic to $\mathbb{R}^{n}\setminus \{\text{pt}\}$. Hence, we obtain 
	\begin{equation}
	...
	\end{equation}
\end{solutions}
\begin{prb}{2018-A-I-4 (Geometry/Topology)}
	Let $D$ denote the closed unit ball in $\mathbb{R}^{n}$, and let $\Psi: D \to D$ be a smooth map. Suppose, moreover, that $\Psi$ sends the $(n - 1)$-sphere $S^{n - 1} = \partial D$ to itself via some diffeomorphism. If $\omega$ is any smooth $n$-form on $D$, use the Poincar{\'e} Lemma to prove that 
		\begin{equation*}
			\int_{D}\Psi^{\ast}\omega = \pm \int_{D}\omega. 
		\end{equation*}
	Then use this to show that $\Psi$ must be surjective. 
\end{prb}
\begin{solutions}
	The Poincar{\'e} Lemma states the following: 
		\begin{quote}
			(\textbf{Poincar{\'e} Lemma}) If $U$ is a star-shaped open subset of $\mathbb{R}^{n}$ or $\mathbb{H}^{n}$, then $\h{p}(U) = 0$ for $p\geq 1$. 
		\end{quote}
	By the Poincar{\'e} Lemma, it follows that $\omega = d\eta$ for some $\eta$. By Stoke's Theorem, we have 
		\begin{equation}
			\int_{D}\Psi^{\ast}\omega = \int_{\partial D}\Psi^{\ast}\eta = \pm \int_{\partial D}\eta =\pm \int_{D}\omega, 
		\end{equation}
	where the sign depends on whether or not $\Psi$ preserves orientation. We observe that the quotient of the integral is $\pm 1$, and computes the degree of the map $\Psi$. But if $\Psi$ were not surjective, then we could compute the degree of $\Psi$ locally at a point not in the image of $\Psi$, so $\Psi$ would have degree 0 (a contradiction). Hence, $\Psi$ must be surjective. 
\end{solutions}
...
\end{prb}
\begin{solutions}
	Translating $E$ and $F$ is necessary, the Lebesgue Density Theorem implies that there exists an interval $I= [-\delta, \delta]$ such that $\frac{3}{4}m(I) \leq m(I \cap E)$ and $\frac{3}{4}\leq m(I \cap F)$, and such that these inequalities also hold for the intervals $rI$ with $r < 1$. Suppose $a \in \frac{1}{2}I$ but $a \notin E + F$. Then $E \cap (F - a) = \varnothing$ so $m((E \cup (F - a)) \cap I) = m(E \cap I) + m((F - a) \cap I)$. Since $(F - a) \cap I \supset (F \cap \frac{1}{2}I) - a$, $m((F - a)\cap I) \geq \frac{3}{8}m(I)$, while $m(E \cap I) \geq \frac{3}{4}m(I)$. Therefore, 
		\begin{equation}
			m(E \cap I) + m((F - a)\cap I) \geq \frac{9}{8}m(I). 
		\end{equation}
	On the other hand, $m((E \cup (F - a))\cap I) \leq m(I)$, which gives a contradiction. Hence, $\frac{1}{2}I \subset E + F$. 
\end{solutions} 
...
\begin{prb}{2018-A-I-5 (Real Analysis)}
	If $\{\mu_{n}\}$ are finite positive measures on $[0,1]$ that converge in the weak$^{\ast}$ topology to $\mu$, prove $\{\mu_{n} \times \mu_{n}\}$ converges in the weak$^{\ast}$-topology to $\mu \times \mu$. 
\end{prb}
\begin{solutions}
	Recall that if $\mu_{n} \to \mu$ in the weak$^{\ast}$ topology, then for all $f \in C([0,1])$, 
		\begin{equation}
			\int_{[0,1]}f\;d\mu_{n}(x) \to \int_{[0,1]}f\;d\mu(x). 
		\end{equation}
	Therefore, our goal is to show that for any $f \in C([0,1]^{2})$, 
		\begin{equation}
			\int_{[0,1]^{2}}f\;d(\mu_{n} \times \mu_{n}) \longrightarrow \int_{[0,1]^{2}}f\;d(\mu \times \mu). 
		\end{equation}
	By the Stone-Weierstrass, functions of the form $g(x)h(y)$, where $g(x), h(y) \in C([0,1])$ are dense in $C([0,1]^{2})$. Hence it suffices to prove that the claim holds for these functions. Since continuous functions on compact spaces are in $L^{1}$, by Fubini-Tonelli, 
		\begin{multline}
			\int g(x)h(y)\;d(\mu_{n}\times \mu_{n}) = \left(\int g(x)\;d\mu_{n}(x)\right)\left(\int h(y)\;d\mu_{n}(y)\right)  \\
			\longrightarrow \left(\int g(x)\;d\mu(x)\right)\left(\int h(y)\;d\mu(y)\right) = \int g(x)h(y)\;d(\mu \times\mu). 
		\end{multline}
	Hence, $\mu_{n} \times \mu_{n} \to \mu \times \mu$ in the weak$^{\ast}$-topology. 
\end{solutions}
...
\end{solutions}
\begin{prb}{2017-J-I-2 (Real Analysis)}
	Recall that $L^{2}(\mathbb{R})$ is the space of Lebesgue measurable functions $f: \mathbb{R}\to \mathbb{C}$ so that $|f|^{2}$ is integrable. Show that the linear map 
		\begin{equation*}
			T: L^{2}(\mathbb{R}) \to L^{2}(\mathbb{R}), \qquad T(f)(x)\coloneqq f(x - 1), \quad \forall x \in \mathbb{R}, f \in L^{2}(\mathbb{R})
		\end{equation*}
	has no non-zero eigenvector. 
\end{prb}
\begin{solutions}
	Assume to the contrary that $T$ has a non-zero eigenvector $f$, which means that $f(x - 1) = \lambda f(x)$ for all $x \in \mathbb{R}$. Since $f$ is not identically zero, $\lambda \neq 0$. In particular, we can write $f(x) = \lambda^{-n}f(x - n)$ for all $n \in \mathbb{Z}$. If we denote $A = \int_{0}^{1}|f(x)|^{2}\;dx$, then
		\begin{equation}
			\int_{n}^{n + 1}|f(x)|^{2}\;dx = |\lambda|^{-2n}A \implies \int_{\mathbb{R}}|f|^{2}\;dm = \sum_{n \in \mathbb{Z}}|\lambda|^{-2n}A, 
		\end{equation}
	which does not converge since $\lambda \neq 0$. This contradicts our claim that $f \in L^{2}$. 
\end{solutions}
...
\begin{prb}{2017-J-II-4 (Geometry/Topology)}
	Let $X = \mathbb{RP}^{2} \vee \mathbb{RP}^{2}$ be the ``wedge'' of two real projective planes, meaning the quotient space obtained from the disjoint union $\mathbb{RP}^{2} \sqcup \mathbb{RP}^{2}$ by identifying a single point $p$ in one copy of $\mathbb{RP}^{2}$ with a single point $\ti{p}$ in the other. (1) What is the fundamental group of $X$? (2) Give a concrete description of the universal cover $\ti{X}$ of $X$, accompanied by a drawing of $X$. 
\end{prb}
\begin{solutions}
	\vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
		\item Let $X = \mathbb{RP}^{2} \vee \mathbb{RP}^{2}$. Since the fundamental group of the wedge product of two manifolds corresponds to the free product of the fundamental groups of each manifold, and the fundamental group of $\mathbb{RP}^{2}$ is $\mathbb{Z}/2\mathbb{Z}$, we conclude that 
			\begin{equation}
				\pi_{1}(X) \cong (\mathbb{Z}/2\mathbb{Z}) \ast (\mathbb{Z}/2\mathbb{Z}). 
			\end{equation}
		\item Recall that $S^{2}$ is the universal covering of $\mathbb{RP}^{2}$; a base point in $\mathbb{RP}^{2}$ has two preimages in $S^{2}$. To obtain $\ti{X}$, wedge another sphere to each of the two preimages of the base point. Then wedge another sphere to each the new preimages of the base point, and keep repeating. $\ti{X}$ resembles the following diagram: 
	\end{enumerate}
	\begin{center}
		\begin{tikzpicture}
			\draw[Stealth-Stealth] (-5,0) -- (5,0); 
			\draw (-3,1) circle(1); \draw[fill=blue] (-3,0) circle (3pt);
			\draw (0,1) circle (1); \draw[fill=blue] (0,0) circle (3pt);
			\draw (3,1) circle (1); \draw[fill=blue] (3,0) circle (3pt);
			\draw[Stealth-] (-5.5,1) -- (-4.5,1); \draw[-Stealth] (4.5,1) -- (5.5,1); 
		\end{tikzpicture}
	\end{center}
\end{solutions}
...
\end{solutions}
\begin{prb}{2017-A-II-1 (Real Analysis)}
	Suppose that $f \in C([0,1])$ is a continuous real-valued function on $[0,1]$ for which 
		\begin{equation*}
			\int_{0}^{1}x^{n}f(x)dx = 0
		\end{equation*}
	for all non-negative integers $n$. Does it follow that $f(x) = 0$ for all $x \in [0,1]$. 
\end{prb}
\begin{solutions}
	It is necessarily true that $f(x) =0$ for all $x \in [0,1]$. By the Stone-Weierstra{\ss} Theorem, one can show that the hypothesis forces 
		\begin{equation}
			\int_{0}^{1}f(x)g(x)dx = 0, 
		\end{equation} 
	for all continuous functions $g(x)$ on $[0,1]$. Taking $g(x) = f(x)$ so that $f(x)g(x) = f(x)^{2}$, we observe that 
		\begin{equation}
			\int_{0}^{1}f^{2}(x)dx = 0 \implies f(x)^{2} = 0 \implies f(x) =0. 
		\end{equation}
...
\begin{prb}{2013-J-I-4 (Real Analysis)}
	Let $f: \mathbb{R} \to \mathbb{R}$ be smooth and such that for every $x$ 
		\begin{equation*}
			\int_{-\infty}^{\infty}\frac{\abs{f(x) - f(y)}}{\abs{x -y}^{2}}\;dy < \infty. 
		\end{equation*}
	Prove $f$ is constant. 
\end{prb}
\begin{solutions}
	Assume the given conditions of the problem. Our goal is to show that $f'(x) = 0$ for every $x \in \mathbb{R}$. Assume to the contrary. Since $f$ is smooth, by Taylor's Theorem, there exists a function $H(y)$ such that 
		\begin{equation}
			f(y) = f(x) + f'(x)(y - x) + H(y), \qquad \lim_{y \to x}H(y) = 0. 
		\end{equation}
	In particular, 
		\begin{equation}
			\abs{f(y) - f(x)} = \abs{f'(x)(y -x) + H(y)} \implies \frac{\abs{f(y) -f(x)}}{\abs{x - y}^{2}} = \abs{ \frac{f'(x)}{x - y} + \frac{H(y)}{(x -y)^{2}} }. 
		\end{equation}
	Since $H(y) \to 0$, for every $\epsilon > 0$, there exists a $\delta > 0$ such that $|x - y| < \delta$ implies $|H(y)| < \epsilon\abs{y - x}$. Taking $\epsilon = f'(x)/2$, for any $y \in (x - \delta, x + \delta)$, 
		\begin{align}
			\begin{split}
				\frac{\abs{f(x) - f(y)}}{\abs{x - y}^{2}} &= \abs{\frac{f'(x)}{x - y} + \frac{H(y)}{(x -y)^{2}} } \\
				&\geq \frac{\abs{f'(x)}}{\abs{x - y}} - \frac{\abs{H(y)}}{\abs{x - y}^{2}} > \frac{\abs{f'(x)}}{\abs{x - y}} - \frac{\abs{f'(x)}}{2\abs{x - y}}
			\end{split}
		\end{align}
	Hence, 
		\begin{equation}
			\int_{x -\delta}^{x + \delta}\frac{\abs{f(x) - f(y)}}{\abs{x - y}^{2}}\;dy \geq \frac{\abs{f'(x)}}{2}\int_{x - \delta}^{x + \delta}\frac{1}{\abs{x - y}}\;dy  = \frac{\abs{f'(x)}}{2\abs{x - y}}= \frac{\abs{f'(x)}}{2}\int_{0}^{\delta}\frac{1}{t}\;dt = \infty, 
		\end{equation}
	which contradicts the hypothesis. Therefore, we must have $f'(x) = 0$ for all $x \in \mathbb{R}$, which means $f$ is constant. 
\end{solutions}
...
\begin{prb}{2013-A-I-3 (Complex Analysis)}
	Suppose $f: D(0,1) \to D(0,1)$ is a holomorphic map from the open unit disc into the closed unit disk. Can the image of $f$ contain both the points $0$ and $1$? 
\end{prb}
\begin{solutions}
	The image of $f$ cannot contain both the points $0$ and $1$. If $f$ is a constant holomorphic function, then the result is trivial. So assume $f$ is a nonconstant holomorphic function. Then by the Open Mapping Theorem, $f$ must map the open unit disc $D(0,1)$ into an open subset $U$ of $\mathbb{C}$. Since $f(D(0,1)) \subset \overline{D(0,1)}$, $U \subset \overline{D(0,1)}$. Since $1$ is contained on the boundary of $\overline{D(0,1)}$, $1 \notin U$. Hence, the image of $f$ cannot contain both the points 0 and 1. 
\end{solutions}
...
\begin{prb}{2014-A-II-5 (Real Analysis)}
	Suppose $f$ is a continuous function on $[0,1]$ and $\int_{0}^{1}f(t)t^{n}\;dt = 0$ for every $n = 0, 1, 2, \ldots$. Prove that $f$ is the zero function. 
\end{prb}
\begin{solutions}
	Let $f$ be a continuous function on $[0,1]$, and assume that $\int_{0}^{1}f(t)t^{n}\;dt = 0$ for all nonnegative integers $n$. We will use the Stone-Weierstra{\ss} Theorem in our argument. Let $g(x)$ be an arbitrary continuous function on $[0,1]$. Since $[0,1]$ is compact, and $\mathbb{R}$ is Hausdorff (which implies that $[0,1]$ is Hausdorff), by the Stone-Weierstra{\ss} Theorem, there exists a sequence of polynomials $\{p_{j}\}_{1}^{\infty}$ that converges to $g$ uniformly. Writing 
		\begin{equation}
			p_{j}(t) = \sum_{k = 1}^{\deg{p_{j}}}a_{k}t^{k}, 
		\end{equation}
	we observe the following 
		\begin{align}
			\begin{split}
				\int_{0}^{1}f(t)g(t)\;dt &= \int_{0}^{1}\lim_{j \to \infty}f(t)p_{j}(t)\;dt \\
				&= \lim_{j \to \infty}\int_{0}^{1}f(t)\sum_{k = 1}^{\deg{p_{j}}}a_{k}t^{k}\;dt \\
				&= \lim_{j \to \infty}\left[\sum_{k = 1}^{\deg{p_{j}}}a_{k}\int_{0}^{1}f(t)t^{k}\;dt\right] \\
				&= \lim_{j \to \infty}(0) = 0, 
			\end{split}
		\end{align}
	where (1) the second equality follows from uniform convergence of the $p_{j}$ to $f$, and (2) the final line follows from the hypothesis. Hence, since $g$ was arbitrary, taking $g = f$ for example, forces $f$ to be identically zero. Hence, the proof concludes. 
\end{solutions}
\begin{prb}{2016-J-II-3 (Real Analysis)}
	Suppose $f$ is a continuous function on $[0,1]$ and $\int_{0}^{1}f(x)x^{k}dx = 0$ for $k =0, \ldots, n$. Prove that either $f$ is identically zero of $f$ must change signs at least $n + 1$ times. (We say $f$ changes sign $n + 1$ times if there are points $0 < x_{1} < \dotsm < x_{n + 1} < 1$ so that $f(x_{j})f(x_{j + 1}) < 0$ for $j = 1, \ldots, n$.) 
\end{prb}
\begin{solutions}
	Suppose $f$ is a continuous function on $[0,1]$ so that $\int_{0}^{1}f(x)x^{k}dx = 0$ for $k =0, \ldots, n$. Suppose $f$ changes sign $m \leq n$ times. By definition of ``change signs'', there exist $m$ points $0 \leq x_{1} < x_{2} < \dotsm < x_{m} \leq 1$ such that $f(x_{j}) = 0$ for $j = 1, \ldots, m$. Then the function $g(x) = \pm f(x)(x - x_{1})\dotsm (x - x_{m})$ is a continuous nonnegative function on $[0,1]$ for some choice of $\pm$, chosen so that the sign of the polynomial $(x - x_{1})\dotsm(x - x_{m})$ is the same as the sign of $f(x)$ on each interval $(0,x_{1}), \ldots, (x_{m - 1}, x_{m})$. Since $(x - x_{1}) \dotsm (x - x_{m})$ is a polynomial of degree $m \leq n$, expanding out the polynomial, and interchanging the sum and the integral, the hypothesis gives us that 
		\begin{equation}
			\int_{0}^{1}g(x)dx = 0. 
		\end{equation}
	Since $g(x) \geq 0$, the above result forces $g$ to be identically zero. Since $(x - x_{1})\dotsm (x - x_{m})$ is not identically zero, we conclude that $f \equiv 0$. Hence, the proof concludes. 
\end{solutions}
\begin{prb}{2016-J-II-6 (Real Analysis)}
	Let $\mathscr{M}$ be a closed linear subspace of $L^{2}([0,1]; \mathbb{R})$ that is contained in $C([0,1]; \mathbb{R})$. \vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item Prove that there exists $A > 0$ such that 
			\begin{equation*}
				\norm{f}_{u} \leq A\norm{f}_{2}
			\end{equation*}
		for all $f \in \mathscr{M}$. Here $\norm{\empspace}_{u}$ is the uniform norm. 
		\item Prove that $\dim{\mathscr{M}} \leq A^{2}$. (\textit{Hint}: Show that if $\{f_{j}\}$ is an $L^{2}$ orthonormal basis of $\mathscr{M}$, then $\sum \abs{f_{j}}^{2} \leq A^{2}$ for all $x \in [0,1]$.)
	\end{enumerate}
\end{prb}
\begin{solutions}
	\vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item For this part of the problem, we required the Closed Graph Theorem, which states the following: 
			\begin{quote}
				(\textbf{Closed Graph Theorem}) If $\mathscr{X}$ and $\mathscr{Y}$ are Banach spaces and $T: \mathscr{X} \to \mathscr{Y}$ is a closed linear map, then $T$ is bounded, where $T$ is a closed linear map iff the following is true: if $x_{n} \to x$ and $Tx_{n} \to y$, then $y = Tx$. 
			\end{quote}
		Let $\iota: \mathscr{M} \hookrightarrow C([0,1];\mathbb{R})$ be the (linear) inclusion map. Since closed linear subspaces of Banach spaces is a Banach space, $\mathscr{M}$ is a Banach space, itself. Therefore, it suffices to show that $\iota$ is closed. Suppose $f_{n} \to f$ in $\mathscr{M}$ (hence, $f_{n} \to f$ in $L^{2}([0,1];\mathbb{R})$) and $f_{n} \to g$ in $C([0,1];\mathbb{R})$ (which means that $f_{n} \to g$ in the supremum norm). By definition of the supremum norm, we observe that 
			\begin{equation}
				0 \leq \norm{f_{n} - g}_{2}^{2} = \int_{0}^{1} \abs{f_{n} - g}^{2}dx \leq \int_{0}^{1}\sup_{x \in [0,1]}\abs{f_{n} - g}^{2}dx = \norm{f_{n} - g}_{u}^{2} \longrightarrow 0 \text{ as $n \to \infty$}. 
			\end{equation}
		Hence, $f_{n} \to g$ in $L^{2}$. But since $f_{n} \to f$ in $L^{2}$ and limits in $L^{2}$ are unique, we conclude that $f = g$ a.e. Therefore, $\iota$ is closed. By the Closed Graph Theorem, we conclude that $\iota$ is bounded. 
		\item \textcolor{red}{[!! Complete Later !!]}
	\end{enumerate}
\end{solutions} 

\begin{prb}{2016-A-II-2 (Algebra)}
	Find the Galois group of the polynomial $p(x) = x^{3} - 2$ over the field $\mathbb{Z}_{11} \coloneqq \mathbb{Z}/11\mathbb{Z}$. 
\end{prb}
\begin{solutions}
	We begin by looking for roots of $p(x)$ over $\mathbb{Z}_{11}$. We find that $p(x)$ has one root, $x = 7$. Therefore, using polynomial long division in $\mathbb{Z}_{11}$, we find that 
		\begin{equation}
			p(x) = (x + 4)(x^{2} + 7x + 5). 
		\end{equation}
	Then it is straightforward to verify that the quadratic $x^{2} + 7x + 5$ is irreducible over $\mathbb{Z}_{11}$ since it has no roots contained in this field. Finding the roots of this quadratic and reducing it modulo 11, 
		\begin{equation}
			x = -\frac{7}{2} \pm \frac{\sqrt{29}}{2} \equiv 2 \pm 6\sqrt{7} \pmod{11}. 
		\end{equation}
	Since $\sqrt{7} \notin \mathbb{Z}_{11}$, the splitting field of $p(x)$ over $\mathbb{Z}_{11}$ must contain the field $\mathbb{Z}_{11}(\sqrt{7})$. On the other hand, both of the aforementioned roots must lie in $\mathbb{Z}_{11}(\sqrt{7})$. Therefore, the splitting field of $p(x)$ is $\mathbb{Z}_{11}(\sqrt{7})$. Since the minimal polynomial of $\sqrt{7}$ over $\mathbb{Z}_{11}$ is $x^{2} - 7$ (which is seen easily by verifying that this polynomial is irreducible over this field), it follows that $[\mathbb{Z}(\sqrt{7}): \mathbb{Z}] = 2$. Hence, the Galois group of $p(x)$ over the field $\mathbb{Z}_{11}$ must be of order 2; since there is only one group of order 2, up to isomorphism, the Galois group must be $\mathbb{Z}_{2} \coloneqq \mathbb{Z}/2\mathbb{Z}$. 
\end{solutions}

\begin{prb}{2016-J-I-2 (Geometry/Topology)}
	Let $X = S^{1} \times S^{1}$ be the $2$-torus, and let $Y = X \setminus \{p\}$ be the complement of one point in the 2-torus. \vspace{-0.35cm}
		\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
...
\begin{solutions}
	\vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item Assume to the contrary that $f: X \to Y$ is a covering map. Since $X$ is compact, $f(X) = Y$ must also be compact. But this is a contradiction since $Y$ is not compact. Therefore, $f$ cannot be surjective and hence not a covering map. 
		\item Suppose there exists a covering map $f: Y \to X$. We recall that $\pi_{1}(Y) \cong \mathbb{Z} \ast \mathbb{Z}$, while $\pi_{1}(X) \cong \mathbb{Z} \oplus \mathbb{Z}$. The former is a nonabelian group, which implies that its image under the induced map $f_{\ast}$ must be nonabelian in $\pi_{1}(X)$, which is a contradiction since $\pi_{1}(X)$ is abelian. Hence, by contradiction, $f$ cannot be a covering map. 
	\end{enumerate}
\end{solutions}

\begin{prb}{2016-A-I-1 (Algebra)}
...
\end{prb}
\begin{solutions}
	Let $p(x) = x^{5} + 1$. We first note that $p(x)$ has the root $-1 \equiv 18 \pmod{19}$. If $\zeta$ is a fifth root of unity in some field extension of $\mathbb{F}_{19}$, then it follows that $-\zeta_{5}^{n}$ is a root for all $n$. Since $5 \mid 19^{2} - 1$ but $5 \nmid 19$, it follows that $\zeta_{5} \in \mathbb{F}_{19^{2}}$. Hence, the splitting field of $p(x)$ is $K = \mathbb{F}_{19^{2}}$. This means that the Galois group $\operatorname{Aut}(K)$ has order $[\mathbb{F}_{19^{2}}: \mathbb{F}_{19}] = 2$. Since there is only one group of order 2 up to isomorphism, we conclude that $\operatorname{Aut}(K) \cong \mathbb{Z}_{2}$. Hence, there is only one nontrivial automorphism, which maps each root to its inverse. 
\end{solutions}

\begin{prb}{2016-A-I-1 (Algebra I)}	For $q(x) = x^{4} - 2$ over the field $\mathbb{F}_{11}$, find the splitting field $K$, the Galois group $\operatorname{Aut}(K)$, and find the action of $\operatorname{Aut}(K)$ on the 4 roots (up to labeling of the roots). 
\end{prb}
\begin{solutions}
	Let $q(x) = x^{4} - 2$ over the field $\mathbb{F}_{11}$. It is straightforward to see that $q(x)$ has no roots in this field. However, $q(x)$ is reducible since 
		\begin{equation}
			q(x) = (x^{2} + 4x + 8)(x^{2} + 7x +  8). 
		\end{equation}
	Is is then straightforward to see that each of these quadratics are irreducible over $\mathbb{F}_{11}$. Let $\alpha$ be a root of $x^{2} + 4x + 8$ so that $\mathbb{F}_{11}(\alpha) \cong \mathbb{F}_{11^{2}}$ since the minimal polynomial of $\alpha$ has degree 2. Since for each $n \geq 1$, there exists a unique extension of degree $n$, up to isomorphism, which is $\mathbb{F}_{11^{n}}$, we conclude that the other irreducible quadratic must also split over $\mathbb{F}_{11^{2}}$. Hence, the splitting field of $q(x)$ is $\mathbb{F}_{11^{2}}$, which is a degree 2 extension. This means that the order of the Galois group of $g(x)$ is $2$. Hence, $K \cong \mathbb{Z}_{2}$. This implies that there exists only one non-trivial automorphism, which is the Frobenius automorphism $\phi(x) = x^{11}$ maps a root of each irreducible quadratic to its conjugate root. 
\end{solutions} 
\begin{prb}{2013-A-I-2 (Algebra)}
	Find the Galois group of $x^{3} - 2$ over the field $\mathbb{Z}_{5}$. 
\end{prb}
\begin{solutions}
	Let $p(x)= x^{3} - 2$. We observe the following:
		\begin{equation}
			\begin{matrix}
				p(0) = -2 \equiv 3 \pmod{5} & p(1) = -1 \equiv 4 \pmod{5} & p(2) = 6 \equiv 1 \pmod{5} \\
				p(3) = 25 \equiv 0 \pmod{5} & p(4) = 62 \equiv 2 \pmod{5}.
			\end{matrix}
		\end{equation}
	Hence, $x = 3$ is a root of $p(x)$. In $\mathbb{Z}_{5}$, we can write 
		\begin{equation}
			p(x) = (x + 2)(x^{2} + 3x + 4). 
		\end{equation}
	It is straightforward to check that $x^{2} + 3x + 4$ is irreducible over $\mathbb{Z}_{5}$ as it has no roots in this field. The roots of the irreducible quadratic are, 
		\begin{equation}
			x = \frac{-3 \pm \sqrt{9 - 16}}{2} = \frac{-3}{2} \pm \frac{\sqrt{-7}}{2} \equiv 1 \pm 3\sqrt{3} \pmod{5}. 
		\end{equation}
	Since $3$ is not a square in $\mathbb{Z}_{5}$, the splitting field of $p(x)$ must contain the field $\mathbb{Z}_{5}(\sqrt{3})$. On the other hand, the field $\mathbb{Z}_{5}(\sqrt{3})$ contains both of the aforementioned roots of the irreducible quadratic. Therefore, the splitting field of $p(x)$ is $\mathbb{Z}_{5}(\sqrt{3})$. Since $3$ is not a square in $\mathbb{Z}_{5}$, the polynomial $x^{2} - 3$ is the minimal polynomial of $\sqrt{3}$. Since this polynomial has degree 2, $[\mathbb{Z}_{5}(\sqrt{3}): \mathbb{Z}_{5}] = 2$. Therefore, the Galois group of $p(x)$ must be of order 2. Thus, we conclude that $\operatorname{Gal}(\mathbb{Z}_{5}(\sqrt{3})/\mathbb{Z}_{5}) \cong \mathbb{Z}_{2}$. 
\end{solutions}

\begin{prb}{2014-A-II-1 (Algebra)}
	Determine the Galois group of the polynomial of the polynomial group $f(x) = x^{4} + 4$ over $\mathbb{Q}$. 
\end{prb}
\begin{solutions}
	Let $f(x) = x^{4} + 4$. The roots of $f(x)$ are $\sqrt[4]{-4}, \zeta_{4}\sqrt[4]{-4}$, $\zeta_{4}^{2}\sqrt[4]{-4}$, and $\zeta_{4}^{3}\sqrt[4]{-4}$, where $\zeta_{4}$ are the primitive fourth roots of unity. Observe that $\zeta_{4} = e^{2\pi i/4} = i$. Moreover, 
		\begin{equation}
			-4 = 4e^{i\pi} \implies \sqrt[4]{-4} = \sqrt{2}e^{i\pi/4} = \sqrt{2} \cdot \left(\frac{1 + i}{\sqrt{2}}\right) = 1 + i. 
		\end{equation}
	This means that the splitting field of $f(x)$ must contain $\mathbb{Q}(i)$. On the other hand, $\mathbb{Q}(i)$ contains all of the roots of $f(x)$. Hence, the splitting field of $f(x)$ over $\mathbb{Q}$ is $\mathbb{Q}(i)$. Since the minimal polynomial of $i$ over $\mathbb{Q}$ is $x^{2} + 1$, which is a degree 2 polynomial, $\operatorname{Gal}(\mathbb{Q}(i)/\mathbb{Q}) \cong \mathbb{Z}_{2}$. 
\end{solutions}
\begin{prb}{2011-A-II-1 (Algebra)}
	Determine the Galois group of $x^{4} - 25$ over $\mathbb{Q}$. 
\end{prb}
\begin{solutions}
	Let $f(x) = x^{4} - 25$. The roots of $f(x)$ are $\zeta_{4}^{j}\sqrt{5}$ for $j = 0, \ldots, 3$, where $\zeta_{4} = e^{2\pi i/4} = i$ is the primitive fourth root of unity. Hence, the splitting field must contain the field $\mathbb{Q}(i,\sqrt{5})$. On the other hand, the field $\mathbb{Q}(i, \sqrt{5})$ contains all of the aforementioned roots. Hence, the splitting field of $f(x)$ over $\mathbb{Q}$ is $\mathbb{Q}(i,\sqrt{5})$. Since the minimal polynomial of $i$ over $\mathbb{Q}$ is $x^{2} + 1$, $[\mathbb{Q}(i): \mathbb{Q}] = 2$. Likewise, since the minimal polynomial of $\sqrt{5}$ over $\mathbb{Q}(i)$ is $x^{2} - 5$, $[\mathbb{Q}(i,\sqrt{5}): \mathbb{Q}(i)] = 2$. Therefore, by the tower law, $\operatorname{Gal}(\mathbb{Q}(i,\sqrt{5})/\mathbb{Q})$ is a group of order 4. Since the Galois group is the group of automorphisms of the field $\mathbb{Q}(i, \sqrt{5})$ that fixes $\mathbb{Q}$, we have the following automorphisms: 
		\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
			\item $e$: $i \mapsto i$, $\sqrt{5} \mapsto \sqrt{5}$: This is the identity permutation. 
			\item $\sigma$: $i \mapsto i$, $\sqrt{5} \mapsto -\sqrt{5}$: This is a permutation of order $2$. 
			\item $\tau$: $i \mapsto -i$, $\sqrt{5} \mapsto \sqrt{5}$: This is also a permutation of order $2$. 
			\item $\rho$: $i \mapsto -i$, $\sqrt{5} \mapsto -\sqrt{5}$: This is equivalent to $\sigma\tau = \tau\sigma$. 
		\end{enumerate}
	Hence, it is straightforward to see that this group is isomorphic to the Klein-4 subgroup $V$ of $S_{4}$. I.e., $\operatorname{Gal}(\mathbb{Q}(i,\sqrt{5})/\mathbb{Q}) \cong V$. 
\end{solutions} 
\begin{prb}{2005-J-I-4 (Algebra)}
	Find the Galois group of the polynomial $p(x) = x^{3} - 3x + 1$ over $\mathbb{Q}$. 
\end{prb}
\begin{solutions}
	By the Rational Root Test, $p(x)$ has no roots in $\mathbb{Q}$, and hence must be irreducible. Therefore, we shall examine its determinant: 
		\begin{equation}
			D = -4p^{3} - 27q^{2} = -4(-3)^{3} - 27(1)^{2} = -27(-4 + 1) = 81. 
		\end{equation}
	Since $81$ is a square in $\mathbb{Q}$ (namely $\sqrt{81} = 9$), we conclude that the Galois group of the polynomial over $\mathbb{Q}$ is isomorphic to $A_{3}$. 
\end{solutions}
\begin{prb}{2003-J-I-5 (Algebra)}
	Let $f(x) = x^{5} - 2$. Find generators and relations for the Galois group $G \coloneqq \operatorname{Gal}(F/\mathbb{Q})$ of the splitting field $F$ of $f(x)$ over the rational numbers $\mathbb{Q}$. 
\end{prb}
\begin{solutions}
	Let $f(x) = x^{5} - 2$, which has the roots $\zeta_{5}^{j}\sqrt[5]{2}$ for $j = 0, \ldots, 4$, where $\zeta_{5}$ is the primitive fifth root of unity. The splitting field of $f(x)$ over $\mathbb{Q}$ must contain the field $\mathbb{Q}(\sqrt[5]{2}, \zeta_{5})$. On the other hand, the field $\mathbb{Q}(\sqrt[5]{2}, \zeta_{5})$ contains all of the roots of $f(x)$. Hence, the splitting field of the polynoomial over $\mathbb{Q}$ is exactly $\mathbb{Q}(\sqrt[5]{2}, \zeta_{5})$. Now, since $\zeta_{5}^{j}$ are the roots to $x^{5} - 1$ over $\mathbb{Q}$, but $x^{5} - 1$ splits into the product $(x -1)(x^{4} + \dotsm + 1)$, the minimal polynomial of $\zeta_{5}$ is a degree 4 polynomial, which means that $[\mathbb{Q}(\zeta_{5}): \mathbb{Q}] = 4$. On the other hand, since $x^{5} - 2$ is irreducible over $\mathbb{Q}(\zeta_{5})$, $[\mathbb{Q}(\sqrt[5]{2},\zeta_{5}): \mathbb{Q}(\zeta_{5})] = 5$. Hence, from the Tower Law, 
		\begin{equation}
			[\mathbb{Q}(\sqrt[5]{2},\zeta_{5}):\mathbb{Q}] = 20 \implies \abs{\operatorname{Gal}(\mathbb{Q}(\sqrt[5]{2},\zeta_{5}))} = 20. 
		\end{equation}
	By classification of groups of order 20, $G$ must be of the form $\mathbb{Z}_{5} \rtimes_{\varphi} (\mathbb{Z}_{2} \times \mathbb{Z}_{2})$ or $\mathbb{Z}_{5} \rtimes_{\varphi} \mathbb{Z}_{4}$ for some $\varphi \in \operatorname{Aut}(\mathbb{Z}_{5}) \cong \mathbb{Z}_{4}$. Since the automorphism group of $\mathbb{Q}(\zeta_{5}))$, which sends $\zeta_{5} \to \zeta_{5}^{j}$ for some $j$ coprime to 5, is cyclic of order 4, we deduce that $G \cong \mathbb{Z}_{5} \rtimes_{\varphi} \mathbb{Z}_{4}$ for some $\varphi \in \operatorname{Aut}(\mathbb{Z}_{5})$. It is straightforward to see that the permutations $\sigma: \sqrt[5]{2} \to \zeta_{5}\sqrt[5]{2}$ (of order 5) and $\tau: \zeta_{5} \to \zeta_{5}^{2}$ are generators of $G$. Moreover, 
		\begin{equation}
			\tau\sigma\tau^{-1}(\zeta_{5}) = \tau\sigma(\zeta_{5}^{4}) = \tau(\zeta_{5}^{4}) = \zeta_{5}, \qquad \text{and} \qquad \tau\sigma\tau^{-1}(\sqrt[5]{2}) = \tau\sigma(\sqrt[5](2)) = \zeta_{5}^{2}\sqrt[5]{2} = \sigma^{2}(\sqrt[5]{2}). 
		\end{equation}
	Hence, we obtain the relation $\tau\sigma\tau^{-1} = \sigma^{2}$. Therefore, 
		\begin{equation}
			G = \braket{\sigma,\tau: \sigma^{5} = \tau^{4} = 1, \tau\sigma\tau^{-1} = \sigma^{2}}. 
		\end{equation}
\end{solutions}
\begin{prb}{2010-A-II-2 (Algebra)}
...
\end{prb}
\begin{solutions}
	Let $c = 1$ so that $f(x) = x^{3} + x -1$. Since $f(1) = 1 + 1 - 1 = 1$ and $f(-1) = -1 - 1 -1 = -3$, we conclude by the rational root test that $f(x)$ has no rational roots, and hence is irreducible over $\mathbb{Q}$. Moreover, since the discriminant of $f(x)$ is, 
		\begin{eqnarray}
			\Delta = -4p^{3} -27q^{2} = -4(1)^{3} - 27(-1)^{2} = -4 - 27 = -31, 
		\end{eqnarray}
	which is \textit{not} a square in $\mathbb{Q}$, the Galois group of $f(x)$ over $\mathbb{Q}$ must be isomorphic to $S_{3}$, which is not abelian. 
\end{solutions} 
\begin{prb}{2004-A-I-4 (Algebra)}
	Give an example of a Galois extension $E$ of the rational numbers $\mathbb{Q}$ such that $\operatorname{Gal}(E/\mathbb{Q})$ is $\mathbb{Z}_{7}$ (the cyclic group of order 7). 
\end{prb}
\begin{solutions}
	Consider the field extension $\mathbb{Q}(\zeta_{29})$, where $\zeta_{29}$ is the primitive 29th root of unity. Since 29 is prime, we observe that 
		\begin{equation}
			\operatorname{Gal}(\mathbb{Q}(\zeta_{29})/\mathbb{Q}) \cong \left(\mathbb{Z}/29\mathbb{Z}\right)^{\ast}, 
		\end{equation}
	which is abelian and cyclic of order 28. Since $7 \mid 28$ and $4 \mid 28$, $\operatorname{Gal}(\mathbb{Q}(\zeta_{29})/\mathbb{Q})$ has exactly one subgroup of order 7 and one subgroup of order 4. I.e., there exists a unique subfield $E \subset \mathbb{Q}(\zeta_{29})$ such that $[E: \mathbb{Q}] = 7$. Hence, we conclude that $\operatorname{Gal}(E/\mathbb{Q})$ is a group of order 7, and hence is isomorphic to $\mathbb{Z}_{7}$. 
\end{solutions} 
\begin{prb}{2016-A-II-1 (Geometry/Topology)}
	Let $X$ be the topological space obtained by making a 2-sphere and a 2-torus touch at a single point. Give a concrete description of the universal cover $\ti{X}$ of $X$, acommpanied by a drawing of $\ti{X}$. 
\end{prb}
\begin{solutions}
	Let $X = S^{2} \vee T^{2}$, obtained by identifying some $p \in S^{2}$ with $q \in T^{2}$. We recall that the universal cover of $S^{2}$ is $S^{2}$ (which is a 1-fold covering of itself), and the universal cover of $T^{2}$ is $\mathbb{R}^{2}$. The universal cover $\ti{X}$ of $X$ is $\mathbb{R}^{2}$, whose integer lattice points are the lifts $\ti{q}$ of $q$, with a copy of $S^{2}$ attached at each such point by identifying its basepoint (the lift of $p$) with that lattice point. Visually: 
		\begin{center}
			\includegraphics[width=0.7\linewidth]{"../Graphics/2016AII1"}
		\end{center}
\end{solutions}
\begin{prb}{2011-J-I-3 (Geometry/Topology)}
...
\end{prb}
\begin{solutions}
	We begin with a modified version of the comps lemma. 
		\begin{quote}
			(\textbf{Comps Lemma}) Let $M$ and $N$ be smooth connected $n$-manifolds, and $f: M \to N$ a local homeomorphism. If $M$ is compact and nonempty, then $N$ is compact and $f$ is a covering map. 
			
			\begin{proof}
				Let $M$ and $N$ be smooth connected $n$ manifolds, $f: M \to N$ a local homeomorphism, and assume $M$ is compact and nonempty. Since local homeomorphisms are open maps, $f(M)$ is open in $N$. On the other hand, since the continuous image of a compact set is compact, and a compact subset of a Hausdorff space (like $N$) is closed, $f(M)$ is closed. Since $f(M)$ is nonempty and $N$ is connected, we conclude that $f(M) = N$. Hence, $N$ is compact and $f$ is surjective. 
				
				Now we show that $f$ is a covering map. Let $y \in N$. Then for each $x \in f^{-1}(y)$, there exists a neighborhood $U_{x}$ such that $f|_{U_{x}}$ is a homeomorphism. Since $M$ is Hausdorff, we may shrink these neighborhoods so that each $x \in M$ is isolated. This means that $f^{-1}(y)$ is discrete. Since discrete subspaces of compact spaces are finite, $f^{-1}(y)$ is finite; let $f^{-1}(y) = \{x_{1}, \ldots, x_{s}\}$. As identified earlier, for each $j = 1, \ldots, s$, there exists a neighborhood $U_{j}$ of $x_{j}$ such that $f|U_{j}$ is a homeomorphism. Since $M$ is Hausdorff, we may shrink these neighborhoods so that they are pairwise disjoint. Now set $V = \bigcap_{1}^{s}f(U_{j})$, and let $\ti{U}_{j} = f^{-1}(V) \cap U_{j}$ for each $j$ so that $V$ is an evenly covered neighborhood of $y$. This shows that $f$ is a covering map. 
			\end{proof} 
		\end{quote} 
	
	Now assume to the contrary that $f: \Sigma \to T$ is a local homeomorphism. By the Comps Lemma, it follows that $f$ is a covering map. Hence, the induced group homorphism $f_{\ast}: \pi_{1}(\Sigma) \to \pi_{1}(S^{1}\times S^{1})$ is injective. We recall that $\pi_{1}(\Sigma) \cong \mathbb{Z} \ast \mathbb{Z}$ is non-abelian, while $\pi_{1}(S^{1} \times S^{1}) \cong \pi_{1}(S^{1}) \times \pi_{1}(S^{1}) \cong \mathbb{Z} \times \mathbb{Z}$. Since $f_{\ast}$ is injective, $f_{\ast}(\pi_{1}(\Sigma))$ is a nonabelian subgroup of $\mathbb{Z} \times \mathbb{Z}$, which is a contradiction. Hence, by contradiction, $f$ cannot be a local homeomorphism. 
\end{solutions}
\begin{prb}{2004-A-I-5 (Geometry/Topology)}
	In this problem, the term ``surface'' means a compact orientable 2-manifold without boundary. \vspace{-0.35cm}
		\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
...
\begin{solutions}
	\vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item Let $\Sigma_{1}$ denote a surface of genus 1 and $\Sigma_{2}$ denote a surface of genus 2; let $p: X \to \Sigma_{1}$ be a covering space. Assume to the contrary that $X$ is homeomorphic to $\Sigma_{2}$; this means that $\pi_{1}(X) \cong \pi_{1}(\Sigma_{2})$. Since $\pi_{1}(\Sigma_{2})$ is nonabelian, the fundamental group of $X$ must also be nonabelian. On the other hand, since $p: X \to \Sigma_{1}$ is a covering space, the induced group homomorphism $p_{\ast}: \pi_{1}(X) \to \pi_{1}(\Sigma_{1})$ is injective. Since $\pi_{1}(X)$ is non-abelian, $p_{\ast}(\pi_{1}(X))$ must be a nonabelian subgroup of $\pi_{1}(\Sigma_{1})$. But this is impossible since $\pi_{1}(\Sigma_{1}) \cong \mathbb{Z} \times \mathbb{Z}$ is abelian. Hence, $X$ cannot be homeomorphic to $\Sigma_{2}$. 
		\item The plot line of this solution is that every surface group is residually finite, and surface groups $\pi_{1}(\Sigma_{g})$ for $g \geq 2$ contains subgroups of every finite index. Now suppose $X$ is a covering space of $\Sigma_{2}$ that is homeomorphic to $\Sigma_{3}$. If the covering is $k$-sheeted, then 
			\begin{equation}
				\chi(X) = k \cdot \chi(\Sigma_{2}), 
			\end{equation}
		where $\chi(\cdot)$ denotes the Euler characteristic. Since $\chi(\Sigma_{2}) = 2 - 2(2) = -2$, and $\chi(X) = \chi(\Sigma_{3}) = 2 - 2(3) = -4$, it follows that $k = 2$. Hence, $X$ must be a connected 2-sheet covering of $\Sigma_{2}$ \textit{if} it exists. This is where the plot line becomes useful. Since $\pi_{1}(\Sigma_{2})$ contains a subgroup of index 2, and by classification of covering spaces, corresponds to a connected 2-sheeted covering. Hence, $\Sigma_{2}$ does have a covering space which is homeomorphic to a surface of genus 3. 
	\end{enumerate}
\end{solutions} 
\begin{prb}{2005-MAT530-II-4 (Geometry/Topology)}
	Determine the number of coverings of the space $\mathbb{RP}^{2} \times \mathbb{RP}^{2}$ (up to equivalence, with path-connected covering space.)
\end{prb}
\begin{solutions}
	Let $X= \mathbb{RP}^{2} \times \mathbb{RP}^{2}$ so that $\pi_{1}(\mathbb{RP}^{2} \times \mathbb{RP}^{2}) \cong \pi_{1}(\mathbb{RP}^{2}) \times \pi_{1}(\mathbb{RP}^{2}) \cong \mathbb{Z}_{2} \times \mathbb{Z}_{2}$. Since $\mathbb{RP}^{2} \times \mathbb{RP}^{2}$ has a universal cover (namely $S^{2} \times S^{2}$), the equivalence classes of coverings of $\mathbb{RP}^{2} \times \mathbb{RP}^{2}$ correspond to the conjugacy classes of subgroups of $\mathbb{Z}_{2} \times \mathbb{Z}_{2}$. Since this group is abelian, the number of coverings of $\mathbb{RP}^{2} \times \mathbb{RP}^{2}$ corresponds to the number of subgroups of $\mathbb{Z}_{2} \times \mathbb{Z}_{2}$. Since there are five such subgroups, it follows that there are five coverings spaces of $\mathbb{RP}^{2} \times \mathbb{RP}^{2}$ up to equivalence. 
\end{solutions}
\begin{prb}{2005-MAT530-II-4 (Geometry/Topology I)}
	Determine the number of coverings of the space $\mathbb{RP}^{1} \times \mathbb{RP}^{1}$ (up to equivalence, with path-connected covering space.)
\end{prb}
\begin{solutions}
	Let $X = \mathbb{RP}^{1} \times \mathbb{RP}^{1}$ so that $\pi_{1}(\mathbb{RP}^{1} \times \mathbb{RP}^{1}) \cong \pi_{1}(\mathbb{RP}^{1}) \times \pi_{1}(\mathbb{RP}^{1}) \cong \mathbb{Z} \oplus \mathbb{Z}$. Since $\mathbb{RP}^{1} \times \mathbb{RP}^{1}$ has a universal cover (namely $\mathbb{R}^{2}$), the equivalence classes of coverings of $\mathbb{RP}^{1} \times \mathbb{RP}^{1}$ correspond to conjugacy classes of subgroups of $\mathbb{Z} \oplus \mathbb{Z}$. Since this group is abelian, the number of coverings of $\mathbb{RP}^{1} \times \mathbb{RP}^{1}$ correspond to the number of subgroups of $\mathbb{Z} \oplus \mathbb{Z}$. Since there are countably many such subgroups, we conclude that there are a countable infinitely many covering spaces of $\mathbb{RP}^{1} \times \mathbb{RP}^{1}$ up to equivalence. 
\end{solutions} 
\begin{prb}{2005-J-II-5 (Geometry/Topology)}
	Give an example of a topological space admitting exactly 3 coverings up to equivalence (including the space itself). 
\end{prb}
\begin{solutions}
	Consider the Lens space $L(4;1)$, which has fundamental group $\mathbb{Z}_{4}$. Since $L(4;1)$ has a universal cover (namely $S^{3}$), $\mathbb{Z}_{4}$ is abelian, and $\mathbb{Z}_{4}$ has exactly 3 subgroups (namely $\{0\}$, $\mathbb{Z}_{2}$, and $\mathbb{Z}_{4}$), we conclude that $L(4\;1)$ has exactly 3 coverings up to equivalence. 
\end{solutions} 
\begin{prb}{2002-A-II-1 (Geometry/Topology)}
	Let $X$ denote three copies of $S^{1}$ identified along a common point. \vspace{-0.35cm}
		\begin{enumerate}[itemsep =-2pt,label = (\roman{*})]
			\item Compute the fundamental group of $X$. 
			\item Show that any continuous map $f: \mathbb{RP}^{2} \to X$ from real projective space is nullhomotopic. 
		\end{enumerate}
\end{prb}
\begin{solutions}
	\vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\roman{*})]
		\item Let $X$ denote three copies of $S^{1}$ identified along a common point: 
			\begin{center}
				\begin{tikzpicture}
					\draw (-2,-2) circle (1); 
					\draw (0,1) circle (1); 
					...
					\draw[dashed] (2,-1) -- (0,0) -- (-2,-1) -- (2,-1);
				\end{tikzpicture}
			\end{center}
		Let $U$ consist of two entire circles, and a portion of the third circle, and let $V$ consist of the third circle and a portion of the second circle. Then $U \cap V$ is contractible so that $\pi_{1}(U \cap V) = 0$; $U$ deformation retracts onto $S^{1} \vee S^{1}$ so that $\pi_{1}(U \cap V) = \braket{\alpha,\beta: \varnothing} \cong S^{1} \vee S^{1}$; $V$ deformation retracts onto $S^{1}$ so that $\pi_{1}(V) = \braket{\gamma:\varnothing}$. Hence, if $i_{1}: U \cap V \hookrightarrow U$ and $i_{2}: U \cap V \hookrightarrow V$, then 
			\begin{equation}
				\pi_{1}(X) \cong \pi_{1}(U) \ast \pi_{1}(V)/N \cong \braket{\alpha,\beta,\gamma: \varnothing} \cong \mathop{\scalebox{2.5}{\ast}}_{j = 1}^{3}\mathbb{Z}. 
			\end{equation}
		\item Let $f: \mathbb{RP}^{2} \to X$ be continuous, and consider the induced group homomorphism $f_{\ast}: \pi_{1}(\mathbb{RP}^{2}) \to X$; the universal cover of $X$ is a tree, and hence contractible. Since $\pi_{1}(\mathbb{RP}^{2}) \cong \mathbb{Z}_{2}$ and $\pi_{1}(X) \cong \mathbb{Z} \ast \mathbb{Z} \ast \mathbb{Z}$, it follows that $f_{\ast}$ is the trivial homomorphism. I.e., $f_{\ast}\pi_{1}(\mathbb{RP}^{2}) = \{e\}\subset \pi_{1}(X)$. Hence, by the General Lifting Lemma, $f$ lifts to a continuous map $\ti{f}: \mathbb{RP}^{2} \to \ti{X}$. But since $\ti{X}$ is contractible, $\ti{f}$ is nulhomotopic. if $\ti{H}$ is a nulhomotopy and $q: \ti{X} \to X$ is a covering map, then $H = q \circ \ti{H}$ is a nulhomotopy from $f$. Hence, $f$ is nulhomotopic. 
	\end{enumerate}
\end{solutions}

\begin{prb}{2015-A-II-3 (Geometry/Topology)}
...
\end{prb}
\begin{solutions}
	We will begin with a technical lemma that we will use in our proof. 
		\begin{quote}
			(\textbf{Modified Comps Lemma}) Let $M, N$ be smooth, connected, $n$-manifolds, and $f: M\to N$ a local homeomorphism. If $M$ is compact and nonempty, then $N$ is compact and $f$ is a covering map. 
			
			\begin{proof}
				Let $M, N$ be smooth, connected, $n$-manifolds, $f: M \to N$ a local homeomorphism, and let $M$ be compact and nonempty. Since local homeomorphisms are open maps, $f(M)$ is open in $N$. Since the continuous image of a compact set is compact, and a compact subspace of a Hausdorff space is closed, $f(M)$ is closed in $N$. Since $f(M)$ is nonempty and $N$ is connected, $f(M) = N$. Hence, $N$ is compact and $f$ is surjective. 
				
				Now let $q \in N$. For each $x \in f^{-1}(q)$, there exists a neighborhood $U_{x}$ such that $f|_{U_{x}}$ is a homeomorphism. Since $M$ is Hausdorff, we may shrink these neighborhoods so that they are pairwise disjoint. This shows that $f^{-1}(q)$ is discrete. Since discrete subspaces of compact spaces are finite, $f^{-1}(q)$ is finite; let $f^{-1}(q) = \{x_{1}, \ldots x_{s}\}$. As mentioned above, for each $j = 1, \ldots, s$, we can obtain a neighborhood $\ti{U}_{j}$ on which $f$ is a local homeomorphism. Since $M$ is Hausdorff, we may shrink these neighborhoods (and re-denote them as $\ti{U}_{j}$ for each $j$) so that $f$ remains a homeomorphism on these shrunken, pairwise disjoint neighborhoods. Set $V = \bigcap_{j = 1}^{s}f(U_{j})$, and define $U_{j} = f^{-1}(V) \cap \ti{U}_{j}$. Then $V$ is an evenly covered neighborhood of $q$. Hence, we conclude that $f$ is a covering map. 
			\end{proof}
		\end{quote}
	Now, assume to the contrary that $f$ is a local homeomorphism; by the Modified Comps Lemma, $f$ is a covering map. This means $\chi(\Sigma_{2}) = k \cdot \chi(\Sigma_{1})$, where $k \in \mathbb{N}$ is the number of sheets of the covering space. Since $\Sigma_{2}$ is of genus 2, its Euler characteristic is $\chi(\Sigma_{2}) = 2 - 2(2) = 2- 4 = -2$. On the other hand, $\chi(\Sigma_{1}) = 2 - 2(1) = 0$. Hence, there exists no positive integer $k$ such that $-2 = k \cdot 0$, which contradicts our claim that $f$ is a covering map. Hence, by contradiction, $f$ cannot be a local homeomorphism. 
\end{solutions}
\begin{prb}{2009-J-I-1 (Geometry/Topology)}
...
\end{prb}
\begin{solutions}
	Let $M^{k}$ be a smooth compact $k$-manifold, and let $F: M \to S^{n}$ be a smooth map, where $n > k$. Since the differential $dF_{p}: T_{p}M \to T_{F(p)}S^{n}$ has rank at most $k < n$ at every $p \in M$, it follows that it is never surjective, and hence, every $q \in F(M)$ is a critical value of $F$. By Sard's Theorem, $F(M)$ has measure zero in $S^{n}$, which means there exists some $q \in S^{n}$ so that $q \in S^{n}\setminus F(M)$. In particular, $F(M) \subset S^{n}\setminus \{q\}$. Now, since $S^{n} \setminus \{q\}$ is diffeomorphic to the contractible space $\mathbb{R}^{n}$, we conclude that $S^{n}\setminus \{q\}$ is a contractible. Let $H: S^{n} \setminus \{q\} \times [0,1]$ be a homotopy between the identity map on $S^{n} \setminus \{q\}$ and a constant map $g: S^{n}\setminus \{q\} \to S^{n} \setminus \{q\}$ defined by $g(s) = q'$ for all $s \in S^{n}\setminus \{q\}$. Then it is straightforward to see that the map $H(F(p),t)$ is a homotopy between $F$ and a constant map on $S^{n}\setminus \{q\}$. Hence, $F$ is homotopic to a constant map. 
\end{solutions}
\begin{prb}{2012-A-I-2 (Geometry/Topology)}
	Prove that every local homeomorphism between compact, connected, topological spaces is a covering map of some finite degree. 
\end{prb}
\begin{solutions}
	Let $M$ and $N$ be compact, connected, topological spaces, and $f: M \to N$ a local homeomorphism. Let $q \in N$. For each $x \in f^{-1}(q)$, there exists a neighborhood $U_{x}$ such that the restriction of $f$ to $U_{x}$ is a homeomorphism. Since $M$ is (assumed) Hausdorff, we can shrink each of the neighborhoods so that they are pairwise disjoint. This means that every $x \in f^{-1}(q)$ is isolated, and hence $f^{-1}(q)$ is discrete. Since discrete subspaces of compact spaces are finite, we conclude that $f^{-1}(q)$ is finite; let $f^{-1}(q) = \{x_{1}, \ldots, x_{s}\}$. As stated above, for each $j = 1, \ldots, s$, there exists a neighborhood $\ti{U_{j}}$ so that $f|_{\ti{U}_{j}}$ is a homeomorphism. Since $M$ is Hausdorff, we may shrink these sets to obtain a pairwise disjoint collection of neighborhoods $\{\ti{\ti{U}}_{j}\}_{1}^{s}$ of each point in the preimage of $q$; the restriction of $f$ to each of these neighborhoods remains a homeomorphism. Setting $V$ to be the open set $\bigcap_{1}^{s}f(\ti{\ti{U}}_{j})$, define 
		\begin{equation}
			U_{j} = f^{-1}(V) \cap \ti{\ti{U}}_{j}. 
		\end{equation}
	Hence, $V$ is an evenly covered neighborhood of $q$. Since $q \in N$ was arbitrary, we conclude that $f$ is a covering map of finite degree $s$. 
\end{solutions}
\begin{prb}{2010-A-II-4 (Geometry/Topology)}
	Let $p$ be a prime. Let $M$ be the two-dimensional torus $S^{1} \times S^{1}$. How many distinct $p$-sheeted covering spaces $\varpi: \ti{M} \to M$ are there such that $\ti{M}$ is path connected? Count these up to isomorphism of covering spaces of $M$, i.e., $\varpi_{1}: \ti{M}_{1} \to M$ is isomorphic to $\varpi_{2}: \ti{M}_{2} \to M$ if there exists a homeomorphism $f: \ti{M}_{1} \to \ti{M}_{2}$ such that $\varpi_{2} \circ f = \varpi_{1}$. 
\end{prb}
\begin{solutions}
	Let $M$ be the two-dimensional torus $S^{1} \times S^{1}$, whose fundamental group is given by 
		\begin{equation}
			\pi_{1}(M) = \pi_{1}(S^{1} \times S^{1}) = \pi_{1}(S^{1}) \oplus \pi_{1}(S^{1}) \cong \mathbb{Z} \oplus \mathbb{Z}. 
		\end{equation}
	Since $M$ has a universal cover (namely, $\mathbb{R} \times \mathbb{R}$), the path-connected covering spaces of $M$ correspond to the conjugacy classes of subgroups of $\mathbb{Z} \oplus \mathbb{Z}$. Since this group is abelian, the path-connected covering spaces correspond to the subgroups of $\mathbb{Z} \oplus \mathbb{Z}$; in particular, $p$-sheeted covering spaces correspond to the subgroups of $\mathbb{Z} \oplus \mathbb{Z}$ of prime index $p$. By the First Isomorphism Theorem for groups, subgroups of $\mathbb{Z}^{2}$ of index $p$ correspond to the kernels of surjective homomorphisms from $\mathbb{Z}^{2}$ onto $\mathbb{Z}_{p}$, with two homeomorphisms having the same kernel iff they differ by multiplication by a unit in $\mathbb{Z}_{p}$. Hence, index-$p$ subgroups of $\mathbb{Z}^{2}$ are in one-to-one correspondence with the 1-dimensional subspaces of $(\mathbb{Z}/p\mathbb{Z})^{2} \cong \mathbb{F}_{p}^{2}$. Since there are a total of $p^{2} - 1$ nonzero elements in $\mathbb{F}_{p}^{2}$, and each 1-dimensonal subspace contains $p-1$ nonzero elements, we conclude that there are $p + 1$ such subspaces. Hence, there are $p + 1$ distinct $p$-sheeted path-connected covering spaces. 
\end{solutions}

\begin{prb}{2015-J-I-I (Complex Analysis)}
	Suppose $f$ and $g$ are two entire holomorphic functions with the property that $\abs{f(z)} \leq \abs{g(z)}$ for every $z \in \mathbb{C}$. Prove that there is a constant $\lambda \in \mathbb{C}$ such that $f = \lambda g$. 
\end{prb}
\begin{solutions}
	The key concepts for this problem are \textit{Liouville's Theorem} and the concept of \textit{removable singularities}. 
		\begin{quote}
			(\textbf{Liouville's Theorem}) Every holomorphic function $f: \mathbb{C} \to \mathbb{C}$ for which there exists a positive number $M$ such that $|f(z)| \leq M$ for all $z \in \mathbb{C}$ is constant. 
			
			(\textbf{Removable Singularities}) If $U \subset \mathbb{C}$ is an open subset, $a \in U$, and $f: U \setminus \{a\} \to \mathbb{C}$ is holomorphic, then $a$ is a \textit{removable singularity} for $f$ if there exists a holomorphic function $g: U \to \mathbb{C}$ which coincides with $f$ on $U\setminus \{a\}$. We say $f$ is \textit{holomorphically extendable} over $U$ if such $g$ exists. 
		\end{quote}
	Assume $f$ and $g$ are entire holomorphic functions with the property that $\abs{f(z)} \leq \abs{g(z)}$ for all $z \in \mathbb{C}$. If $f, g \equiv 0$, then we are done trivially. So assume $f, g \nequiv 0$. Consider the function $f(z)/g(z)$; by our hypothesis, 
		\begin{equation}
			\abs{\frac{f(z)}{g(z)}} \leq 1 \quad \text{ for all $z \in \mathbb{C}$}. 
		\end{equation}
	To invoke Liouville's Theorem, we must show that $f(z)/g(z)$ is holomorphic. Suppose $g(z_{0}) = 0$; without loss of generality, assume this is the only root of $g(z)$. Otherwise, we can apply the following argument repeatedly. Since $|f(z)| \leq |g(z)|$ for all $z$, it follows that $z_{0}$ must also be a root of $f$. Suppose $f(z) = (z - z_{0})^{n}\ti{f}(z)$ and $g(z) = (z - z_{0})^{m}\ti{g}(z)$ for some $n, m \in \mathbb{N}$. Suppose that $n < m$. Then since 
		\begin{equation}
			\lim_{z \to z_{0}}\frac{f(z)}{g(z)} = \lim_{z \to z_{0}}(z - z_{0})^{n - m} \frac{\ti{f}(z)}{\ti{g}(z)} \to \infty, 
		\end{equation}
	which contradicts our hypothesis that $\abs{f(z)} \leq \abs{g(z)}$ for all $z$. Therefore, $n \geq m$, which means $z_{0}$ must be a removable singularity of $f/g$. Hence, by Riemann's Theorem on Removable Singularities, there exists an holomorphic extension, $h$ of $f/g$ defined as follows: 
		\begin{equation}
			h(z) = 
				\begin{cases}
					\frac{f(z)}{g(z)}, & z \neq z_{0}, \\
					\lim_{z \to z_{0}}\frac{f(z)}{g(z)}, & z = z_{0}. 
				\end{cases}
		\end{equation}
	Since $|h(z)| \leq 1$ for all $z \in \mathbb{C}$, by Liouville's Theorem, we conclude that there exists $\lambda \in \mathbb{C}$ so that $h(z)= \lambda$. Since $h$ is the holomorphic extension of $f/g$, we conclude that
		\begin{equation}
			f(z) =\lambda g(z). 
		\end{equation}
\end{solutions}
\begin{prb}{2015-A-II-2 (Geometry/Topology)}
	Let $M$ be a connected oriented smooth $n$-dimensional manifold-with-boundary, and suppose that $\partial M \neq \varnothing$. Let $\Phi: M \to M$ be a smooth map whose restriction to $\partial M$ is the identity. If $\omega$ is any smooth $n$-form on $M$, prove that 
		\begin{equation*}
...
\end{prb}
\begin{solutions}
	Let $M$ be a connected, oriented smooth $n$-dimensional manifold-with-boundary such that $\partial M \neq \varnothing$. Let $\Phi: M \to M$ be a smooth map so that $\Phi|_{\partial M} = \operatorname{id}_{\partial M}$. This implies that $\Phi$ \textit{preserves} orientation. Let $\omega$ be an arbitrary smooth $n$-form on $M$. Since the top de Rham cohomology group \textcolor{red}{[!! Complete Later !!]}
\end{solutions}
\begin{prb}{2014-J-II-5 (Real Analysis)}
	Suppose that $f: \mathbb{R} \to \mathbb{R}$ is measurable with respect to the Lebesgue measure $\mu$ and $f(x) = f(x + 1) = f(x + \pi)$ for almost all $x$. Prove that $f$ is a constant almost everywhere. 
\end{prb}


\begin{prb}{Foll-6-I-9 (Real Analysis)}
	Suppose $1 \leq p < \infty$. If $\norm{f_{n} - f}_{p} \to 0$, then $f_{n} \to f$ in measure, and hence some subsequence converges to $f$ a.e. On the other hand, if $f_{n} \to f$ in measure and $\abs{f_{n}} \leq g \in L^{p}$ for all $n$, then $\norm{f_{n} - f}_{p} \to 0$. 
\end{prb}
\begin{solutions}
	\vspace{-0.35cm}
	\begin{enumerate}[itemsep =-2pt,label = (\roman{*})]
		\item Suppose $\norm{f_{n} - f}_{p} \to 0$, which means that 
			\begin{equation}
				\norm{f_{n} - f}_{p}^{p} = \int \abs{f_{n}(x) - f(x)}^{p} \longrightarrow 0. 
			\end{equation}
		For $\epsilon > 0$, define 
			\begin{equation}
				E_{n}(\epsilon) \coloneqq \brac*{x: \abs{f_{n}(x) - f(x)} \geq \epsilon}. 
			\end{equation}
		Then 
			\begin{equation}
				0 \longleftarrow \int \abs{f_{n}(x) - f(x)}^{p} \geq \int_{E_{n}(\epsilon)}\abs{f_{n}(x) - f(x)}^{p} \geq \int_{E_{n}(\epsilon)}\epsilon^{p} = \mu(E_{n}(\epsilon))\epsilon^{p} \geq 0. 
			\end{equation}
		Since $\epsilon > 0$ was arbitrary, it follows that $\mu(E_{n}(\epsilon)) \to 0$. Hence, $f_{n} \to f$ in measure. 
		\item Now assume $f_{n} \to f$ in measure and that $|f_{n}| \leq g \in L^{p}$ for all $n$. Then there exists a subsequence $\{g_{n}\} \subset \{f_{n}\}$ that converges to $f$ $\mu$-a.e; this implies that $\abs{f} \leq g$ $\mu$-a.e. Hence, since $|g_{n}(x) - f(x)|^{p} \leq 2^{p}g^{p}$ for all $n$, and $g^{p} \in L^{1}$, by the Dominated Convergence Theorem, we conclude that 
			\begin{equation}
				\int \abs{g_{n}(x) - f(x)}^{p} \longrightarrow 0. 
			\end{equation}
		I.e., the subsequence converges to $f$ in the $L^{p}$-norm. Now assume to the contrary that $f_{n} \mathop{\not\longrightarrow}_{L^{p}} f$, which means that for some $\epsilon > 0$, there exists a subsequence $\{h_{n}\} \subset \{f_{n}\}$ such that $\int |h_{n} - f|^{p} \geq \epsilon$ for all $n$. But since $h_{n}$ satisfies the same conditions as the original sequence $\{f_{n}\}$, there exists a subsubsequence $\{\ti{h}_{n}\} \subset \{h_{n}\} \subset \{f_{n}\}$ that converges $\mu$-a.e. to $f$ and hence converges in $L^{p}$ to $f$, contradicting the hypothesis on $\{h_{n}\}$. Hence, by contradiction, every subsequence of $\{f_{n}\}$ admits a subsubsequence that converges to $f$ under the $L^{p}$ norm, and therefore, $f_{n} \mathop{\longrightarrow}_{p} f$. 
	\end{enumerate}
\end{solutions}
\begin{prb}{Foll-6-I-10 (Real Analysis)}
	Suppose $1 \leq p < \infty$. If $f_{n}, f \in L^{p}$ and $f_{n} \to f$ a.e., then $\norm{f_{n} - f}_{p} \to 0$ iff $\norm{f_{n}}_{p} \to \norm{f}_{p}$. (Use Exercise 20 in \S2.3.)
\end{prb}
\begin{solutions} 
	Let $1 \leq p < \infty$, $f_{n}, f \in L^{p}$, and assume $f{n} \to f$ a.e. 
\end{solutions}
