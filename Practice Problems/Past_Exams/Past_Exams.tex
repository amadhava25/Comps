...
\end{prb}
\begin{solutions}
	This problem requires the Cauchy Integral Formula, which states the following: 
		\begin{quote}
			\textbf{(Cauchy's Integral Formula)} Let $U$ be an open subset of the complex plane $\mathbb{C}$, and suppose the closed disk $\mathbb{D}$ defined as 
				\begin{equation}
					\mathbb{D}= \brac*{z \in \mathbb{C}: \abs{z - z_{0}} \leq r}
				\end{equation}
			is completely contained in $U$. Let $f: U\to \mathbb{C}$ be a holomorphic function. Then for every $a \in \operatorname{int}{\mathbb{D}}$, 
				\begin{equation}
					f^{(n)}(a) = \frac{n!}{2\pi i}\oint_{\partial \mathbb{D}}\frac{f(z)}{(z - a)^{n + 1}}\;dz. 
				\end{equation}
		\end{quote}
	Returning to our original problem, assume that $|P(z)| \leq 1$ for all $|z| = 1$. Then by the Cauchy Integral Formula, 
		\begin{equation}
			n! = P^{(n)}(0) = \frac{n!}{2\pi i}\oint_{|z| = 1}\frac{P(z)}{z^{n + 1}}\;dz = 
		\end{equation}
	\textcolor{red}{[!! Complete Later !!]}
\end{solutions}
...
\begin{prb}{2019-A-I-3 (Geometry/Topology)}
