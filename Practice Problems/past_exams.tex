...
\begin{document}
	...
	\end{equation*}
	Since the sum and the integral are additive, the result holds for our dense subset. Fix an arbitrary $f \in C(\mathbb{T})$, and some $\epsilon > 0$. Then by the density of $\mathcal{A}$, there exists some $g \in \mathcal{A}$ so that for any $x \in \mathbb{T}$, $|f(x) - g(x)| < \epsilon$. 
