...
\title{Comps Mega Review \\ \normalsize{With 21 Illustrations} \vspace{-0.5cm}}
\author{\me}
\date{June 4, 2026}
...
\begin{document}
	...
	\end{prb}
	Let $X$ be the space obtained by gluing the boundary circle of the M\"obius band to a great circle (e.g., the equator) in $S^{2}$. We can compute the fundamental group of $X$ using CW structures. We assign to $S^{2}$ the CW structure comprised one 0-cell $v$, one 1-cell $e_{C}^{1}$ corresponding to the equator of $S^{2}$, and two 2-cells $e_{N}^{2}$ and $e_{S}^{2}$ corresponding to the northern and southern hemispheres. These 2-cells are both attached along the loop $e^{1}_{C}$. Now let $a$ be a 1-cell corresponding to the core circle of the M\"obius band, and attach $a$ to the vertex $v$. Let $e_{M}^{2}$ denote the 2-cell corresponding to the body of the M\"obius band. Since the boundary circle of the M\"obius band is attached to $e_{C}^{1}$, $\partial e_{M}^{2}$ is attached along $e_{C}^{1}$. But since the boundary circle of the M\"obius band wraps around the core circle $a$ twice, one has that $e_{C}^{1} = a^{2}$. Therefore, by van Kampen's theorem, we conclude that 
	\begin{equation*}
		\pi_{1}(X) \cong \braket{e_{C}^{1}, a \mid e_{C}^{1} = 1, e_{C}^{1} = 1, e_{C}^{1} = a^{2}} \cong \braket{a \mid a^{2} = 1} \cong \mathbb{Z}/2\mathbb{Z}. 
	\end{equation*}
	...
	\end{prb}
	Let $X$ be the space obtained by gluing the boundary circle of a M\"obius band to the equator of the 2-torus. We can compute the fundamental group of $X$ using CW structures. We assign to $T^{2}$ the CW structure comprised of one 0-cell $v$, two 1-cells $e^{1}_{C}$ and $e^{1}_{R}$ (where $e^{1}_{C}$ corresponds to the equator of the 2-torus), and one 2-cell $e^{2}_{T}$; $\partial e^{2}_{T}$ is attached along the boundary word $e^{1}_{R}e^{1}_{C}{e^{1}_{R}}^{-1}{e^{1}_{R}}^{-1}$. For the M\"obius band, we consider the 1-cell $a$ corresponding to the core circle that is attached to $V$. We also introduce the 2-cell $e^{2}_{M}$ corresponding to the body of $M$; $\partial e^{2}_{M}$ is attached to the equator $e^{1}_{C}$. But since the boundary of the M\"obius band wraps around the core circle twice, one has that $e^{1}_{C}= a^{2}$. Thus, by van Kampen's theorem, we obtain the following: 
	\begin{equation*}
		\pi_{1}(X) \cong \braket{a, e^{1}_{C}, e^{1}_{R} \mid [e^{1}_{C}, e^{1}_{R}] = 1, e^{1}_{C} = a^{2}} \cong \braket{a, e^{1}_{R} \mid [a^{2}, e^{1}_{R}] = 1}. 
	\end{equation*}
...
f2		\begin{equation*}
	...
	\end{prb}
	Assume to the contrary that there exists some $z \in \{|z| < |\alpha|\}$ such that $f(z) = 0$. Then by the Schwarz-Pick Lemma, one must have 
	\begin{equation*}
		|\alpha| = \abs{\frac{0 - \alpha}{1 - 0 (\alpha)}} \leq \abs{\frac{z - 0}{1 - \overline{z}(0)}} = |z|, 
	\end{equation*}
	which is a contradiction. Therefore, $f$ cannot have a zero inside this open disk. 
