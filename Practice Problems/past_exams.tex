...
\begin{document}
	...
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		...
		\item Now let $M = \mathbb{R}^{2n}$  We will explicitly construct a $\theta$ for which $\omega_\theta$ is nowhere vanishing. Let $(x_1, x_2, \ldots, x_{2n})$ be standard coordinates on $M$, and define the 1-form 
			\begin{equation*}
	...
	\end{prb}
	Let $M$ be a closed manifold that has an even dimensional sphere $S^{2n}$ as its universal cover. Let $\Gamma$ be the group of deck transformations\footnote{Note that a \textbf{deck transformation} is a homeomorphism $f: \ti{M} \to \ti{M}$ such that $p \circ f = p$, where $p: \ti{M} \to M$ is the covering map. In particular, deck transformations have he uniqueness property that states that if two such transformations are equal at any point $x \in \ti{M}$, then the two transformations are identical. Moreover, the identity map on $\ti{M}$ is a deck transformation. Thus if the deck transformation $h$ has a fixed point, then it must be the identity map.} on $S^{2n}$ (note that this implies $\pi_1(X) \cong \Gamma$, and every nonidentity element of $G$ acts freely on $S^{2n}$). Suppose $h: S^{2n} \to S^{2n}$ is fixed-point free. Then the Lefschetz number, 
		\begin{equation*}
			0 =L(h) = 1 + \operatorname{deg}(h) =0. 
		\end{equation*}
	Hence, every nonidentity deck transformation must have degree $-1$. Let $g_1, g_2 \in G$ be two nonidentity elements. Then $\operatorname{deg}(g_1g_2) = 1$. A nonidentity deck transformation cannot have degree 1, since then the Lefschetz number would be 2, which implies the existence of a fixed point. Therefore, $g_1g_2 = \operatorname{id}$, so that $G$ has at most one nonidentity element. Therefore, either $G$ is trivial or $G \cong \mathbb{Z}/2\mathbb{Z}$. This concludes the proof. 
	...
	\end{prb}
	Let $G$ be a nonabelian group of order $pq$, where $p,q$ are distinct primes. Assume without loss of generality that $p > q$. Let $n_p$ denote the number of $p$-subgroups of $G$. By Cauchy's theorem, since $G$ must contain at least one element of order $p$, it follows that $n_p \neq 0$. So let $H$ be a subgroup of $G$ of order $p$; this implies that $[G:p] = q$. But since any subgroup whose index is equal to the smallest prime divisor of $G$ is normal, one has that $H$ is normal in $G$. Moreover, since $|H| = p$, by the classification of groups of prime order, one has that $H \cong \mathbb{Z}/p \mathbb{Z}$. Next, by Cauchy's theorem, $G$ must have a subgroup $K$ of order $q$; consequently $K \cong \mathbb{Z}/q\mathbb{Z}$. Let $n_q$ denote the number of subgroups of $G$ of order $q$; we have already shown $n_q \geq 1$. 
