...
\begin{document}
	...
	\end{prb}
	Let $\gamma$ be a closed curve in $\mathbb{C}\setminus \{0\}$ with winding number $1$ around the origin, and define the normalized curve $\gamma': [0,1] \to S^1 \coloneqq \{z \in \mathbb{C}\setminus \{0\}: \norm{z} = 1\}$ defined by $\gamma'(t) = \gamma(t)/\norm{\gamma(t)}$. It is straightforward to see that $\gamma'(t)$ is well-defined and continuous. Since the universal cover of $S^1$ is $\mathbb{R}$ and $[0,1]$ is simply connected, it follows that $\gamma'$ lifts to a continuous map $\ti{\gamma}: [0,1] \to \mathbb{R}$, such that $\gamma' = p \circ \ti{\gamma}$, where $p: \mathbb{R}\twoheadrightarrow \mathbb{R}$ is the map defined by, 
	\begin{equation*}
		p(\theta) \coloneqq e^{2\pi i \theta}. 
	\end{equation*}
	Note that 
	\begin{align*}
		\begin{split}
			\gamma'(t_{1}) = -\gamma'(t_2) &\iff e^{2\pi i \ti{\gamma}(t_{1})} = -e^{2\pi i \ti{\gamma}(t_2)} \\
			&\iff e^{2\pi i (\ti{\gamma}(t_1) - \ti{\gamma}(t_2))} = -1
		\end{split}
	\end{align*}
