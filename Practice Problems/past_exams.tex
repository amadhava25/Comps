...
\begin{document}
	...
	\end{prb}
	Let $D$ be the rank-2 distribution in $\mathbb{R}^{3}$ spanned by the two vector fields $V$ and $W$ defined above. By the Frobenius Theorem, $D$ is integrable iff $D$ is involutive iff $[V, W]$ is a smooth section of $D$. We begin by computing, 
	\begin{align*}
		\begin{split}
			[V, W] &= (\partial_{x} + 2xy\partial_{z})(x\partial_{x}) + (\partial_{x} + 2xy\partial_{z})(\partial_{y}) + (\partial_{x}+ 2xy \partial_{z})((2x^{2}y + x^{2} - 2y)\partial_{z}) \\
			&\textcolor{white}{=} \hspace{5pt}- x\partial_{x}(\partial_{x} + 2xy\partial_{z}) - \partial_{y}(\partial_{x} + 2xy\partial_{z}) - (2x^{2}y + x^{2}-2y)\partial_{z}(\partial_{x}+ 2xy\partial_{z}) \\
			&= \partial_{x} + 0 + (4xy + 2x)\partial_{z} - 2xy\partial_{z} - 2x\partial_{z} - 0 = \partial_{x} + 2xy\partial_{z} = V. 
		\end{split} 
	\end{align*}
	Since $V$ is a smooth section of $D$, we conclude that $D$ is involutive, and thus integrable. To find an explicit formula for the integral submanifold passing through the point $(0,0,z_{0})$, we start by finding an annihilating 1-form that defines $D$. Let $\omega = a\;dx + b\;dy + c\;dz$. Then one has: 
	\begin{align*}
		\begin{split}
			0 = \omega(V) &= a + 2xyc.  \\
			0 = \omega(W) &= ax + b + (2x^{2}y + x^{2} -2y)c. 
		\end{split}
	\end{align*}
	Using the first equation in the second equation, one has that 
	\begin{equation*}
		(-2x^{2}y + 2x^{2}y + x^{2} - 2y)c + b = 0 \implies (x^{2}-2y)c + b= 0. 
	\end{equation*}
	Choose $c = 1$ so that $b = 2y - x^{2}$. Then $a = -2xy$. So an expression for $\omega$ is $\omega = -2xy\;dx + (2y - x^{2})\;dy + dz$. Note that we can write, 
	\begin{equation*}
		\omega = d(-x^{2}y + y^{2} + z), 
	\end{equation*}
	so that the integral submanifolds of $D$ are given by the level sets $-x^{2}y + y^{2} + z = c$. Since we are looking for the integral submanifold passing through $(0,0,z_{0})$, one has $c = z_{0}$ so that the explicit formula is simply, 
	\begin{equation*}
		z = z_{0} + x^{2}y - y^{2}. 
	\end{equation*}


	\begin{prb}{2016-J-II-1}
		Let $M$ be a smooth $n$-manifold, and let $\theta$ be a smooth 1-form on $M$ which is nonzero at every point. Show that $\theta \wedge d\theta = 0$ everywhere on $M$, if and only if, for each $p \in M$, we can find a coordinate system $(x^{1}, \ldots, x^{n})$ centered at $p$ in which 
		\begin{equation*}
			\theta = f(x^{1}, \ldots, x^{n})\;dx^{1}
		\end{equation*}
		for some smooth function $f > 0$. 
	\end{prb}
	Let $M$ be a smooth $n$-manifold, and let $\theta$ be a smooth 1-form on $M$ which is nonzero at every point. Assume that for each $p \in M$, we can find a coordinate system $(x^{1}, \ldots, x^{n})$ centered at $p$ in which 
	\begin{equation*}
		\theta = f(x^{1}, \ldots, x^{n})\;dx^{1}
	\end{equation*}
	for some smooth function $f > 0$. We first compute, $d\theta = df  \wedge dx^{1}$. Therefore, $\theta \wedge d\theta = f\;dx^{1} \wedge (df \wedge dx^{1}) = 0$. Now assume that $\theta \wedge d\theta = 0$ everywhere on $M$. Since $\theta$ is a nowhere vanishing $1$-form, let $\mathscr{D} = \ker{\theta}$ denote a codimension-1 distribution on $M$. Since $\operatorname{codim}(D) = 1$, at every $p \in M$, one can write $T_{p}M = D_{p} \oplus \mathbb{R}e_{n}$ for some $e_{n} \in T_{p}M \setminus D_{p}$. Since $\theta \wedge d\theta = 0$, $D$ is involutive, and thus Frobenus integrable (via the Frobenius theorem). This means that for each $p \in M$, it is possible to find a coordinate system $(x^{1}, \ldots, x^{n})$ centered at $p$ such that $D_{p}$ is spanned by $\partial_{x^{2}}, \ldots, \partial_{x^{n}}$. In particular, we see that $\theta$ must annihilate these $n$ basis vectors so that $\theta$ is of the form $f(x^{1}, \ldots, x^{n})\;dx^{1}$ for some smooth function $f$. Since $\theta$ is nowhere vanishing, $f$ must also be nowhere vanishing. If $f(p) < 0$, then we may apply the coordinate transformation $x^{1} \mapsto -x^{1}$ and $f \mapsto -f$ so that $\theta = f\;dx^{1} > 0$ on some neighborhood of $p$. 
	...
	Let $M$ be a smooth $n$-manifold, and let $\theta$ be a smooth 1-form on $M$ such that $\theta \neq 0$ everywhere. This means that $D$ is a codimension $1$ distribution on $M$. By the Frobenius theorem, $D$ is Frobenius integrable if and only if it is involutive, which is to say that if $X, Y \in \Gamma(D)$ are smooth sections of $D$, then their Lie bracket $[X, Y]$ is a smooth section of $D$. We begin by recalling the following formulas: 
	\begin{align*}
		\begin{split}
			d\theta(X, Y) &= X(\theta(Y)) - Y(\theta(X)) - \theta([X, Y]). \\
			(\theta \wedge d\theta)(X, Y, Z) &= \theta(X)d\theta(Y,Z) + \theta(Y)d\theta(Z, X) + \theta(Z)d\theta(X, Y). 
		\end{split} 
	\end{align*}
	Fix an arbitrary point $p \in M$. Since $\operatorname{codim}{D_{p}} = 1$, we can form a basis for $T_{p}M$ by choosing a basis $\{e_{1}, \ldots, e_{n-1}\}$ as a smooth local frame for $D$ on a neighborhood $U$ of $p$, extending a single vector $e_{n,p} \in T_{p}M\setminus D_{p}$ to a local vector field $e_{n}$ with $\theta(e_{n}) \neq 0$, and extending the collection $\{e_{1}, \ldots, e_{n-1}\}$ by $e_{n}$. We note that $\theta \wedge d\theta = 0$ if and only if it vanishes on all basis triples $(e_{i}, e_{j}, e_{k})$. If $i, j, k \in \{1, \ldots, n - 1\}$, then $(\theta \wedge d\theta)(e_{i}, e_{j}, e_{k}) = 0$ trivially since $\theta(e_{i}), \theta(e_{j}), \theta(e_{k}) = 0$. Now assume $i \in \{1, \ldots, n - 1\}$, and $j, k = n$. Then 
	\begin{align*}
		\begin{split}
			(\theta \wedge d\theta)(e_{i}, e_{n}, e_{n}) &= \theta(e_{n})d\theta(e_{n}, e_{i}) + \theta(e_{n})d\theta(e_{i},e_{n}) = 0, 
		\end{split}
	\end{align*}
	by skew-symmetry and multilinearity. Thus we have the final case: assume $i, j \in \{1, \ldots, n -1\}$ and $k = n$. Then 
	\begin{equation*}
		(\theta \wedge d\theta)(e_{i}, e_{j}, e_{n}) = \theta(e_{n})d\theta(e_{i}, e_{j}). 
	\end{equation*}
	Since $e_{n}\notin D_{p}$, $\theta(e_{n}) \neq 0$. Thus $(\theta \wedge d\theta)(e_{i}, e_{j}, e_{k}) = 0$ iff $d\theta(e_{i}, e_{j}) = 0$ iff $\theta([e_{i}, e_{j}]) = 0$ iff $[e_{i}, e_{j}] \in D_{p}$. if and only if for all smooth local sections $X, Y \in \Gamma(D\vert{}_U)$, $[X, Y]_p \in D_p$. Since $p \in M$ is arbitrary, it follows that $\theta \wedge d\theta = 0$ if and only if $D$ is involutive if and only if $D$ is Frobenius integrable.
	...
	\begin{prb}{2010-A-II-1}
		Let $M$ be a smooth $n$-manifold. Prove that a nowhere zero 1-form $\alpha$ on $M$ satisfies 
		\begin{equation*}
			\alpha \wedge d\alpha = 0 
		\end{equation*}
		if and only if there exists a covering of $M$ by open neighborhoods, and on each neighborhood there exists a coordinate system $(x_{1}, \ldots, x_{n})$ and a nowhere zero, smooth function $f(x_{1}, \ldots, x_{n})$ such that 
		\begin{equation*}
			\alpha = f(x_{1}, \ldots, x_{n})\;dx_{n}. 
		\end{equation*}
	\end{prb}
	Let $M$ be a smooth $n$-manifold, and let $\alpha$ be a nowhere vanishing 1-form on $M$> First assume that there exists a nowhere zero smooth function $f(x_{1}, \ldots, x_{n})$ such that $\alpha = f(x_{1}, \ldots, x_{n})\;dx_{n}$. Then one has $d\alpha = df \wedge dx_{n}$ so that $\alpha \wedge d\alpha = f\;dx_{n} \wedge (df \wedge dx_{n}) = 0$. Now assume that $\alpha \wedge d\alpha = 0$ at every $p \in M$. Let $D = \ker{\alpha}$ be a rank $(n-1)$-distribution on $M$. Since $\alpha \wedge d\alpha = 0$, $D$ is involutive and thus Frobenius integrable. This means that for every $p \in M$, it is possible to find a coordinate neighborhood $(U_{p}, (x^{i}_{(p)})_{i = 1}^{n})$ such that $\partial_{x^{1}}, \ldots, \partial_{x^{n-1}}$ spans $D$. This means that $\alpha$ annihilates each $\partial_{x^{1}}, \ldots, \partial_{x^{n}}$ so that $\alpha$ must be of the form $f(x^{1}, \ldots, x^{n})\;dx^{n}$ in $U_{p}$. Since $\alpha$ is nowhere vanishing on $U_{p}$ for every $p$, we conclude that $f$ is nowhere zero. Thus the proof concludes. 
	...
	\begin{prb}{LB-2020-F-6}
		Consider the two vector fields 
		\begin{equation*}
			V = \frac{\partial}{\partial x^{1}} + x^{3}\frac{\partial}{\partial x^{2}} \quad \text{and} \quad W = \frac{\partial}{\partial x^{3}} + x^{1}\frac{\partial}{\partial x^{4}}
		\end{equation*}
		on $\mathbb{R}^{4}$. Is there a 3-dimensional submanifold $X^{3} \subset \mathbb{R}^{4}$ that passes through the origin $(0,0,0,0)$ and is tangent to both $V$ and $W$ at every $p \in X$? If so, find it explicitly. Otherwise, prove that it cannot exist. 
	\end{prb}
	Let $V$ and $W$ be the two smooth vector fields on $\mathbb{R}^{4}$. If $V$ and $W$ are tangent to a 3-dimensional submanifold $X^{3} \subset \mathbb{R}^{4}$ that passes through the origin, then $[V, W]$ (the Lie bracket of $V$ and $W$) must also be tangent to $X^{3}$. Let $D$ be the rank-3 distribution in $\mathbb{R}^{4}$ defined by $D = \operatorname{span}\{V, W, [V, W]\}$. Thus the problem is equivalent to asking if there exists an integral submanifold of $D$ passing through the origin $(0,0,0,0)$. By the Frobenius theorem, this amounts to showing that the Lie bracket of each pair of spanning vector fields is a smooth section of $D$ and that $V, W, [V, W]$ are linearly independent at the origin. We begin by computing, 
	\begin{align*}
		\begin{split}
			[V, W] &= \left(\partial_{1} + x^{3}\partial_{2}\right)(\partial_{3}) + (\partial_{1} + x^{3}\partial_{2})(x^{1}\partial_{4}) - (\partial_{3} + x^{1}\partial_{4})(\partial_{1})- (\partial_{3} + x^{1}\partial_{4})(x^{3}\partial_{2}) \\
			&= 0 + \partial_{4} - 0 - \partial_{2} = -\frac{\partial}{\partial x^{2}} + \frac{\partial}{\partial x^{4}}. 
		\end{split}
	\end{align*}
	First, at the origin, $V = \partial_{1}$, $W = \partial_{3}$, $[V, W] = -\partial_{2} + \partial_{4}$, which are linearly independent. Next, we compute, 
	\begin{align*}
		\begin{split}
			[V, [V, W]] &= (\partial_{1}+ x^{3}\partial_{2})(-\partial_{2}) + (\partial_{1} + x^{3}\partial_{2})(\partial_{4}) - (-\partial_{2} + \partial_{4})(\partial_{1}) - (\partial_{2} + \partial_{4})(x^{3}\partial_{2}) \\
			&= 0 + 0 - 0 + 0 =0.  \\
			[W, [V, W]] &= (\partial_{3} + x^{1}\partial_{4})(-\partial_{2}) + (\partial_{3} + x^{1}\partial_{4})(\partial_{4}) -(-\partial_{2} + \partial_{4})(\partial_{3}) - (-\partial_{2} + \partial_{4})(x^{1}\partial_{4}) \\
			&= 0 + 0 - 0 - 0 = 0. 
		\end{split}
	\end{align*}
	Thus since $0 \in D$, we conclude that $D$ is an integrable rank-3 distribution. Therefore, by the Frobenius theorem, there exists a 3-dimensional submanifold $X^{3} \subset \mathbb{R}^{4}$ passing through the origin so that at each $p \in X$, $X$ is tangent to both $V$ and $W$. To find an expression for $X^{3}$ we seek to find a annihilating 1-form\footnote{We look for an annihilating \textit{1}-form because $D$ is a codimension 1 distribution on $\mathbb{R}^{4}$.} for the distribution $D$. Let $\omega = a_{1}dx^{1} + a_{2}dx^{2} + a_{3}dx^{3} + a_{4}dx^{4}$ be a annihilating 1-form; i.e., $\omega(V), \omega(W), \omega([V, W])= 0$. Plugging in, one has 
	\begin{align*}
		\begin{split}
			0 = \omega(V) &= a_{1} + a_{2}x^{3} \implies a_{1} =-a_{2}x^{3}. \\
			0 = \omega(W) &= a_{3} + + a_{4}x^{1} \implies a_{3} = -a_{4}x^{1}. \\
			0 = \omega([V,W]) &= -a_{2} + a_{4} \implies a_{2} = a_{4}. 
		\end{split}
	\end{align*}
	Set $a_{2} = 1 = a_{4}$ so that $a_{1} = -x^{3}$ and $a_{3} = -x^{1}$. Then 
	\begin{equation*}
		\omega = -x^{3}\;dx^{1} + dx^{2} -x^{1}\;dx^{3} + dx^{4} = d(x^{2} + x^{4}) - d(x^{1}x_{3}) = d(x^{2} + x^{4} - x^{1}x_{3}) = 0. 
	\end{equation*}
	$X$ is defined by $\omega = 0$ so that $x^{2}+ x^{4} - x^{1}x^{3} = C$, here $C$ is some constant. Since $X$ passes through the origin, one has $C = 0$. Therefore, $X$ is given in coordinates by $x^{2} + x^{4} - x^{1}x^{3}= 0$. 
	
	\begin{prb}{AZ-2011-F-2}
		Let $X$ and $Y$ be the vector fields on $\mathbb{R}^{3}$ given by 
		\begin{equation*}
			X = \frac{\partial}{\partial x} + x\frac{\partial}{\partial y} + y\frac{\partial}{\partial z}, \qquad Y = y\frac{\partial}{\partial x} + z\frac{\partial}{\partial y} + \frac{\partial}{\partial z}. 
		\end{equation*}
		\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
			\item Compute $[X, Y]$. 
			\item Is there a coordinate chart $\varphi = (x_{1}, x_{2}, x_{3}): U \to \mathbb{R}^{3}$ on a neighborhood of the origin in $\mathbb{R}^{3}$ such that 
			\begin{equation*}
				X|_{U} = \frac{\partial}{\partial x_{1}}, \qquad Y|_{U} = \frac{\partial}{\partial x_{2}}? 
			\end{equation*} 
		\end{enumerate}
	\end{prb}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		...
		\begin{align*}
			\begin{split}
				[X, Y] &= (\partial_{x} + x\partial_{y} + y\partial_{z})(y\partial_{x}) + (\partial_{x} + x\partial_{y} + y\partial_{z})(z\partial_{y}) + (\partial_{x} + x\partial_{y} + y\partial_{z})(\partial_{z}) \\
				&\textcolor{white}{=} \hspace{2pt} - (y\partial_{x} + z\partial_{y} + \partial_{z})(\partial_{x}) - (y\partial_{x} + z\partial_{y} + \partial_{z})(x\partial_{y}) - (y\partial_{x} + z\partial_{y} + \partial_{z})(y\partial_{z}) \\
				&= x\partial_{x} + y\partial_{y} + 0 - 0 -y\partial_{y} - z\partial_{z} \\
				&= x\frac{\partial}{\partial x} - z\frac{\partial}{\partial z}. 
			\end{split} 
		\end{align*}
		\item From (a), we see that $[X, Y]$ does not vanish in any neighborhood $U$ of the origin in $\mathbb{R}^{3}$ since any such neighborhood contains points $(x, y, z)$ where neither $x$ nor $z$ are both zero. If there was a coordinate chart $\varphi = (x_{1}, x_{2}, x_{3}): U \to \mathbb{R}^{3}$ on a neighborhood $U$ of the origin in $\mathbb{R}^{3}$ such that $X|_{U} = \partial_{x_{1}}$ and $Y|_{U} = \partial_{x_{2}}$, then $[X, Y]|_{U} = 0$, which we've shown is not possible. Therefore, it is not possible to find such a coordinate chart. 
	\end{enumerate}
	...
	\begin{prb}{2004-J-II-1}
		...
		\end{equation*}
	\end{prb}
	Let $X = S^{2} \times S^{1}$, and let $Y = T^{3}$. Let $f: X \to Y$ denote an infinitely differentiable map. One notes by K\"unneth's formula that 
	\begin{equation*}
		H^{3}_{\text{dR}}(X) \cong H^{2}_{\text{dR}}(S^{2}) \times H^{1}_{\text{dR}}(S^{1}) \cong \mathbb{R}. 
	\end{equation*}
	On the other hand, $H^{3}_{\text{dR}}(T^{3}) \cong \mathbb{R}^{3}$ is generated by the product $[d\theta_{1}] \smile [d\theta_{2}] \smile [d\theta_{3}]$, where each $[d\theta_{j}] \in H^{1}_{\text{dR}}(T^3)$. The pullback of each $d\theta_{j}$ is itself a degree-one form so that the pullback their product is the product of three degree-one forms, and hence is zero (since we showed $H^{3}_{\text{dR}}(X)$ to be one-dimensional). Thus the proof concludes. 
	...
	\begin{prb}{AZ-2011-F-4}
		Show that there exist a closed 1-form $\alpha$ on $\mathbb{RP}^{n}$ and a smooth function $f: [0,1] \to \mathbb{RP}^{n}$ so that 
		\begin{equation*}
			f(0) = f(1) \qquad \text{and} \qquad \int_{[0,1]}f^{\ast}\alpha \neq 0
		\end{equation*}
		if and only if $n = 1$. 
	\end{prb}
	First assume $n = 1$. Since $\mathbb{RP}^{1} \simeq S^{1}$, the 1-form $\alpha = d\theta$ and the function $f(t) = e^{2\pi i \theta}$ give a nonzero integral (i.e., $2\pi$). 
	
	If $n < 0$, then $\mathbb{RP}^{n} = \varnothing$ so that the result holds trivially. If $n = 0$, then $\mathbb{RP}^{n} = \{\text{pt}\}$ so that there exist no nonzero 1-forms. 
	
	Now assume $n \geq 2$. Since $\pi_{1}(\mathbb{RP}^{n}) \cong \mathbb{Z}/2\mathbb{Z}$ and $H^{1}_{\text{dR}}(\mathbb{RP}^{n}) \cong \operatorname{Hom}(\pi_{1}(\mathbb{RP}^{n}), \mathbb{R})$, it follows that $H^{1}_{\text{dR}}(\mathbb{RP}^{n}) = 0$. Thus every closed 1-form $\alpha$ on $\mathbb{RP}^{n}$ is exact. Fix an arbitrary closed 1-form $\alpha = dh$, where $h: \mathbb{RP}^{n} \to \mathbb{R}$ is a smooth function. Let $f: [0,1] \to \mathbb{RP}^{n}$ be any smooth function such that $f(0) = f(1)$. Then 
	\begin{equation*}
		\int_{[0,1]}f^{\ast}\alpha = \int_{[0,1]}f^{\ast}dh = \int_{[0,1]}d(f^{\ast}h) = \int_{[0,1]}d(h \circ f) = \int_{\partial[0,1]}h \circ f = h(f(1)) - h(f(0)) = 0. 
	\end{equation*}
	...
	\begin{prb}{2018-A-II-6}
		Let $M$ be a smooth, compact $m$-manifold without boundary, and suppose that $\Phi: M \to T^{k}$ is a smooth submersion. Here 
		\begin{equation*}
			T^{k} = \underbrace{S^{1} \times \dotsm \times S^{1}}_{k} = \mathbb{R}^{k}/\mathbb{Z}^{k}
		\end{equation*}
		is the $k$-dimensional torus; and saying that the smooth map $\Phi$ is a submersion just means that its derivative is surjective at every point. Prove that the de Rham cohomology $H^{1}_{\text{dR}}(M)$ is at least $k$-dimensional. 
	\end{prb}
	Let $M$ be a smooth compact $m$-manifold without boundary, and assume that $\Phi: M \to T^{k}$ is a smooth submersion. We start by computing (e.g., using K\"unneth's formula) that $H^{1}_{\text{dR}}(T^{k}) \cong \mathbb{R}^{k}$; thus let $[d\theta_{1}], \ldots, [d\theta_{k}]$ be the nonzero generators of $H^1_{\text{dR}}(T^{k})$, and let $[\alpha_{1}], \ldots, [\alpha_{k}]$ be the pullback of these cohomology classes into $H^{1}_{\text{dR}}(M)$. We start by showing that these latter cohomology classes are linearly independent. Suppose one has 
	\begin{equation*}
		\sum_{j = 1}^{k}a_{j}[\alpha_{j}] = 0. 
	\end{equation*}
	In particular, the following relation holds for the representative $1$-forms $\alpha_{j}$: 
	\begin{equation*}
		\sum_{j = 1}^{k}a_{j}\alpha_{j} = d\beta, \qquad \beta \in \Omega^{0}(M). 
	\end{equation*}
	I.e., $\beta$ is a smooth function on $M$. Since $M$ is compact, by the extreme value theorem, it follows there exists a point $p \in M$ at which $d\beta = 0$ ($\beta$ has a local extrema at this point); let $q = \Phi(p)$. Note that 
	\begin{equation*}
		\sum_{j = 1}^{k}a_{j}\alpha_{j} = \Phi^{\ast}\eta, \qquad \eta = \sum_{j = 1}^{k}a_{j}\alpha_{j}. 
	\end{equation*}
	Since $\Phi$ is a smooth submersion, it follows that $\Phi^{\ast}\eta$ is zero at $q$ if and only if $\eta$ is zero at $q$. But since the $d\theta_{j}$ are linearly independent everywhere, $\eta$ is zero at $q$ if and only if the $a_{j}$ are all zero. Thus, this establishes that the cohomology classes $[\alpha_{j}]$ are all linearly independent. Thus, $\dim{H^{1}_{\text{dR}}(M)} \geq k$. 
	...
	Assume the hypotheses, and also assume that $F$ has no critical points; i.e., assume that $dF_{p}: T_{p}M to T_{F(p)}\mathbb{R}^{n}$ is surjective at every $p \in M$. By the rank-nullity theorem, it follows that $F$ is also a smooth immersion, so that by the inverse function theorem, $F$ is a local homeomorphism. Since local homeomorphisms are open maps, $F(M)$ is open and nonempty in $\mathbb{R}^{n}$. Since $f$ is continuous, and $M$ is compact, $F(M)$ is compact in $\mathbb{R}^{n}$; $F(M)$ is closed in $\mathbb{R}^{n}$ by the fact that $\mathbb{R}^{n}$ is Hausdorff. Thus since $F(M)$ is a nonempty open and closed subset of $\mathbb{R}^{n}$, and $\mathbb{R}^{n}$ is connected, $F(M) = \mathbb{R}^{n}$. But this is impossible since $\mathbb{R}^{n}$ itself is not compact (take e.g., the open cover consisting of a unit open ball centered at each point in $\mathbb{R}^{n}$). Therefore, $F$ must have a critical point 
	...
	\begin{prb}{2009-A-II-2}
		Consider the following 2-manifolds: the plane $\mathbb{R}^{2}$, the torus $T^{2}$, and the complement $T^{2}\setminus \{p\}$ of some point $p$ in $T^{2}$. \vspace{-0.25cm}
		\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
			\item Prove that there does not exist a smooth immersion $F: T^{2} \to \mathbb{R}^{2}$.
			\item Draw a picture of a smooth immersion $F: T^{2}\setminus \{p\} \to \mathbb{R}^{2}$. 
		\end{enumerate}
	\end{prb}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item Assume to the contrary that there exists a smooth immersion $F: T^{2} \to \mathbb{R}^{2}$; i.e., assume that $dF_{p}: T_{p}T^{2} \to T_{F(p)}\mathbb{R}^{2}$ is injective at every $p \in T^{2}$. Since $\dim{T_{p}T^{2}} = 2 = \dim{T_{p}\mathbb{R}^{2}} < \infty$, it also follows that $df_{p}$ is surjective at every $p \in T^{2}$. Thus $df_{p}$ is bijective at every $p$, which means that $f$ is a local diffeomorphism. Consider the image $f(T^{2}) \subseteq \mathbb{R}^{2}$. Since $T^{2}$ is compact and $f$ is continuous, $f(T^{2})$ is compact (hence closed) in $\mathbb{R}^{2}$. Since local diffeomorphisms are open maps, $f(T^{2})$ is open in $\mathbb{R}^{2}$. Since $T^{2}$ is nonempty, $f(T^{2})$ is nonempty. Therefore, since $\mathbb{R}^{2}$ is connected, $f(T^{2}) = \mathbb{R}^{2}$, which is a contradiction since $\mathbb{R}^{2}$ is not compact. Therefore, by contradiction, there cannot exist a smooth immersion $f: T^{2} \to \mathbb{R}^{2}$. 
		\item \complete 
	\end{enumerate}
	...
	\end{prb}
	Let $W_{1}, W_{2}$ be 2-dimensional linear subspaces of $\mathbb{R}^{4}$ that intersect only at the origin, and define $X = \mathbb{R}^{4} \setminus (W_{1} \cup W_{2})$. Since $X$ is connected, $H^{0}_{\text{dR}}(X) \cong \mathbb{R}$. On the other hand, since $X$ is a connected, noncompact smooth manifold without boundary, $H^{4}_{\text{dR}}(X) = 0$. Now consider the open subsets $U = \mathbb{R}^{4}\setminus W_{1}$ and $V = \mathbb{R}^{4}\setminus W_{2}$. Then $U \cap V = X$ and $U \cup V = \mathbb{R}^{4} \setminus \{0\} \cong S^{3}$. Moreover, $U, V \cong S^{1}$. Thus, by the Mayer-Vietoris long exact sequence for the de Rham cohomology, we have the exact sequence 
	\begin{equation*}
	...
	\begin{prb}{2021-J-II-3}
		Let $M$ denote a connected and closed smooth manifold which is not orientable; let $\rho: \hat{M} \to M$ denote the two-fold covering map from the orientable covering manifold $\hat{M}$ for $M$. Show that the mapping $\rho^{\ast}: H^{\ast}_{\text{dR}}(M) \to H^{\ast}_{\text{dR}}(\hat{M})$ induced by $\rho$ on the de Rham cohomology is injective. 
	\end{prb}
	Let $M$ be a connected and closed smooth nonorientable manifold, and let $\rho: \hat{M} \to M$ denote the two-fold covering map from $\hat{M}$ for $M$. Since $\rho$ is a 2-fold covering map, there exists a nontrivial deck transformation $\tau: \hat{M} \to M$ satisfying $\tau^{2} = \operatorname{id}_{\hat{M}}$. Let $\alpha$ be a closed $k$-form on $M$, and assume that $\rho^{\ast}\alpha = d\beta$ on $\hat{M}$, where $\beta \in \Omega^{k -1}(\hat{M})$. In words, we are assuming that $[\alpha]$ belongs to the kernel of $\rho^{\ast}$. Since $\rho^{\ast}$ is injective iff its kernel is trivial, our goal reduces to showing that $\alpha$ is an exact $k$-form so that $[\alpha] = 0$. We define the average, 
	\begin{equation*}
		\beta_{0} = \frac{1}{2}(\beta + \tau^{\ast}\beta) \implies \tau^{\ast}\beta_{0} = \frac{1}{2}(\tau^{\ast}\beta + {\tau^{\ast}}^{2}\beta) = \frac{1}{2}(\tau^{\ast}\beta + \beta). 
	\end{equation*}
	Thus since $\beta_{0}$ is invariant under the deck transformations, it descends to a $k - 1$ form $\gamma$ on $M$ with $\rho^{\ast}\gamma = \beta_{0}$. We compute, 
	\begin{equation*}
		d\beta_{0} = \frac{1}{2}(\tau^{\ast}\rho^{\ast}\alpha + \rho^{\ast}\alpha) = \frac{1}{2}((\rho \circ\tau)^{\ast}\alpha + \rho^{\ast}\alpha) = \rho^{\ast}\alpha, 
	\end{equation*}
	where we used the property of deck transformations to write $\rho \circ \tau = \rho$. Thus, $\rho^{\ast}\gamma = \beta_{0} \implies \rho^{\ast}(d\gamma) = d\beta_{0} = \rho^{\ast}\alpha$. Since $\rho$ is a local diffeomorphism, pullback on forms is injective so that $d\gamma = \alpha$. Since we've shown $\alpha$ to be an exact $k$-form on $M$, $[\alpha] = 0$ so that $\rho^{\ast}: H^{\ast}_{\text{dR}}(M) \to H^{\ast}_{\text{dR}}(\hat{M})$ is injective. 
	...
	\begin{prb}{2011-A-II-6}
		The $n$-torus $\mathbb{T}^{n} = \mathbb{R}^{n}/\mathbb{Z}^{n}$ is both a smooth $n$-manifold and an abelian group, by virtue of structures inherited from the real vector space $\mathbb{R}^{n}$. In particular, $\mathbb{T}^{n}$ acts on itself by smooth maps corresponding to translations of $\mathbb{R}^{n}$. By averaging, prove that each de Rham cohomology class on $\mathbb{T}^{n}$ contains a unique differential form which is translation-invariant. Then use this observation to calculate the dimensions of the de Rham cohomology vector spaces $H^{p}_{\text{dR}}(\mathbb{T}^{n})$ for all $p$. 
	\end{prb}
	Let $\mathbb{T}^n = \mathbb{R}^n / \mathbb{Z}^n$ equipped with the normalized Haar volume form $dg = dx_1 \wedge \dots \wedge dx_n$, such that:
	
	\begin{equation*}
		\int_{\mathbb{T}^n} dg = 1
	\end{equation*}
	
	For each $g \in \mathbb{T}^n$, let $L_g : \mathbb{T}^n \to \mathbb{T}^n$ be the translation map $L_g(x) = x + g$. Given any differential $p$-form $\alpha \in \Omega^p(\mathbb{T}^n)$, we define its \textbf{average} $\bar{\alpha}$ as:
	
	\begin{equation*}
		\bar{\alpha} = \int_{\mathbb{T}^n} (L_g^* \alpha) \, dg
	\end{equation*}
	
	To show that $\bar{\alpha}$ is translation-invariant, pick any $h \in \mathbb{T}^n$. Pulling back $\bar{\alpha}$ along $L_h$ gives:
	
	\begin{equation*}
		L_h^* \bar{\alpha} = L_h^* \left( \int_{\mathbb{T}^n} L_g^* \alpha \, dg \right) = \int_{\mathbb{T}^n} (L_g \circ L_h)^* \alpha \, dg = \int_{\mathbb{T}^n} L_{g+h}^* \alpha \, dg
	\end{equation*}
	
	By the translation invariance of the Haar measure $dg$ (performing the change of variables $g' = g + h$), we obtain:
	
	\begin{equation*}
		L_h^* \bar{\alpha} = \int_{\mathbb{T}^n} L_{g'}^* \alpha \, dg' = \bar{\alpha}
	\end{equation*}
	
	Thus, $\bar{\alpha}$ is translation-invariant.
	
	Since $\mathbb{T}^n$ is connected, each translation $L_g$ is smoothly homotopic to the identity map $\text{id}_{\mathbb{T}^n} = L_0$. 
	
	By the homotopy formula for differential forms, there exists a smooth homotopy operator $K_g : \Omega^p(\mathbb{T}^n) \to \Omega^{p-1}(\mathbb{T}^n)$ (varying smoothly with $g$) such that:
	
	\begin{equation*}
		L_g^* \alpha - \alpha = d(K_g \alpha) + K_g(d\alpha)
	\end{equation*}
	
	Since $\alpha$ represents a cohomology class, it is closed ($d\alpha = 0$). This simplifies the formula to:
	
	\begin{equation*}
		L_g^* \alpha - \alpha = d(K_g \alpha)
	\end{equation*}
	
	Integrating both sides over $g \in \mathbb{T}^n$:
	
	\begin{equation*}
		\bar{\alpha} - \alpha = \int_{\mathbb{T}^n} (L_g^* \alpha - \alpha) \, dg = \int_{\mathbb{T}^n} d(K_g \alpha) \, dg = d \left( \int_{\mathbb{T}^n} K_g \alpha \, dg \right)
	\end{equation*}
	
	Defining $\beta = \int_{\mathbb{T}^n} K_g \alpha \, dg \in \Omega^{p-1}(\mathbb{T}^n)$, we have $\bar{\alpha} - \alpha = d\beta$. Therefore, $\bar{\alpha}$ and $\alpha$ differ by an exact form, proving that:
	
	\begin{equation*}
		[\bar{\alpha}] = [\alpha] \in H^p_{\text{dR}}(\mathbb{T}^n)
	\end{equation*}
	
	Suppose $\omega_1$ and $\omega_2$ are two translation-invariant $p$-forms in the same cohomology class $[\alpha]$. Then their difference $\omega = \omega_1 - \omega_2$ is translation-invariant and exact:
	
	\begin{equation*}
		\omega = d\eta \quad \text{for some } \eta \in \Omega^{p-1}(\mathbb{T}^n)
	\end{equation*}
	
	Averaging both sides gives:
	
	\begin{equation*}
		\bar{\omega} = \overline{d\eta} = d\bar{\eta}
	\end{equation*}
	
	Since $\omega$ is already translation-invariant, $\bar{\omega} = \omega$. Furthermore, $\bar{\eta}$ is a translation-invariant $(p-1)$-form.
	
	In the standard coordinate system $(x_1, \dots, x_n)$ on $\mathbb{T}^n$, a differential form is translation-invariant if and only if its coefficient functions are \textbf{constant}. Thus, $\bar{\eta}$ can be written as:
	
	\begin{equation*}
		\bar{\eta} = \sum_{I} c_I \, dx_I, \quad c_I \in \mathbb{R}
	\end{equation*}
	
	Taking the exterior derivative of constant-coefficient forms yields zero:
	
	\begin{equation*}
		d\bar{\eta} = 0 \implies \omega = 0 \implies \omega_1 = \omega_2
	\end{equation*}
	
	This proves that each cohomology class contains a \textbf{unique} translation-invariant representative.
	
	From the results above, the inclusion of translation-invariant forms $\Omega^p_{\text{inv}}(\mathbb{T}^n)$ into the space of closed forms induces a canonical vector space isomorphism:
	
	\begin{equation*}
		H^p_{\text{dR}}(\mathbb{T}^n) \cong \Omega^p_{\text{inv}}(\mathbb{T}^n)
	\end{equation*}
	
	The space $\Omega^p_{\text{inv}}(\mathbb{T}^n)$ is spanned by all basic $p$-forms with constant real coefficients:
	
	\begin{equation*}
		\left\{ dx_{i_1} \wedge dx_{i_2} \wedge \dots \wedge dx_{i_p} \;\middle|\; 1 \le i_1 < i_2 < \dots < i_p \le n \right\}
	\end{equation*}
	
	These forms are linearly independent, so the dimension of $\Omega^p_{\text{inv}}(\mathbb{T}^n)$ is precisely the number of ways to choose $p$ distinct indices from $n$:
	
	\begin{equation*}
		\dim H^p_{\text{dR}}(\mathbb{T}^n) = \binom{n}{p}
	\end{equation*}
	...
	\begin{prb}{2003-J-I-4}
		Let $X$ be a smooth connected orientable $n$-manifold, and let $Y \subset X$ be a smooth, nonempty, compact orientable $(n-1)$-dimensional submanifold. $Y$ admits a neighborhood in $X$ diffeomorphic to $Y \times \mathbb{R}$, $\mathbb{R}$ the real numbers (do not prove this). Show that if $X \setminus Y$ is connected, then $H^{1}_{\text{dR}}(X) \neq 0$, where $H^{1}_{\text{dR}}$ denotes the first de Rham cohomology group. (Hint: consider a suitable differential form with compact support in a neighborhood of $Y$.) 
	\end{prb}
		Let $Y$ be a compact $(n -1)$-manifold, and assume that $U \simeq Y \times \mathbb{R} \simeq Y \times (-1,1)$, which means that every point $p \in U$ can be specified by the coordinates $(x, t)$ ($x \in Y$, and $t \in (-1,1)$). Fix an arbitrary $0 < \epsilon < 1$, and let $\omega$ be a 1-form on $U$ of the form $f(t)\;dt$, where $\operatorname{supp}{f(t)} \subset (-\epsilon, \epsilon)$ such that $\int_{-\epsilon}^{\epsilon}f(t)\;dt = 1$. Since $\omega$ vanishes near $\pm 1$, we can smoothly extend $\omega$ by 0 to all of $X$; it is straightforward to check that $\omega$ is a closed smooth 1-form on $X$. It suffices to show that $\omega$ is not exact: if $\omega$ were exact (i.e., if $\omega = dh$ for some smooth function $h$, then necessarily, $\int_{\gamma}\omega = 0$ for any closed loop $\gamma \subset X$). Now fix some $\epsilon < a < 1$ and $x_{0} \in Y$, and let $\gamma$ be a closed loop based at $(x_{0}, -a)$ with the following components: (1) let $\gamma_{1}: (x_{0}, -a) \to (x_{0}, a)$ be the straight line segment passing through $Y$, (2) let $\gamma_2$ be a path from $(x_0, a)$ to $(x_0, -a)$ contained in $X \setminus (Y \times (-\epsilon, \epsilon))$. We note that such a loop $\gamma_2$ exists because $X \setminus (Y \times (-\epsilon, \epsilon))$ is open and connected (as it deformation retracts from the connected manifold $X \setminus Y$), hence path-connected (since $X$ is locally path-connected). By construction, $\omega$ vanishes identically outside $Y \times (-\epsilon, \epsilon)$, so $\omega = 0$ along $\gamma_2$. This means, $\int_{\gamma}\omega = \int_{-a}^{a}f(t)\;dt$. But since $\operatorname{supp}{f} \subset (-\epsilon, \epsilon) \subset (-a,a)$, one has that $\int_{\gamma}\omega = \int_{-\epsilon}^{\epsilon}f(t)\;dt = 1 \neq 0$. Thus $\omega$ cannot be an exact 1-form, which means $[\omega] \neq 0\in H^{1}_{\text{dR}}(X)$. Thus $H^{1}_{\text{dR}}(X) \neq 0$. 
	...
	\begin{prb}{LB-2020-F-5}
		Let $M$ be a smooth compact connected manifold, and suppose that $\varphi \in \Omega^{1}(M)$ is a closed 1-form that satisfies $\varphi \neq 0$ at every point of $M$. Show that $H^{1}(M) \neq 0$, that $\pi_{1}(M)$ is infinite, and that the universal cover of $M$ is non-compact. 
	\end{prb}
	Let $M$ be a smooth compact connected manifold. 
	\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
		\item Let $\varphi \in \Omega^{1}(M)$ be a closed nowhere vanishing $1$-form on $M$, and assume to the contrary that $\varphi$ is exact; i.e., assume there exists a smooth function $f: M \to \mathbb{R}$ such that $\varphi = df$. Since $M$ is compact, it follows by the extreme value theorem that $f$ has critical points in $M$; i.e., there exists some point $p \in M$ such that $df_{p} = 0$. But this implies that $\varphi(p) = 0$, which is a contradiction! Thus $\varphi$ cannot be exact so that $[\varphi] \in H^{1}_{\text{dR}}(M)$ is nonzero. Hence, $H^{1}_{\text{dR}}(M) \neq 0$. 
		\item Assume to the contrary that $\pi_{1}(M)$ is finite. We recall that $H^{1}_{\text{dR}}(M) \cong \operatorname{Hom}(\pi_{1}(M), \mathbb{R})$. For any nonzero $[\gamma] \in \pi_{1}(M)$, the order of $[\gamma]$ is finite. So if $\phi \in \operatorname{Hom}(\pi_{1}(M), \mathbb{R})$, then the order of $\phi([\gamma])$ must divide the order of $[\gamma]$. But every nonzero element in $\mathbb{R}$ has infinite order. This means that $\phi([\gamma])$ must be zero. Thus $\varphi$ is the trivial homomorphism. This implies that $\operatorname{Hom}(\pi_{1}(M), \mathbb{R})$ is trivial so that $H^{1}_{\text{dR}}(M)$ is trivial, which is a contradiction. Therefore, $\pi_{1}(M)$ must be infinite. 
		\item Let $\hat{p}: \hat{M} \to M$ denote the universal cover of $M$, and assume to the contrary that $\hat{M}$ is compact. We note that $\pi_{1}(M) \cong \Gamma$, where $\Gamma$ is the group of deck transformations on $\hat{M}$. Since we showed that $\pi_{1}(M)$ is infinite, for each $x \in M$, $\hat{p}^{-1}(x)$ is a closed, discrete, infinite subset, which means $\hat{p}^{-1}(x)$ cannot be compact. However, we also know that every closed subspace of a compact space (like $\hat{M}$) must be compact. Thus by contradiction, $\hat{M}$ cannot be compact. 
	\end{enumerate}

	\begin{prb}{LB-2020-F-8}
		Let $$\mathbb{T}^{n} = \mathbb{R}^{n}/\mathbb{Z}^{n} \simeq \underbrace{S^{1} \times \dotsm S^{1}}_{n}$$ be the so-called $n$-dimensional torus, or $n$-torus. Show that each of the constant-coefficient differential forms 
		\begin{equation*}
			dx^{i_{1}} \wedge \dotsm \wedge dx^{i_{k}} \in \Omega^{k}(\mathbb{R}^{n}), \qquad 1 \leq i_{1} < \dotsm < i_{k} \leq n, 
		\end{equation*}
		is the pullback to $\mathbb{R}^{n}$ of a closed differential form on $\mathbb{T}^{n} = \mathbb{R}^{n}/\mathbb{Z}^{n}$. Using an integration argument, then prove that the cohomology classes of these forms are linearly independent in $H^{k}(\mathbb{T}^{n})$. As a consequence, conclude that 
		\begin{equation*}
			\dim{H^{k}(\mathbb{T}^{n})} \geq \binom{n}{k}
		\end{equation*}
		for any integers $k$ and $n$ with $0 \leq k \leq n$. 
	\end{prb}
	Let $\mathbb{T}^{n}= \mathbb{R}^{n}/\mathbb{Z}^{n}$ denote the $n$-dimensional torus. Since each constant-coefficient differential $k$-form $dx^{i_{1}}\wedge \dotsm \wedge dx^{i_{k}}$ is invariant under translations by $\mathbb{Z}^{n}$, each $dx^{i_{1}} \wedge \dotsm \wedge dx^{i_{k}}$ descends uniquely to a closed (sinc $dx^{i_{1}} \wedge \dotsm \wedge dx^{i_{k}}$ is closed) $k$-form $\alpha_{i_{1}\dotsm i_{k}}$ on $\mathbb{T}^{n}$. For each $k$-tuple $\ti{I} = (i_{1}, \ldots, i_{k})$ where $i_{1} < i_{2} < \dotsm < i_{k}$, let $T_{\ti{I}}$ denote the coordinate $k$-torus in the $x^{i_{1}}, \ldots, x^{i_{k}}$ direction. Then it follows that for a strictly increasing $k$-tuple $\ti{J}$  
	\begin{equation*}
		\int_{T_{\ti{J}}}\alpha_{\ti{I}} = \begin{cases}1, & \ti{I} = \ti{J} \\ 0, & \ti{I} \neq \ti{J} \end{cases}
	\end{equation*}
	since the pullback $\alpha_{\ti{I}}$ is merely the volume form on $T_{\ti{I}}$. 
	Now suppose that $\mathscr{I}$ denotes the set of all strictly increasing $k$-tuples (where each component belongs to the set $\{1, \ldots, n\}$), and assume that  
	\begin{equation*}
		\sum_{\ti{I} \in \mathscr{I}}a_{\ti{I}}[\alpha_{\ti{I}}] = 0. 
	\end{equation*} 
	This implies that the representative $k$-form $\sum_{\ti{I} \in \mathscr{I}}a_{\ti{I}}\alpha_{\ti{I}}$ is exact. Thus since $\mathbb{T}^{n}$ is compact and boundaryless, Stokes' theorem tells us that for any strictly increasing $k$-tuple $\ti{J} \in \mathscr{I}$, 
	\begin{equation*}
		\int_{T_{\ti{J}}}\sum_{\ti{I} \in \mathscr{I}}a_{\ti{I}}\alpha_{\ti{I}} = 0. 
	\end{equation*}
	On the other hand, by our work above, one has that 
	\begin{equation*}
		\int_{T_{\ti{J}}}\sum_{\ti{I} \in \mathscr{I}}a_{\ti{I}}\alpha_{\ti{I}} = a_{\ti{J}}. 
	\end{equation*}
	In this way, we can then show that each of the coefficients in the above sum must be zero so that all $\binom{n}{k}$ classes in $H^{k}_{\text{dR}}(M)$ are linearly independent. Consequently, we conclude that 
	\begin{equation*}
		\dim{H^{k}_{\text{dR}}(\mathbb{T}^{n})} \geq \binom{n}{k}. 
	\end{equation*}
	\begin{prb}{LB-2020-F-9}
		If $M$ is a smooth $.n$-manifold such that $\dim{H^{k}(M)}$ is finite-dimensional for each $k$, the \textit{Euler characteristic} of $M$ is defined to be the integer 
		\begin{equation*}
			\chi(M) = \sum_{k =0}^{n}(-1)^{k}\dim{H^{k}(M)}. 
		\end{equation*}
		If $M = U \cup V$, where $U, V \subset M$ are open sets such that $H^{k}(U)$, $H^{k}(V)$, and $H^{k}(U \cap V)$ are finite-dimensional for every $k$, show that 
		\begin{equation*}
			\chi(M) = \chi(U) + \chi(V) - \chi(U \cap V). 
		\end{equation*}
	\end{prb}
	Let $M$ be a smooth $n$-manifold such that $\dim{H^{k}(M)}$ is finite for each $k$, and let $U, V$ be open subsets of $M$ such that $M = U \cup V$, and $H^{k}(U), H^{k}(V)$, $H^{k}(U \cap V)$ are finite-dimensional for every $k$. By the Mayer-Vietoris theorem, we have the long exact sequence, 
	\begin{multline*}
		0 \to H^{0}(M) \xrightarrow{i^{\ast} \oplus j^{\ast}} H^{0}(U) \oplus H^{0}(V) \xrightarrow{k^{\ast} - l^{\ast}} H^{0}(U \cap V) \xrightarrow{\delta} H^{1}(M) \xrightarrow{i^{\ast} \oplus j^{\ast}} H^{1}(U) \oplus H^{1}(V) \xrightarrow{k^{\ast} - l^{\ast}} H^{1}(U \cap V) \\
		\xrightarrow{\delta} H^{2}(M) \xrightarrow{i^{\ast} \oplus j^{\ast}} H^{2}(U) \oplus H^{2}(V) \xrightarrow{k^{\ast}- l^{\ast}}\dotsm \xrightarrow{\delta} H^{n}(M)\xrightarrow{i^{\ast} \oplus j^{\ast}} H^{n}(U) \oplus H^{n}(V) \xrightarrow{k^{\ast} - l^{\ast}} H^{n}(U \cap V) \xrightarrow{\delta} 0. 
	\end{multline*}
	By exactness and the rank-nullity theorem, one has that the alternating sum of the dimensions of each of the vector spaces in the long-exact sequence is equal to 0. Labeling each of the vector spaces in the above long-exact sequence by $V_{0}$ (in that order), this implies that 
	\begin{equation*}
		\sum_{k = 0}^{n}(-1)^{k}\dim{V_{k}} = 0. 
	\end{equation*}
	In particular, one has 
	\begin{align*}
		\begin{split}
		\sum_{k = 0}^{n}(-1)^{k}\dim{V_{k}} &= \sum_{k = 0}^{n}(-1)^{3k}\dim{H^{k}(M)} + \sum_{k = 0}^{n}(-1)^{3k + 1}\dim{H^{k}(U)} + \sum_{k = 0}^{n}(-1)^{3k + 1}\dim{H^{k}(V)} \\
		&\mathcolor{white}{=} + \sum_{k = 0}^{n}(-1)^{3k + 2}\dim{H^{k}}(U \cap V) \\
		&= \sum_{k = 0}^{n}(-1)^{k}\dim{H^{k}(M)} -\sum_{k = 0}^{n}(-1)^{k}\dim{H^{k}(U)} -\sum_{k = 0}^{n}(-1)^{k}\dim{H^{k}(V)} + \sum_{k = 0}^{n}(-1)^{k}\dim{H^{k}(U \cap V)} \\
		&= \chi(M) - \chi(U) - \chi(V) + \chi(U \cap V). 
		\end{split}
	\end{align*}
	Therefore, since the final (finite) quantity is equal to zero, we conclude that 
	\begin{equation*}
		\chi(M) = \chi(U) + \chi(V) - \chi(U \cap V). 
	\end{equation*}
	
	\begin{prb}{LB-2020-F-10}
		Let $M$ be a smooth manifold for which $\dim{H^{k}(M)}$ is finite-dimensional for each $k$. Notice that $M \times S^{n}$ can be written as the union of two open sets, each of which is diffeomorphic to $M \times \mathbb{R}^{n}$. Using this observation, prove by induction on $n$ that 
		\begin{equation*}
			\chi(M \times S^{n}) = 
			\begin{cases}
				2\chi(M) & \text{if $n$ is even} \\
				0 & \text{if $n$ is odd}. 
			\end{cases}
		\end{equation*}
		Then use this to calculate the Euler characteristic of any Cartesian product 
		\begin{equation*}	 
			S^{n_{1}} \times S^{n_{2}} \times \dotsm S^{n_{\ell}}
		\end{equation*}
		of a finite collection of spheres. 
	\end{prb}
	We begin by proving the claim that $M \times S^{n}$ can be written as the union of two open sets, each of which is diffeomorphic to $M \times \mathbb{R}^{n}$. Indeed, let $U = S^{1}\setminus \{N\}$ and $V = S^{1} \setminus \{S\}$, where $N$ and $S$ denote the north and south poles, respectively, be open subsets of $S^{n}$. Then it follows that $U$ and $V$ are both diffeomorphic to $\mathbb{R}^{n}$ under the stereographic projection map. Under the product topology, the subsets $M \times U$ and $M \times V$ are both open subsets of $M \times S^{n}$ that are diffeomorphic to $M\times \mathbb{R}^{n}$ under the product map $\operatorname{id}_{M} \times \sigma_{U,V}$, where $\sigma_{U, V}$ are the stereographic projection maps from $U$ and $V$, respectively. Now we will prove the claim regarding the Euler characteristics using induction on $n$. 
	
	\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
		\item Consider $n = 0$ (even). Then $S^{n} = S^{0} = \{\text{pt}, -\text{pt}\}$. This means that $M \times S^{0}$ is diffeomorphic to $M \sqcup M$. Therefore, for each $p \geq 0$, one has that 
		\begin{equation*}
			H^{p}_{\text{dR}}(M \times S^{0}) \cong H^{p}_{\text{dR}}(M \sqcup M) \cong H^{p}_{\text{dR}}(M) \oplus H^{p}_{\text{dR}}(M) \implies \dim{H^{p}_{\text{dR}}(M \times S^{0})} = 2 \cdot \dim{H^{p}_{\text{dR}}(M)}. 
		\end{equation*}
		Using the definition of the Euler characteristic, we obtain 
		\begin{equation*}
			\chi(M \times S^{0}) = \sum_{k = 0}^{\dim{M}}(-1)^{k}\dim{H^{k}_{\text{dR}}(M \times S^{0})} = 2 \cdot \sum_{k = 0}^{\dim{M}}(-1)^{k}\dim{H^{k}_{\text{dR}}(M)} = 2\chi(M). 
		\end{equation*}
		\item Now assume that we have computed $\chi(M \times S^{p})$ for all $0 \leq p \leq n$. We wish now to compute $\chi(M \times S^{n + 1})$. As the hint suggests, let $U$ and $V$ be open subsets of $M \times S^{n + 1}$ defined by $U = M \times S^{n + 1} \setminus \{N\}$ and $V = M \times S^{n + 1} \setminus \{S\}$. Since $U$ and $V$ are each diffeomorphic to $M \times \mathbb{R}^{n + 1}$, and $\mathbb{R}^{n + 1}$ is contractible (homotopy equivalent to a point), it follows that $\chi(U), \chi(V) = \chi(M)$. On the other hand, $U \cap V \simeq M \times S^{n + 1}\setminus \{N, S\}$. Since $S^{n + 1} \setminus \{N, S\}$ is homotopy equivalent to $S^{n}$, we conclude that $U \cap V$ is homotopy equivalent to $M \times S^{n}$ so that $\chi(U \cap V) = \chi(M \times S^{n})$. Thus by the previous problem, one has that 
		\begin{equation*}
			\chi(M \times S^{n + 1}) = 2\chi(M) - \chi(M \times S^{n}). 
		\end{equation*}
		From this, one sees that, e.g., 
		\begin{equation*}
			\chi(M \times S^{1}) = 2\chi(M) - \chi(M \times S^{0}) = 0. 
		\end{equation*}
		Thus this concludes the proof. 
	\end{enumerate}
	Now consider the Cartesian product $S^{n_{1}} \times S^{n_{2}} \times \dotsm \times S^{n_{\ell}}$. We note that 
	\begin{equation*}
		\chi(S^{m}) = 
		\begin{cases}
			2 &\text{$m$ even} \\
	...
	because 
	\begin{equation*}
		\chi(S^{m}) = (-1)^{0}\dim{H^{0}_{\text{dR}}(S^{m})} + (-1)^{m}\dim{H^{m}_{\text{dR}}(S^{m})} = \dim{\mathbb{R}} + (-1)^{m} \dim{\mathbb{R}} = 1 + (-1)^{m}. 
	\end{equation*}
	Suppose first that the dimension of one of the factor spheres in the Cartesian product is odd; i.e., assume that $n_{\ell}$ is odd (after possibly rearranging and relabeling the factors). Then our work above shows that $\chi((S^{n_{1}} \times \dotsm S^{n_{\ell - 1}}) \times S^{n_{\ll}}) = 0$. On the other hand, assume that every $n_{j}$ is even. Then our work above shows that 
	\begin{equation*}
		\chi((S^{n_{1}} \times \dotsm S^{n_{\ell - 1}}) \times S^{n_{\ell}}) = 2 \cdot \chi((S^{n_{1}} \times \dotsm S^{n_{\ell - 2}}) \times S^{n_{\ell - 1}}) = 4 \cdot \chi((S^{n_{1}} \times \dotsm \times S^{n_{\ell - 3}}) \times S^{n_{\ell - 1}}) = \dotsm = 2^{\ell - 1}\chi(S^{n_{1}}) = 2^{\ell}. 
	\end{equation*}
	Hence, we conclude that 
	\begin{equation*}
		\chi(S^{n_{1}} \times \dotsm \times S^{n_{\ell}}) = 
		\begin{cases}
			2^{\ell} & \text{if each $n_{1}, \ldots, n_{\ell}$ is even} \\
			0 & \text{otherwise}. 
		\end{cases}
	\end{equation*}
	\newpage 
	...
	\begin{prb}{2014-A-II-6}
		Let $X$ be a compact connected manifold and $M \subset X$ a compact connected embedded hypersurface (i.e., $\dim{M}= \dim{X} - 1$). Suppose that the normal bundle of $M$ in $X$ is not orientable. Prove that $\pi_{1}(X) \neq 0$. 
	\end{prb}
	Let $X$ be a compact connected manifold and $M \subset X$ a compact connected embedded hypersurface (i.e., $\dim{M} = \dim{X} - 1$). Assume that $\pi_{1}(X) =0$. This means that $X$ is orientable as it has no index 2 subgroup. Since $X$ is orientable, any open subset of $X$ must also be orientable. Since $\dim{M} = \dim{X} - 1$, the normal bundle of $M$ in $X$ is a line bundle. By the Tubular Neighborhood Theorem, it follows that the normal line bundle $N$ is diffeomorphic to a tubular neighborhood of $M$ in $X$, which must therefore be orientable. But this contradicts our hypothesis that $N$ is nonorientable. Thus by contradiction, $\pi_{1}(X) \neq 0$. 
	...
	\begin{prb}{LB-2020-F-7}
		Let $M$ be a smooth $m$-manifold, $m \geq 2$, and let $\omega \in \Omega^{m}(M)$ be a top-degree form that satisfies $\omega \neq 0$ at every point. Such a differential form $\omega$ is often called a ``volume form,'' because we can use it to assign an $m$-dimensional volume to an open set in $M$ via integration. We now fix a specific volume form $\omega$ for the remainder of this problem. 
		
		For any $(m-2)$-form $\varphi \in \Omega^{m-2}(M)$, show that the equation 
		\begin{equation*}
			V \lrcorner \omega = d\varphi 
		\end{equation*}
		defines a unique vector field $V \in \mathfrak{X}(M)$, and that this vector field is then ``volume preserving,'' in the sense that $\omega$ is invariant under its flow. What cohomological condition on $M$ is equivalent to the statement that every ``volume preserving'' vector field arises from some $\varphi$ via the above equation? 
	\end{prb}
	Let $\Phi: \mathfrak{X}(M) \to \Omega^{m-1}(M)$ be the map defined by $\Phi(X) = X \lrcorner \omega$. Because $\Phi$ is $C^\infty(M)$-linear, it is induced by a pointwise linear map $\Phi_p: T_p M \to \Lambda^{m-1}(T_p^* M)$ at each point $p \in M$.
	
	The vector spaces $T_p M$ and $\Lambda^{m-1}(T_p^* M)$ have equal finite dimension:
	
	
	$$\dim T_p M = m = \binom{m}{m-1} = \dim \Lambda^{m-1}(T_p^* M)$$
	
	To determine the kernel of $\Phi_p$, choose local coordinates $(x^1, \dots, x^m)$ centered at $p$ so that $\omega_p = c\, dx^1 \wedge \dots \wedge dx^m$ with $c \neq 0$. For $v = \sum_{i=1}^m v^i \frac{\partial}{\partial x^i} \in \ker \Phi_p$:
	
	
	$$\Phi_p(v) = c \sum_{i=1}^m (-1)^{i-1} v^i \, dx^1 \wedge \dots \wedge \widehat{dx^i} \wedge \dots \wedge dx^m = 0$$
	
	Because the set $\{dx^1 \wedge \dots \wedge \widehat{dx^i} \wedge \dots \wedge dx^m\}_{i=1}^m$ forms a basis of $\Lambda^{m-1}(T_p^* M)$ and $c \neq 0$, each coefficient satisfies $v^i = 0$, meaning $\ker \Phi_p = \{0\}$. By the rank-nullity theorem, $\Phi_p$ is a linear isomorphism for every $p \in M$, making $\Phi$ a smooth bundle isomorphism. For any $\varphi \in \Omega^{m-2}(M)$, the exterior derivative $d\varphi \in \Omega^{m-1}(M)$ is smooth, so $V = \Phi^{-1}(d\varphi)$ is the unique smooth vector field satisfying $V \lrcorner \omega = d\varphi$. To show that $V$ is volume-preserving, compute the Lie derivative $\mathcal{L}_V \omega$ using Cartan's magic formula:
	
	
	$$\mathcal{L}_V \omega = d(V \lrcorner \omega) + V \lrcorner d\omega$$
	
	Because $\omega$ is an $m$-form on an $m$-dimensional manifold, its exterior derivative $d\omega \in \Omega^{m+1}(M)$ is identically zero. Substituting $V \lrcorner \omega = d\varphi$:
	
	
	$$\mathcal{L}_V \omega = d(d\varphi) + V \lrcorner 0 = 0$$
	
	since $d^2 = 0$. Therefore, $\mathcal{L}_V \omega = 0$, proving $V$ is volume-preserving. The equivalent cohomological condition is $H^{m-1}_{\text{dR}}(M) = 0$. Suppose every volume-preserving vector field arises as $X \lrcorner \omega = d\varphi$ for some $\varphi \in \Omega^{m-2}(M)$. Let $\alpha \in \Omega^{m-1}(M)$ be an arbitrary closed form ($d\alpha = 0$). Since $\Phi$ is bijective, there exists a unique vector field $X \in \mathfrak{X}(M)$ such that $X \lrcorner \omega = \alpha$. Evaluating its Lie derivative gives:
	
	
	$$\mathcal{L}_X \omega = d(X \lrcorner \omega) + X \lrcorner d\omega = d\alpha + 0 = 0$$
	
	Thus $X$ is volume-preserving. By hypothesis, $X \lrcorner \omega = d\varphi$ for some $\varphi \in \Omega^{m-2}(M)$, which yields $\alpha = d\varphi$. Hence every closed $(m-1)$-form is exact, so $H^{m-1}_{\text{dR}}(M) = 0$.
	
	Conversely, suppose $H^{m-1}_{\text{dR}}(M) = 0$. Let $X \in \mathfrak{X}(M)$ be any volume-preserving vector field ($\mathcal{L}_X \omega = 0$). Applying Cartan's formula:
	
	
	$$0 = \mathcal{L}_X \omega = d(X \lrcorner \omega) + X \lrcorner d\omega = d(X \lrcorner \omega)$$
	
	This implies $X \lrcorner \omega$ is a closed $(m-1)$-form. Because $H^{m-1}_{\text{dR}}(M) = 0$, every closed $(m-1)$-form is exact, so there exists $\varphi \in \Omega^{m-2}(M)$ such that $X \lrcorner \omega = d\varphi$.
	...
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		...
		More simply, if $p: X \to Y$ is a covering map, then by definition, $p$ is continuous and surjective. Therefore, $Y$ must be compact. But since $Y$ is an open and nonclosed subset of $T^2$, and compact subsets of Hausdorff spaces must necessarily be closed, we have arrived at a contradiction. 
	...
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		...
		Alternatively, assume to the contrary that $p: X \to \Sigma_{1}$ is a covering space such that $X \simeq \Sigma_{2}$. In particular, since homeomorphisms induce isomorphisms on the level of fundamental groups, $\pi_{1}(X) \cong \pi_{1}(\Sigma_{2}) \cong \braket{a_{1}, b_{1}, a_{2}, b_{2} \mid [a_{1}, b_{1}][a_{2}, b_{2}] = 1}$, which is nonabelian. On the other hand, we know that covering maps induce injective maps on the level of fundamental groups so that $\pi_{1}(X)$ is isomorphic to a subgroup of $\pi_{1}(\Sigma_{1}) \cong \mathbb{Z} \times \mathbb{Z}$. This is a contradiction since the latter group is abelian, while the former is nonabelian. Thus there cannot be such a covering space. 
	...
	\begin{prb}{2015-A-II-2}
		Let $M$ be a connected oriented smooth compact $n$-dimensional manifold with boundary, and suppose that $\partial M \neq \varnothing$. Let $\Phi: M \to M$ be a smooth map whose restriction to $\partial M$ is the identity. If $\omega$ is any smooth $n$-form on $M$, prove that 
		\begin{equation*}
			\int_{M}\Phi^{\ast}\omega = \int_{M}\omega. 
		\end{equation*}
		Then use this to prove that $\Phi$ must be surjective. 
	\end{prb}
	Let $M$ be a connected oriented smooth compact $n$-dimensional manifold with boundary $\partial M$, and let $\Phi: M \to M$ be a smooth map whose restriction to $\partial M$ is the identity. First, one has that $H^{n}_{\text{dR}}(M)= 0$. Let $\omega$ be a smooth $n$-form on $M$ so that $\omega$ is exact; let $\beta \in \Omega^{n - 1}(M)$ such that $\omega = d\beta$. Then Stokes' Theorem implies 
	\begin{align*}
		\begin{split}
			\int_{M}\Phi^{\ast}\omega &= \int_{M}\Phi^{\ast}d\beta = \int_{M}d\Phi^{\ast}\beta = \int_{\partial M}\Phi^{\ast}\beta \\
			&= \int_{\partial M}\beta = \int_{M}d\beta = \int_{M}\omega. 
		\end{split}
	\end{align*}
	Since $\omega$ was arbitrary, the proof of the first part of the problem concludes. Now assume to the contrary that $\Phi$ is not surjective. Since $M$ is compact, $\Phi$ is proper so that $\Phi(M)$ is closed in $M$. Therefore, $M\setminus \Phi(M)$ is open. Let $q \in M\setminus \Phi(M)$, and let $\ti{U}$ be an open neighborhood of $q$ in $M \setminus \Phi(M)$. Let $\omega$ be an $n$-form supported in $U$ so that $\int_{M} \omega = 1$. Since $\Phi^{\ast}\omega = 0$, one has that 
	\begin{equation*}
		0 = \int_{M}\Phi^{\ast}\omega = \int_{M}\omega = 1, 
	\end{equation*}
	which is a contradiction. Therefore, we conclude that $\Phi(M) = M$ so that $\Phi$ is surjective. 
	...
	\begin{prb}{2012-J-II-3}
		...
		\end{equation*}
		on fundamental groups induced by $f$. Suppose that the image of $f_{\#}$ has finite index, call it $\operatorname{ind}(f)$. Prove that $\operatorname{ind}(f)$ divides $\deg(f)$. 
	\end{prb}
	Let $M$ and $N$ be compact connected orientable smooth manifolds, and let $f: M \to N$ be a smooth mapping so that the image of the induced map $f_{\#}: \pi_{1}(M) \to\pi_{1}(N)$ has finite index, $\operatorname{ind}(f)$. Let $H \coloneqq f_{\#}(\pi_{1}(M))$. By the Galois correspondence of covering spaces, $H$ corresponds to a connected covering space $p: N_{H} \to N$ of degree $\operatorname{ind}(f)$. Since $H \subseteq p_{\ast}(N_{H})$, $f$ lifts to a smooth map $\ti{f}: M \to N_{H}$ so that $f = p \circ \ti{f}$. In particular, using the definition of the degree of a smooth map, one has that $\deg(f) = \deg(p) \cdot \deg(\ti{f})$. Thus it follows that $\operatorname{ind}(f) = \deg(p)$ divides $\deg(f)$ so that the proof concludes. 
	...
	\begin{prb}{AZ-2011-F-1}
		Let $M$ and $N$ be compact oriented connected $n$-manifolds and $f: M \to N$ a smooth map. Show that there exists a unique number $(\deg{f}) \in \mathbb{R}$ such that 
		\begin{equation*}
			\int_{M}f^{\ast}\omega = (\deg{f}) \cdot \int_{N}\omega \qquad \forall \omega \in \Omega^{n}(N). 
		\end{equation*}
	\textit{Note}: there are two parts to this problem. 
	\end{prb}
	Let $M$ and $N$ be compact oriented connected $n$-manifolds and $f: M \to N$ be a smooth map. The two parts to this problem are to show (1) that such a number exists, and (2) to show this number is unique. We begin with a proof of uniqueness. 
	
	Suppose $k_1, k_2 \in \mathbb{R}$ both satisfy $\int_M f^*\omega = k \int_N \omega$ for all $\omega \in \Omega^n(N)$. Choose any $n$-form $\omega_0 \in \Omega^n(N)$ with $\int_N \omega_0 \neq 0$ (which exists because $H^n_{\text{dR}}(N) \cong \mathbb{R}$). Then:$$k_1 \int_N \omega_0 = \int_M f^*\omega_0 = k_2 \int_N \omega_0 \implies (k_1 - k_2) \int_N \omega_0 = 0.$$ Since $\int_N \omega_0 \neq 0$, it follows that $k_1 = k_2$. Now we will show existence. Since $N$ is compact connected and oriented, $H^{n}_{\text{dR}}(N)$ is one-dimensional, and hence is generated by the cohomology class of a nowhere vanishing $n$-form, $[\omega_{N}]$ whose integral over $N$ is nowhere vanishing. Thus for any $\omega \in \Omega^{n}(N)$, $[\omega] = c \cdot [\omega_{N}] = [c\omega_{N}]$. This means that $\omega$ and $c\omega_{N}$ are cohomologous; i.e., there exists an $n-1$ form $\eta$ on $N$ such that $\omega - c\omega_{N} = d\eta$. Since $N$ is a compact manifold without boundary $\int_{N}d\eta = 0$ so that $\int_{N}\omega = c\int_{N}\omega_{N}$. Likewise, it follows that $[f^{\ast}\omega]$ and $[cf^{\ast}\omega_{N}]$ are cohomologous $n$-forms on $M$ such that $\int_{M}f^{\ast}\omega = c\cdot \int_{M}f^{\ast}\omega_{N}$. Define 
	\begin{equation*}
		\deg{f} \coloneqq \frac{\int_{M}f^{\ast}\omega_{N}}{\int_{N}\omega_{N}}. 
	\end{equation*}
	From this, one concludes that 
	\begin{equation*}
		\int_{M}f^{\ast}\omega = c\int_{M}f^{\ast}\omega_{N} = c \cdot \deg{f} \cdot \int_{N}\omega_{N} = \deg{f} \cdot \int_{N}\omega. 
	\end{equation*}
	Since $\omega \in \Omega^{n}(N)$ was arbitrary, the proof concludes. 
	...
	\end{prb}
	Let $K$ be an arbitrary field, and consider the polynomial ring $K[x,y]$ in two variables. It suffices to show that there exists an ideal of $K[x,y]$ that is not principal. Consider the ideal $(x, y)$. First, we claim that $(x,y)$ is proper. Every polynomial in $(x,y)$ is of the form $a(x,y)\cdot x + b(x,y) \cdot y$. In particular, at the origin $(x,y) = (0,0)$, one has that every polynomial in the ideal vanishes. But this means that the constant polynomial $1$ cannot be a part of $(x,y)$. Now assume to the contrary that $(x, y)$ is principal: $(c) = (x, y)$. Since $c$ must be a divisor of both $x$ and $y$, and $x, y$ are coprime, $c$ must be a unit. But this implies that $(c) = K[x,y]$ so that $(x,y)$ is the whole polynomial ring; we explicitly showed that this was not the case. Thus by contradiction, it follows that $(x,y)$ is not principle, and so $K[x,y]$ is not a principal ideal domain. 
...
f2		\begin{equation*}
	...
	\end{prb}
	Let $f(x)$ be an irreducible polynomial of degree $n$ over a field $F$, and let $g(x) \in F[x]$. Let $h(x)$ be an irreducible factor of $f(g(x)) \in F[x]$. Let $\alpha$ be a root of $h(x)$, which means that $f(g(\alpha)) = 0$; i.e., $\beta \coloneqq g(\alpha)$ is a root of $f$. This implies $[F(\beta):F] = n$ since $\deg{f} = n$. But since $\beta = g(\alpha)$, one has $F \subset F(\beta)\subset F(\alpha)$. Therefore, $[F(\beta):F]$ divides $[F(\alpha):F] = \deg{h}$, whence $n$ divides $\deg{h}$. 
	...
	\end{prb}
	Let $G$ be a finite group, and $P \subset G$ a normal Sylow $p$-subgroup. This necessarily implies that $P$ is the unique Sylow $p$-subgroup of $G$. Since $\varphi$ is a group homomorphism, the first isomorphism theorem implies that $|\varphi(P)|$ divides $|P|$ so that $\varphi(P)$ must be a $p$-subgroup of $G$. By maximality of Sylow $p$-subgroups, $\varphi(P)$ must be contained in a Sylow $p$-subgroup of $G$. But since $P$ is the only Sylow $p$-subgroup of $G$, $\varphi(P) \subset P$. Thus the proof concludes. 
	...
	\end{prb}
	Let $P$ be a Sylow $p$-subgroup of $G$, and let $H$ be a $p$-subgroup of $G$ contained in $N(P)$. Since $N(P)$ is the normalizer of $P$, $P$ is a normal Sylow $p$-subgroup of $N(P)$, and thus is the unique Sylow $p$-subgroup of $N(P)$. Since $H$ is a $p$-subgroup of $G$ and thus a $p$-subgroup of $N(P)$, maximality of Sylow $p$-subgroups implies that $H$ is contained in a Sylow $p$-subgroup of $N(P)$, and thus in $P$.
	...
	Thus the proof concludes. 
		...
		We begin by assuming that both $E$ and $F$ have finite measure. So define the function 
			...
			\begin{equation*}
				\int_{F}\chi_{E}(x - t)\;dt = \int_{\mathbb{R}}\chi_{E}(x - t)\chi_{F}(t)\;dt > 0, 
			\end{equation*}
	...
	\begin{align*}
		\begin{split}
			...
			&\leq \int_{0}^{1}\left(\int_{0}^{x}\;dt\right)\left(\int_{0}^{x}|f'(t)|^2\;dt\right)\;dx \qquad \text{(by Cauchy-Schwarz)} \\
	...
	\begin{align*}
		\begin{split}
			...
			&\leq \left[\int_{0}^{1}|f(x)|^2\;dx\right]^{1/2} + \left[\int_{0}^{1}|f'(x)|^2\;dx\right]^{1/2} \qquad (\text{by the Cauchy-Schwarz Inequality}), 
		\end{split}
	\end{align*}
	...
	\begin{prb}{F-7-22}
		A sequence $\{f_{n}\}$ in $C_{0}(X)$ converges weakly to $f \in C_{0}(X)$ iff $\sup\norm{f_{n}}_{u} < \infty$ and $f_{n} \to f$ pointwise.  
	\end{prb}
	Let $\{f_{n}\}$ be a sequence in $C_{0}(X)$. 
	
	($\Longleftarrow$) Assume that $\sup\norm{f_{n}}_{u} < \infty$ and $f_{n} \to f$ pointwise. Fix some arbitrary $\mu \in M(X)$, the space of complex Radon measures on $X$ with finite total variation. One notices that for every $n \geq 1$ and every $x \in X$, 
	\begin{equation*}
		|f_{n}(x)| \leq \sup\norm{f_{n}}_{u} \cdot \chi_{X} \in L^{1}(X), 
	\end{equation*}
	where the statement on the right side of the inequality follows from the fact that since $\mu$ is a finite measure, 
	\begin{equation*}
		\int \sup\norm{f_{n}}_{u}\;d\mu = \mu(X) \cdot \sup\norm{f_{n}}_{u} < \infty. 
	\end{equation*}
	Therefore, by the Dominated Convergence Theorem, one has that $\int f_{n}\;d\mu \to \int f\;d\mu$. Since $\mu \in M(X)$ was arbitrary, we conclude that $f_{n} \to f$ weakly in $C_{0}(X)$. 
	
	($\Longrightarrow$) Now assume that $f_{n} \to f \in C_{0}(X)$ weakly. This means that for every $\mu \in M(X)$, $\int f_{n}\;d\mu\to \int f\;d\mu$. Since the Dirac measure $\delta_{x}$ is an element of $M(X)$, we have $\int f_{n}\;d\delta_{x} \to \int f\;d\delta_{x} \iff f_{n}(x) \to f(x)$ for every $x \in X$. Thus $f_{n} \to f$ pointwise. To prove uniform boundedness, we view each $f_{n}$ as a linear functional $\hat{f}: M(x) \to \mathbb{C}$ defined by 
	\begin{equation*}
		\hat{f}_{n}(\mu) = \int_{X}f_{n}\;d\mu. 
	\end{equation*}
	Since $f_{n} \to f$ weakly, it follows that for each $\mu \in M(X)$, $\{\hat{f}_{n}(\mu)\}_{n = 1}^{\infty}$ is convergent in $\mathbb{C}$, and hence bounded; i.e., one has that $\sup |\hat{f}_{n}(\mu)|< \infty$ for every $\mu \in C_{0}(X)$. The Uniform Boundedness Principle then implies that $\sup \norm{\hat{f}_{n}}_{\text{op}}< \infty$, where $\norm{\empspace}_{\text{op}}$ denotes the operator norm. Now recall that $C_{0}(X) \hookrightarrow C_{0}(X)^{\ast\ast} \cong M(X)^{\ast}$ isometrically so that $\norm{\empspace}_{\text{op}} = \norm{\empspace}_{u}$. Therefore, $\norm{\hat{f}_{n}}_{\text{op}} = \norm{f_{n}}_{u}$ for every $n$ so that $\sup\norm{f_{n}}_{u} < \infty$. 
	
	\begin{prb}{F-8-4}
		If $f \in L^{\infty}$ and $\norm{\tau_{y}f- f}_{L^{\infty}} \to 0$ as $y \to 0$, then $f$ agrees with a uniformly continuous function. (Let $A_{r}f$ be as in Theorem 3.18. Then $A_{r}f$ is uniformly continuous for $r > 0$ and uniformly Cauchy as $r \to 0$.)
	\end{prb}
	Let $f \in L^{\infty}$ such that $\norm{\tau_{y}f - f}_{L^{\infty}} \to 0$ as $y\to 0$. Quickly note that the condition $\norm{\tau_{y}f - f}_{L^{\infty}} \to 0$ as $y \to 0$ does \textit{not} imply $f$ is uniformly continuous as the definition for uniform continuity requires the uniform norm $\norm{\empspace}_{u} \coloneqq \sup_{x}|f(x)|$, and not the $L^{\infty}$ norm. Since $L^{\infty}$ functions are $L^{1}_{\text{loc}}$, for $x \in \mathbb{R}^{n}$ and $r > 0$, we define $A_{r}f(x)$ to be the average value of $f$ in $B(r, x)$: 
	\begin{equation*}
		A_{r}f(x) = \frac{1}{m(B(r,x))} \int_{B(r,x)}f(y)\;dy. 
	\end{equation*}
	By Theorem 3.18, it follows that $\lim_{r \to 0}A_{r}f(x) = f(x)$ for a.e. $x \in \mathbb{R}^{n}$. Thus it suffices to show that (1) $A_{r}f$ is uniformly continuous in $x$ and (2) uniformly Cauchy as $r \to 0$ because (2) implies that $A_{r}f(x)$ is uniformly convergent as $r \to 0$ and since the uniform limit of uniformly continuous functions is uniformly continuous, coupled with (1), we must have that $\lim_{r \to 0}A_{r}f(x)$ is uniformly continuous. \vspace{-0.25cm}
	\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
		\item To show $A_{r}f(x)$ is uniformly continuous in $x$ for $r > 0$, we must show that as $y \to 0$, $\norm{\tau_{y}A_{r}f - A_{r}f}_{u} \to 0$. For any fixed $x \in \mathbb{R}^{n}$, one has that 
		\begin{align*}
			\begin{split}
				|\tau_{y}A_{r}f(x) - A_{r}f(x)| &= \abs{\frac{1}{m(B(r,x - y))}\int_{B(r,x - y)}f(u)\;du - \frac{1}{m(B(r,x))}\int_{B(r,x)}f(u)\;du} \\
				&= \frac{1}{m(B(r, x))}\abs{\int_{B(r, x)}\left[\tau_{y}f(u) - f(u)\right]\;du} \leq \frac{1}{m(B(r,x))}\int_{B(r,x)}|\tau_{y}f(u)- f(u)|\;du\\
				&\leq \frac{1}{m(B(r,x))}\int_{B(r, x)}\norm{\tau_{y}f - f}_{L^{\infty}}\;du = \norm{\tau_{y}f - f}_{L^{\infty}} \xrightarrow{y \to 0} 0. 
			\end{split}
		\end{align*}
		Thus, this shows that $A_{r}f$ is uniformly continuous in $x$. 
		\item Now we will show that $A_{r}f$ is uniformly Cauchy as $r \to 0$; doing so requires us to show that $\norm{A_{r}f(x) - A_{r'}f(x)}_{u} \to 0$ as $r, r' \to 0$. We will first show that as $r \to 0$, $\norm{A_{r}f - f}_{u} \to 0$ as $r \to 0$. For any fixed $x \in \mathbb{R}^{n}$, one can write 
		\begin{align*}
			\begin{split}
				A_{r}f(x) &= \frac{1}{m(B(r,x))}\int_{B(r,x)}f(u)\;du = \frac{1}{m(B(r,0))}\int_{B(r,0)}f(x + y)\;dy = \frac{1}{m(B(r,0))}\int_{B(r,0)}\tau_{-y}f(x)\;dy,\\
				f(x) &= \frac{1}{m(B(r,0))}\int_{B(r,0)}f(x)\;dy.
			\end{split}
		\end{align*}
		This implies that 
		\begin{align*}
			\begin{split}
				|A_{r}f(x) - f(x)| &\leq \frac{1}{m(B(r,0))}\int_{B(r,0)}|\tau_{-y}f(x) - f(x)|\;dy \\
				&\leq \frac{1}{m(B(r,0))}\int_{B(r,0)}\norm{\tau_{-y}f - f}_{L^{\infty}}\;dy \leq \sup_{|y|\leq r}\norm{\tau_{y}f - f}_{L^{\infty}} \xrightarrow{r \to 0} 0, 
			\end{split}
		\end{align*}
		by our hypothesis on $f$. Thus, for any $0 < r' < r$, one has that 
		\begin{equation*}
			|A_{r}f(x)- A_{r'}f(x)| \leq |A_{r}f(x) - f(x)| + |A_{r'}f(x) - f(x)| \xrightarrow{r, r' \to 0} 0
		\end{equation*}
		for all $x \in \mathbb{R}^{n}$ so that $A_{r}f$ is uniformly Cauchy (hence uniformly convergent) as $r \to 0$. 
	\end{enumerate}
	Therefore, $\lim_{r \to 0}A_{r}f(x)$ is the uniform limit of uniformly continuous functions, and thus is uniformly continuous itself. Since $f(x)$ agrees with this limit a.e., we conclude that $f(x)$ agrees with a uniformly continuous function a.e. 
	...
	\begin{align*}
		\begin{split}
			|(f \ast g)(x + h) - (f \ast g)(x)| &= \abs{\int_{\mathbb{R}}\left[f(x + h - y) - f(x - y)\right]g(y)\;dy} \\
			&\leq \left(\int_{\mathbb{R}}|f(x + h - y) - f(x - y)|^{p}\;dy\right)^{1/p} \cdot \norm{g}_{L^q}. 
		\end{split}
	\end{align*}
	Now we compute, 
	\begin{align*}
		\begin{split}
			\left(\int_{\mathbb{R}}|f(x + h - y) - f(x - y)|^{p}\;dy\right)^{1/p} &= \left(\int_{\mathbb{R}}|f(t + h) - f(t)|^p\;dt\right)^{1/p} \\
			&= \norm{\tau_hf - f}_{L^p}. 
		\end{split}
	\end{align*}
	...
	\end{prb} 
	Let $f \in L^1(\mathbb{R}) \cap L^{2}(\mathbb{R})$, and assume that $\hat{f} > 0$ a.e. on $\mathbb{R}$. Let $S = \{x \mapsto f(x - y)\}$. We recall that in a Hilbert space, a nonempty subspace $M$ is dense in the space iff its orthocomplement, $M^{\perp}$ consists of only the zero vector. Thus we wish to show that $S^{\perp}$ is $\{0\}$. Suppose $g \in S^{\perp}$. This means that 
	\begin{equation*}
	...
	\begin{align*}
		\begin{split}
			...
			&= \frac{1}{\pi^{1/2}r} \cdot \norm{f}_{L^{2}(D)} < \frac{K}{1 - |z|},  
		\end{split}
	\end{align*}
	where we define $K > 2\pi^{-1/2} \cdot \norm{f}_{L^{2}(D)} > 0$. 
	...
	Alternatively, one could also use the Hahn-Banach theorem. Indeed, by the Hahn-Banach Theorem, $S \cap L^{4}(\mathbb{R})$ is dense in $L^{4}(\mathbb{R})$ if and only if for every continuous linear functional $\chi \in (L^{4}(\mathbb{R}))^{\ast} \cong L^{4/3}(\mathbb{R})$ such that $\int_{\mathbb{R}}f(x)\chi(x)\;dx$ for every $f \in S \cap L^{4}(\mathbb{R})$, $\chi(x) \equiv 0$. So let $\chi$ be such a continuous linear functional. Let $\phi \in C_{c}^{\infty}(\mathbb{R}) \subset L^{1}(\mathbb{R}) \cap L^{4}(\mathbb{R})$, and $g \in C_{c}^{\infty}(\mathbb{R})$ such that $\int_{\mathbb{R}}g(x)\;dx = 1$. Then define 
	\begin{equation*}
		f(x) = \phi(x) - \left(\int_{\mathbb{R}}\phi(y)\;dy\right)g(x). 
	\end{equation*}
	One notices that $\int_{\mathbb{R}}f(x)\;dx = 0$, $f$ is continuous, and $f \in L^{1}(\mathbb{R}) \cap L^{4}(\mathbb{R})$. Thus $f \in S$. Now one computes, 
	\begin{equation*}
		\int_{\mathbb{R}}f(x)\chi(x)\;dx = 0 \iff \int_{\mathbb{R}}\phi(x)\chi(x)\;dx = c \cdot \int_{\mathbb{R}}\phi(y)\;dy \equiv c \cdot \int_{\mathbb{R}}\phi(x)\;dx, 
	\end{equation*}
	where 
	\begin{equation*}
		c \coloneqq \int_{\mathbb{R}}\chi(x)g(x)\;dx < \infty 
	\end{equation*}
	by H\"older's inequality. Thus, one has that $\int_{\mathbb{R}}\phi(x)(\chi(x) - c)\;dx = 0$. Since $\phi(x) \neq 0$ and the integrand is continuous, it follows that $\chi(x) \equiv c$ a.e. But since $\chi \in L^{4/3}(\mathbb{R})$, one must have $c = 0$. Thus since $\chi \equiv 0$ a.e., and $\chi$ was chosen arbitrarily, it follows by the Hahn-Banach Theorem that $S \cap L^{4}(\mathbb{R})$ is dense in $L^{4}(\mathbb{R})$. 
	...
	\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
		...
		Alternatively, one could also use the Hahn-Banach theorem. By the Hahn-Banach Theorem, $S \cap L^{2}(\mathbb{R})$ is dense in $L^{2}(\mathbb{R})$ if and only if for every continuous linear functional $\chi \in (L^{2}(\mathbb{R}))^{\ast} \cong L^{2}(\mathbb{R})$ such that $\int_{\mathbb{R}}f(x)\chi(x)\;dx$ for every $f \in S \cap L^{2}(\mathbb{R})$, $\chi(x) \equiv 0$. So let $\chi$ be such a continuous linear functional. Let $\phi \in C_{c}^{\infty}(\mathbb{R}) \subset L^{1}(\mathbb{R}) \cap L^{2}(\mathbb{R})$, and $g \in C_{c}^{\infty}(\mathbb{R})$ such that $\int_{\mathbb{R}}g(x)\;dx = 1$. Then define 
		\begin{equation*}
			f(x) = \phi(x) - \left(\int_{\mathbb{R}}\phi(y)\;dy\right)g(x). 
		\end{equation*}
		One notices that $\int_{\mathbb{R}}f(x)\;dx = 0$, $f$ is continuous, and $f \in L^{1}(\mathbb{R}) \cap L^{2}(\mathbb{R})$. Thus $f \in S$. Now one computes, 
		\begin{equation*}
			\int_{\mathbb{R}}f(x)\chi(x)\;dx = 0 \iff \int_{\mathbb{R}}\phi(x)\chi(x)\;dx = c \cdot \int_{\mathbb{R}}\phi(y)\;dy \equiv c \cdot \int_{\mathbb{R}}\phi(x)\;dx, 
		\end{equation*}
		where 
		\begin{equation*}
			c = \int_{\mathbb{R}}\chi(x)g(x)\;dx < \infty 
		\end{equation*}
		by Hoelder's inequality. Thus, one has that $\int_{\mathbb{R}}\phi(x)(\chi(x) - c)\;dx = 0$. Since $\phi(x) \neq 0$ and the integrand is continuous, it follows that $\chi(x) \equiv c$ a.e. But since $\chi \in L^{2}(\mathbb{R})$, one must have $c = 0$. Thus since $\chi \equiv 0$ a.e., and $\chi$ was chosen arbitrarily, it follows by the Hahn-Banach Theorem that $S \cap L^{2}(\mathbb{R})$ is dense in $L^{2}(\mathbb{R})$. 
	\end{enumerate}
	...
	\begin{prb}{F-2-33}
		If $f_{n} \geq 0$ and $f_{n} \to f$ in measure, then $\int f \leq \liminf\int f_{n}$. 
	\end{prb}
	Let $f_{n} \geq 0$ and $f_{n} \to f$ in measure. Let $\{f_{n_{j}}\}_{j = 1}^{\infty}$ be an arbitrary subsequence. Then since $f_{n_{j}} \to f$ in measure, there exists a subsubsequence $\{f_{n_{j_{k}}}\}$ that converges to $f$ pointwise a.e. Then Fatou's lemma implies that 
	\begin{equation*}
		\int f \leq \liminf \int f_{n_{j_{k}}}. 
	\end{equation*}
	Thus since for any arbitrary subsequence of $\{f_{n}\}$, there exists a subsubsequence $\{f_{n_{j_{k}}}\}$ satisfying $\int f \leq \liminf \int f_{n_{j_{k}}}$, we conclude that $\int f \leq \liminf \int f_{n}$. 
	
	\begin{prb}{F-2-34}
		Suppose $|f_{n}| \leq g \in L^{1}$  and $f_{n} \to f$ in measure. \vspace{-0.25cm} 
		\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
			\item $\int f = \lim \int f_{n}$. 
			\item $f_{n} \to f$ in $L^{1}$. 
		\end{enumerate}
	\end{prb}
	Suppose that for all $n \geq 1$, $|f_{n}| \leq g \in L^{1}$ and that $f_{n} \to f$ in measure. 
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item Consider the sequence $\{\int f_{n}\}_{n = 1}^{\infty}$, and let $\{\int f_{n_{j}}\}_{j = 1}^{\infty}$ be a subsequence. Since $f_{n} \to f$ in measure, $f_{n_{j}} \to f$ in measure, and thus there exists a subsubsequence $\{f_{n_{j_{k}}}\}_{k = 1}^{\infty}$ that converges to $f$ pointwise a.e. But for every $k \geq 1$, $|f_{n_{j_{k}}}| \leq g \in L^{1}$. Thus by the Dominated Convergence Theorem, one has that $\int f_{n_{j_{k}}} \xrightarrow{k \to \infty} \int f$. Since we showed that any arbitrary subsequence of $\{\int f_{n}\}$ has a subsubsequence that converges to $\int f$, we conclude that $\int f_{n}$ converges to $\int f$; i.e., $\int f = \lim \int f_{n}$. 
		\item Now consider the sequence $\{\norm{f_{n} - f}_{L^{1}}\}_{n = 1}^{\infty}$, and consider an arbitrary subsequence $\{\norm{f_{n_{j}} - f}_{L^{1}}\}_{j = 1}^{\infty}$. Then since $f_{n} \to f$ in measure, $f_{n_{j}} \to f$ in measure so that there exists a subsubsequence $\{f_{n_{j_{k}}}\}$ that converges to $f$ pointwise a.e.; this implies $|f_{n_{j_{k}}} - f|$ converges to 0 pointwise a.e. Since $|f_{n_{j_{k}}} - f| \leq 2(|g| + |f|) \in L^{1}$, we have by the dominated convergence theorem that 
		\begin{equation*}
			\norm{f_{n_{j_{k}}} - f}_{L^{1}} = \int |f_{n_{j_{k}}} - f| \xrightarrow{k \to \infty} \int 0 = 0. 
		\end{equation*}
		Thus since we have shown that every subsequence of $\{\norm{f_{n} - f}_{L^{1}}\}$ has a convergent subsubsequence that converges to $0$, we conclude that $f_{n} \to f$ in $L^{1}$. 
	\end{enumerate}
	
	\begin{prb}{F-2-36}
		If $\mu(E_{n}) < \infty$ for all $n \in \mathbb{N}$ and $\chi_{E_{n}} \to f$ in $L^{1}$, then $f$ is (a.e. equal to) the characteristic function of a measurable set. 
	\end{prb}
	Let $\mu(E_{n}) < \infty$ for all $n \in \mathbb{N}$ and assume that $\chi_{E_{n}} \to f$ in $L^{1}$. This implies there exists a subsequence $\{\chi_{E_{n_{j}}}\}$ that converges to $f$ pointwise a.e. I.e., there exists a set $N$ of measure $0$ such that for all $x \in X\setminus N$, $\chi_{E_{n_{j}}} \to f$. Since $\chi_{E_{n_{j}}}(x)$ is either 0 or 1, it follows that any pointwise limit of these functions must also either be $0$ or $1$ a.e. That is, $f(x) \in \{0,1\}$ for a.e. $x \in X$. Let $E = \{x \in X: f(x) = 1\}$. Since $f$ is the pointwise a.e. limit of measurable functions, $f$ is measurable so that $E$ is also measurable. Thus $f = \chi_{E}$. 
	
	\begin{prb}{F-2-38}
		Suppose $f_{n} \to f$ in measure and $g_{n} \to g$ in measure. \vspace{-0.25cm} 
		\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
			\item $f_{n} + g_{n} \to f + g$ in measure. 
			\item $f_{n}g_{n} \to fg$ in measure if $\mu(X) < \infty$, but not necessarily if $\mu(X) = \infty$. 
		\end{enumerate}
	\end{prb}
	\begin{enumerate}[itemsep =-2pt, label = (\alph{*})]
		\item Since $f_{n} \to f$ in measure and $g_{n} \to g$ in measure, for every $\epsilon > 0$, 
		\begin{align}
			\begin{split}
				&\mu\left(\left\{x: |f_{n}(x) - f(x)|\geq \frac{\epsilon}{2}\right\}\right) \to 0 \text{ as } n \to \infty;  \\
				&\mu\left(\left\{x: |g_{n}(x) - g(x)|\geq \frac{\epsilon}{2}\right\}\right) \to 0 \text{ as } n \to \infty. 
			\end{split}
		\end{align}
		Therefore, since 
		\begin{equation}
			\left\{x: |f_{n}(x) + g_{n}(x) - f(x) - g(x)| \geq \epsilon \right\} \subseteq \left\{x: |f_{n}(x) - f(x)|\geq \frac{\epsilon}{2}\right\} \cup \left\{x: |g_{n}(x) - g(x)|\geq \frac{\epsilon}{2}\right\}
		\end{equation}
		it follows from monotonicity and subadditivity of the measure that 
		\begin{multline}
			0 \leq \mu\left(\left\{x: |f_{n}(x) + g_{n}(x) - f(x) - g(x)| \geq \epsilon \right\}\right)  \\ \leq \mu\left(\left\{x: |f_{n}(x) - f(x)|\geq \frac{\epsilon}{2}\right\}\right) + \mu\left(\left\{x: |g_{n}(x) - g(x)|\geq \frac{\epsilon}{2}\right\}\right). 
		\end{multline}
		Since the terms on the right side of the inequality chain vanish as $n \to \infty$, it follows that $f_{n} + g_{n} \to f + g$ in measure. 
		\item Now suppose $\mu(X) < \infty$, and $f_{n} \to f$, $g_{n} \to g$ in measure. Let $\epsilon > 0$. Then we note that by the triangle inequality
		\begin{align}
			\begin{split}
				\abs{fg - f_{n}g_{n}} &\leq \abs{f - f_{n}}|g| + \abs{f_{n}}\abs{g - g_{n}} \\
				&\leq \abs{f - f_{n}}|g| + \abs{f - f_{n}}\abs{g -g_{n}} + \abs{f}\abs{g - g_{n}}. 
			\end{split}
		\end{align}
		Hence, it follows that 
		\begin{multline}
			0 \leq \mu\left(\PDset{\abs{fg - f_{n}g_{n}}\geq \epsilon}\right) \leq\mu\left(\PDset{\abs{f- f_{n}}\abs{g} \geq \epsilon/3}\right) + \mu\left(\PDset{\abs{g-g_{n}}\abs{g} \geq \epsilon/3}\right)  \\
			+ \mu\left(\PDset{\abs{f - f_{n}}\abs{g - g_{n}} \geq \epsilon/3}\right). 
		\end{multline}
		Consider the last term first: 
		\begin{align}
			\begin{split}
				\mu\left(\PDset{\abs{f - f_{n}}\abs{g - g_{n}}\geq \epsilon/3}\right) \leq \mu\left(\PDset{\abs{f - f_{n}} \geq \sqrt{\epsilon/3}}\right) + \mu\left(\PDset{\abs{g - g_{n}}\geq \sqrt{\epsilon/3}}\right)
			\end{split}
		\end{align}
		Since $f_{n} \to f$ and $g_{n} \to g$ in measure, both terms on the right side of the inequality vanish as $n \to \infty$. Hence, the term on the left also vanishes as $n \to \infty$. Now consider the term $\mu(\{\abs{f - f_{n}}\abs{g} \geq \epsilon/3\})$. For each positive integer $j$, let $E_{j}$ be the set $\{|g|\geq j\}$. Then it is clear that $E_{j}$ is a decreasing sequence of sets. Hence, since $\mu(X) < \infty$, by continuity of the measure, $\lim{\mu(E_{j})} \to 0$, which means that for every $\epsilon > 0$, there exists some positive integer $N$ such that for all $n \geq N$, $\mu(E_{n}) < \delta$. In particular, $\mu(\{\abs{g} \geq N\}) = \mu(E_{n}) < \delta$. Therefore, for the $\epsilon$ used in the above steps, let us pick a choice of $N$ such that $\mu(\{\abs{g}\geq N\})\leq \epsilon$. Then 
		\begin{equation}
			\mu\left(\{\abs{f - f_{n}}\abs{g} \geq \epsilon/3\}\right) \leq \mu\left(\{\abs{f - f_{n}} \geq \epsilon/N\}\right) + \mu(\{\abs{g}\geq N\}). 
		\end{equation}
		And as $N \to \infty$, both terms on the right side of the inequality vanish. Hence, the term on the left side must also vanish. Likewise, $\mu(\{\abs{g - g_{n}}\abs{f}\geq \epsilon/3\})$ must also vanish. Hence, putting all of these components together, we find that $\mu(\{\abs{fg - f_{n}g_{n}}\geq \epsilon\})$ must vanish proving the desired statement. 
		
		If $\mu(X) = \infty$, then $f_{n}g_{n}$ do not necessarily converge to $fg$ in measure. Consider $X = \mathbb{R}$, and $f_{n} = g_{n} = x + 1/n$. It is clear to see that $f_{n}$ and $g_{n}$ both converge to $x$ in measure. Next, $f_{n}g_{n} = x^{2} + (2/n)x + (1/n^{2})$. For some arbitrary but fixed $\epsilon > 0$, we see that 
		\begin{equation}
			\mu\left(\left\{x: \abs{f_{n}g_{n} - fg}\geq \epsilon\right\}\right) = \mu\left(\left\{x: \frac{2x}{n} + \frac{1}{n^{2}} \geq \epsilon\right\}\right) = \mu\left(\left[\frac{n^{2}\epsilon - 1}{2n}, \infty\right)\right) = \infty, \forall n \in \mathbb{N}. 
		\end{equation}
		Hence, $f_{n}g_{n}$ does not converge to $fg$ in measure. 
	\end{enumerate}
	
	\begin{prb}{F-2-42}
		Let $\mu$ be the counting measure on $\mathbb{N}$. Then $f_{n} \to f$ in measure iff $f_{n} \to f$ uniformly. 
	\end{prb}
	Let $\mu$ be the counting measure on $\mathbb{N}$. 
	
	($\Longrightarrow$) Assume $f_{n} \to f$ uniformly. Fix $\epsilon > 0$. Then uniform convergence implies that there exists some positive integer $N$ such that for all $n \geq N$ and $x \in \mathbb{N}$, $|f_{n}(x) - f(x)| < \epsilon$. In particular, for all $n \geq N$, $\mu(\{x \in \mathbb{N}: |f_{n}(x) - f(x)| \geq \epsilon\}) = 0$. Thus it follows that $\mu(\{x \in \mathbb{N}: |f_{n}(x) - f(x)| \geq \epsilon\}) \xrightarrow{n \to \infty} 0$ so that $f_{n} \to f$ in measure. 
	
	($\Longrightarrow$) Now assume that $f_{n} \to f$ in measure, which means that for each fixed $\epsilon, \epsilon' > 0$, there exists a positive integer $N$ such that for all $n \geq N$, $\mu(\{x \in \mathbb{N}: |f_{n}(x) - f(x)| \geq \epsilon\}) \leq \epsilon'$. Fix $\epsilon' = \frac{1}{2}$, and let $N$ be the positive integer threshold. Since $\mu$ is the counting measure and for all $n \geq N$, 
	\begin{equation*}
		0 \leq \mu(\{x \in \mathbb{N}: |f_{n}(x) - f(x)| \geq \epsilon\}) \leq \frac{1}{2}, 
	\end{equation*}
	we conclude that for all $n \geq N$, $\mu(\{x \in \mathbb{N}: |f_{n}(x) - f(x)| \geq \epsilon\}) = 0$, which means that $\{x \in \mathbb{N}: |f_{n}(x) - f(x)| < \epsilon\} = \mathbb{N}$ since $\mu$ is the counting measure. Therefore, $f_{n} \to f$ uniformly. 
	
	\begin{prb}{F-6-9}
		Suppose $1 \leq p < \infty$. If $\norm{f_{n}-f}_{L^p} \to 0$, then $f_{n} \to f$ in measure, and hence some subsequence converges to $f$ a.e. On the other hand, if $f_{n} \to f$ in measure and $|f_{n}| \leq g \in L^{p}$ for all $n$, then $\norm{f_{n}-f}_{L^{p}} \to 0$.
	\end{prb}
	Suppose $1 \leq p < \infty$ and assume that $\norm{f_{n} - f}_{L^{p}} \to 0$. Fix an arbitrary $\epsilon > 0$, and let $E_{n,\epsilon} \coloneqq \{x \in X: |f_{n}(x) - f(x)| \geq \epsilon\}$. Then one observes that 
	\begin{equation*}
		\norm{f_{n} - f}_{L^{p}} = \left(\int_{X}|f_{n}(x) - f(x)|^{p}\;dx\right)^{1/p} \geq \left(\int_{E_{n,\epsilon}}|f_{n}(x) - f(x)|^{p}\;dx\right)^{1/p} \geq \epsilon \cdot \mu(E_{n,\epsilon})^{1/p}. 
	\end{equation*}
	Since the left side tends to 0 in the limit $n \to \infty$, so must the right side. Therefore, $\mu(E_{n, \epsilon})^{1/p} \xrightarrow{n \to \infty} 0$, which implies $\mu(E_{n, \epsilon}) \xrightarrow{n \to \infty} 0$. Thus by definition, $f_{n} \to f$ in measure. Since convergence in measure implies converges of a subsequence pointwise a.e., the first part of the proof is complete. Now assume that $f_{n} \to f$ in measure and $|f_{n}| \leq g \in L^{p}$ for all $n$. We consider the sequence $\{\norm{f_{n} - f}_{L^{p}(X)}\}_{n = 1}^{\infty}$. Let $\{\norm{f_{n_{j}} - f}_{L^{p}(X)}\}_{j = 1}^{\infty}$ be an arbitrary subsequence. Since $f_{n} \to f$ in measure, $f_{n_{j}} \to f$ in measure. Thus there exists a further subsequence $\{f_{n_{j_{k}}}\}_{k = 1}^{\infty}$ that converges to $f$ pointwise a.e. Since 
	\begin{equation*}
		|f_{n_{j_{k}}} - f|^{p} \leq 2^{p}(|f_{n_{j_{k}}}|^{p} + |f|^{p}) \leq 2^{p}(|g|^{p} + |f|^{p}) \in L^{1}(X)
	\end{equation*}
	because $f, g \in L^{p}(X)$, and $|f_{n_{j_{k}}}(x) - f(x)|^{p} \xrightarrow{k \to \infty} 0$ pointwise a.e., the Lebesgue dominated convergence theorem tells us that $f_{n_{j_{k}}} \to f$ in $L^{p}(X)$; i.e., $\norm{f_{n_{j_{k}}} - f}_{L^{p}} \xrightarrow{k \to \infty} 0$. Thus since every subsequence of $\{\norm{f_{n} - f}_{L^{p}(X)}\}_{n=1}^{\infty}$ has a convergent subsubsequence, we conclude that $\norm{f_{n} - f}_{L^{p}} \to 0$. 
	
	\begin{prb}{F-6-10}
		Suppose $1 \leq p < \infty$. If $f_{n}, f \in L^{p}$ and $f_{n} \to f$ a.e., then $\norm{f_{n} - f}_{L^{p}} \to 0$ iff $\norm{f_{n}}_{L^{p}} \to \norm{f}_{p}$. (Use Exercise 20 in \S 2.3.) 
	\end{prb}
	Let $1 \leq p < \infty$, $f_{n}, f \in L^{p}$ and $f_{n} \to f$ a.e. 
	
	($\Longrightarrow$) Suppose $\norm{f_{n} - f}_{L^{p}} \to 0$; fix an arbitrary $\epsilon > 0$. Then there exists a positive integer $N$ such that that for all $n \geq N$, $\norm{f_{n} - f}_{L^{p}} < \epsilon$. This implies by the Reverse Triangle Identity that for all $n \geq N$,
	\begin{equation*}
		\abs{\norm{f_{n}}_{L^{p}} - \norm{f}_{L^{p}}} \leq \norm{f_{n} - f}_{L^{p}} < \epsilon. 
	\end{equation*}
	Thus $\norm{f_{n}}_{L^{p}} \to \norm{f}_{L^{p}}$. 
	
	($\Longrightarrow$) Now assume that $\norm{f_{n}}_{L^{p}} \to \norm{f}_{L^{p}}$. Note that $2^{p}(|f_{n}|^{p} + |f|^{p}), 2^{p + 1}|f|^{p} \in L^{1}$ and $2^{p}(|f_{n}|^{p} + |f|^{p}) \to 2^{p + 1}|f|^{p}$ a.e. Likewise, since $|f_{n} - f|^{p} \leq 2^{p}(|f_{n}|^{p} + |f|^{p})$, $|f_{n} - f|^{p} \in L^{1}$ for all $n$, as is the zero function. We also have $|f_{n} -f|^{p} \to 0$ a.e. By the given hypothesis, one has that 
	\begin{equation*}
		\int 2^{p}(|f_{n}|^{p} + |f|^{p}) = 2^{p}\norm{f_{n}}_{L^{p}}^{p} + 2^{p}\norm{f}_{L^{p}}^{p} \xrightarrow{n \to \infty}2^{p}\norm{f}_{L^{p}}^{p} + 2^{p}\norm{f}_{L^{p}}^{p} = 2^{p + 1}\norm{f}_{L^{p}}^{p} = \int 2^{p + 1}|f|^{p}. 
	\end{equation*}
	Therefore, by Exercise 20 in \S 2.3, we conclude that 
	\begin{equation*}
		\norm{f_{n} - f}_{L^{p}}^{p} = \int |f_{n} - f|^{p} \xrightarrow{n \to \infty} \int 0 = 0 \implies \norm{f_{n} - f}_{L^{p}} \xrightarrow{n \to \infty} 0. 
	\end{equation*}
	...
	\end{prb}
	Fix $g \in L^{\infty}$, and define the operator $T$ on $L^{p}(X)$ by $Tf = fg$ for all $f \in L^{p}(X)$; we first assume $1 \leq p < \infty$. It follows that for all $f \in L^{p}(X)$, $Tf \in L^{p}(X)$ because 
	\begin{equation*}
		\norm{Tf}_{L^{p}}^{p} = \int |Tf|^{p} = \int |f|^{p}|g|^{p}\leq \int |f|^{p}\norm{g}_{L^{\infty}}^{p} = \norm{g}_{L^{\infty}}^{p} \cdot \norm{f}_{L^{p}}^{p} < \infty. 
	\end{equation*}
	In particular, this also shows that $T$ is bounded with operator norm at most $\norm{g}_{L^{\infty}}$ since for any $f \in L^{p}(X)$, $\norm{Tf}_{L^{p}} \leq \norm{g}_{\infty} \cdot \norm{f}_{L^{p}}$. Now assume $p = \infty$. Then the claim follows immediately since for a.e. $x \in X$ and every $f \in L^{\infty}(X)$, 
	\begin{equation*}
		|Tf(x)| = |f(x)||g(x)| \leq \norm{f}_{L^{\infty}} \cdot \norm{g}_{L^{\infty}} \implies \norm{Tf}_{L^{\infty}} =\operatorname{ess sup}_{x \in X}|Tf(x)| \leq \norm{f}_{L^{\infty}} \cdot \norm{g}_{L^{\infty}}. 
	\end{equation*}
	Therefore, $T$ is still bounded with operator norm at most $\norm{g}_{L^{\infty}}$. Now assume that $\mu$ is semifinite. We start by assuming $1 \leq p < \infty$. Fix $\epsilon > 0$, and define 
	\begin{equation*}
		E_{\epsilon} \coloneqq \{x \in X: |g(x)| \geq \norm{g}_{L^{\infty}} - \epsilon\}; 
	\end{equation*}
	since $\mu$ is semifinite, there exists a subset $E \subseteq E_{\epsilon}$ with $0 < \mu(E) < \infty$. Take $f = \chi_{E} \in L^{p}(X)$. Then one has 
	\begin{equation*}
		\norm{Tf}_{L^{p}} = \left(\int_{X}|f(x)|^{p}|g(x)|^{p}\right)^{1/p} \geq \left(\int_{E}|f|^{p}(\norm{g}_{L^{\infty}} - \epsilon)^{p}\right)^{1/p} = \norm{f}_{L^{p}} \cdot (\norm{g}_{L^{\infty}} - \epsilon). 
	\end{equation*}
	In particular, this implies that 
	\begin{equation*}
		\frac{\norm{Tf}_{L^{p}}}{\norm{f}_{L^{p}}} \geq \norm{g}_{\infty} - \epsilon. 
	\end{equation*}
	But by definition of the operator norm, one has that 
	\begin{equation*}
		\norm{T} \geq \frac{\norm{Tf}_{L^{p}}}{\norm{f}_{L^{p}}} \implies \norm{T} \geq \norm{g}_{\infty} - \epsilon. 
	\end{equation*}
	Thus taking the limit $\epsilon \to 0$ shows that $\norm{T} \geq \norm{g}_{\infty}$. Coupled with our observation above, $\norm{T} = \norm{g}_{\infty}$. Now suppose $p = \infty$. Define $\epsilon, E_{\epsilon}$, $E$, and $f$ as in the $1 \leq p < \infty$ case. Then one has that for any $x \in X \setminus E$, $|Tf(x)| = 0$, while for any $x \in E$, 
	\begin{equation*}
		|Tf(x)| \geq \norm{g}_{L^\infty} - \epsilon = \norm{f}_{L^{\infty}} \cdot (\norm{g}_{L^{\infty}} - \epsilon), 
	\end{equation*}
	since $\norm{f}_{L^{\infty}} = 1$. Therefore, 
	\begin{equation*}
		\norm{Tf}_{L^{\infty}} = \operatorname{ess sup}_{x \in X}|Tf(x)| \geq \operatorname{ess sup}_{x \in E}|Tf(x)| \geq \norm{f}_{L^{\infty}} \cdot (\norm{g}_{L^\infty} - \epsilon). 
	\end{equation*}
	This implies 
	\begin{equation*}
		\norm{T} \geq \frac{\norm{Tf}_{L^{\infty}}}{\norm{f}_{L^{\infty}}} \geq \norm{g}_{L^{\infty}} - \epsilon \xrightarrow{\epsilon \to 0} \norm{T} \geq \norm{g}_{\infty}. 
	\end{equation*}
	Thus we conclude that $\norm{T} = \norm{g}_{L^{\infty}}$ when $\mu$ is semifinite. 
	
	\begin{prb}{F-6-33}
		Given $1 < p < \infty$, let $Tf(x) = x^{-1/p}\int_{0}^{x}f(t)\;dt$. If $p^{-1} + q^{-1} = 1$, then $T$ is a bounded linear map from $L^{q}((0,\infty))$ to $C_{0}((0, \infty))$. 
	\end{prb}
	Fix $1 < p < \infty$, and let $q$ be the conjugate exponent to $p$. First, we will show that $T$ is linear operator on $L^{q}((0,\infty))$. For any $f, g \in L^{q}((0,\infty))$ and $\alpha, \beta \in \mathbb{R}$, one has that 
	\begin{align*}
		\begin{split}
			T(\alpha f(x) + \beta g(x)) &= x^{-1/p}\int_{0}^{x}\alpha f(t) + \beta g(t)\;dt = x^{-1/p}\int_{0}^{x}\alpha f(t)\;dt + x^{-1/p}\int_{0}^{x}\beta g(t)\;dt \\
			&= \alpha x^{-1/p}\int_{0}^{x}f(t)\;dt + \beta x^{-1/p}\int_{0}^{x}g(t)\;dt \\
			&= \alpha Tf(x) + \beta Tg(x). 
		\end{split}
	\end{align*}
	Thus $T$ is linear. Now we will show that for any $f \in L^{q}((0,\infty))$, $Tf(x)$ is continuous on $(0,\infty)$. Since $C_{c}^{\infty}((0,\infty))$ is dense in $L^{q}((0,\infty))$, let $\varphi \in C_{c}^{\infty}((0,\infty))$. Then for any $x \in (0, \infty)$, 
	\begin{align*}
		\begin{split}
			T\varphi(x + h) &= (x + h)^{-1/p} \cdot \int_{0}^{x + h}\varphi(t)\;dt = (x + h)^{-1/p} \cdot \int_{0}^{\infty}\varphi(t)\chi_{(0, x + h)}(t)\;dt.
		\end{split}
	\end{align*}
	Pointwise, $\varphi(t) \chi_{(0, x + h)}(t) \xrightarrow{h \to 0} \varphi(t)\chi_{(0,x)}(t)$ a.e. Moreover, $|\varphi(t)\chi_{(0, x + h)}(t)| \leq |\varphi(t)| \in L^{1}((0,\infty))$. Therefore, by the Lebesgue Dominated Convergence Theorem, 
	\begin{equation*}
		\lim_{h \to 0}T\varphi(x + h) = \lim_{h \to 0}(x + h)^{-1/p} \cdot \lim_{h \to 0}\int_{0}^{\infty}\varphi(t)\chi_{(0, x +  h)}(t)\;dt = x^{-1/p} \cdot \int_{0}^{\infty}\varphi(t)\chi_{(0,x)}(t)\;dt = T\varphi(x). 
	\end{equation*}
	Now let $f \in L^{q}((0, \infty))$ be arbitrary, fix $\epsilon > 0$, and choose $\varphi \in C_{c}^{\infty}((0,\infty))$ such that $\norm{f - \varphi}_{L^{q}(0, \infty)} < \epsilon/3$. By our work above, there exists some $\delta > 0$ so that $|h| < \delta$ implies $|T\varphi(x + h) - T\varphi(x)|< \epsilon/3$. Then one has that for any $x \in (0, \infty)$ and $|h| < \delta$,  
	\begin{align*}
		\begin{split}
			|Tf(x + h) - Tf(x)| &= |Tf(x + h) - T\varphi(x + h) + T\varphi(x + h) - T\varphi(x) + T\varphi(x) - Tf(x)| \\
			&\leq (x + h)^{-1/p} \cdot \int_{0}^{\infty}|f(t) - \varphi(t)|\chi_{(0, x + h)}\;dt + |T\varphi(x + h) - T\varphi(x)| + x^{-1/p}\cdot \int_{0}^{\infty}|f(t) - \varphi(t)|\chi_{(0,x)}\;dt \\
			&\leq \norm{f - \varphi}_{L^{q}(0, \infty)} + |T\varphi(x + h) - T\varphi(x)| + \norm{f - \varphi}_{L^{q}(0, \infty)} \\
			&\leq \frac{2\epsilon}{3} + |T\varphi(x + h) - T\varphi(x)| < \epsilon. 
		\end{split}
	\end{align*}
	Therefore, $Tf$ is continuous for any $f \in L^{q}((0, \infty))$. Now we must show that for any $f \in L^{q}((0, \infty))$, $\lim_{x \to \infty}Tf(x) = 0$. Fix $f \in L^{q}((0,\infty))$, $\epsilon > 0$, and let $\varphi \in C_{c}^{\infty}((0, \infty))$ such that $\norm{f - \varphi}_{L^{q}} < \epsilon$. Then since $\lim_{x \to \infty}\int_{0}^{x}\varphi(t)\;dt \leq \norm{\varphi}_{L^{1}(0,\infty)} < \infty$, $\lim_{x \to \infty}|T\varphi(x)| = 0$. Next, since  
	\begin{equation*}
		|Tf(x)| \leq |T(f - \varphi)(x)| + |T\varphi(x)| \leq \epsilon + |T\varphi(x)|, 
	\end{equation*} 
	\begin{equation*}
		\limsup_{x \to \infty}|Tf(x)| \leq \epsilon. 
	\end{equation*}
	Taking $\epsilon \to 0$, we conclude that $Tf(x) \to0$ as $x \to \infty$. The final result left to be shown is that $T$ is bounded. But this follows immediately: for any $f \in L^{q}((0,\infty))$, 
	\begin{equation*}
		|Tf(x)| \leq x^{-1/p} \cdot \int_{0}^{\infty}|f(t)|\chi_{(0,x)}\;dt \leq \norm{f}_{L^{q}(0,\infty)}, 
	\end{equation*}
	so that the operator norm of $T$ is at most 1. 
	
		\begin{prb}{F-8-7}
		If $f$ is locally integrable on $\mathbb{R}^{n}$ and $g \in C^{k}$ has compact support, then $f \ast g \in C^{k}$. 
	\end{prb}
	Let $f$ be a locally integrable function on $\mathbb{R}$, and $g \in C_{c}^{k}$. Consider an exhaustion of $\mathbb{R}$ by compact sets $\{K_{j}\}_{j = 1}^{\infty}$. Further, let $S^{\text{cpt}} \subset \mathbb{R}$ be a compact set containing the support of $g$. We will show $f \ast g \in C^{k}(K_{j})$ for each $j$. \vspace{-0.25cm}
	\begin{enumerate}[itemsep =-2pt,label = (\roman{*})]
		\item Since for each multiindex $|\alpha|\leq k$, $\partial^{\alpha}g: S \to \mathbb{R}$, by the extreme value theorem, we conclude that $\partial^{\alpha}g$ is bounded for each $|\alpha|\leq k$. 
		\item For each $j$, let $\psi_{j} \in C_{c}^{\infty}$ be a smooth function so that $\psi_{j} \equiv 1$ on $K_{j} - S = \{x - y: x\in K_{j}, y \in S\}$. Then $\psi_{j}f \in L^{1}(\mathbb{R})$. 
		\item By Proposition 8.10, we must have $(\psi_{j}f) \ast g \in C^{k}$ for each $j$. But we note that for each $x \in K_{j}$, 
		\begin{equation}
			(\psi_{j}f)\ast g(x) = \int_{S}(\psi_{j}f)(x-y)g(y)\;dy = \int_{S}f(x-y)g(y)\;dy \eqqcolon (f \ast g)(x), 
		\end{equation}
		where we used the fact that $\psi_{j}(x-y) = 1$ for all $x \in K_{j}$, $y \in S$. Since this property holds for each $K_{j}$, we conclude that $f \ast g \in C^{k}$. 
	\end{enumerate}
	
	\begin{prb}{F-8-19}
		(A variation on the theme of Exercise 18) If $f \in L^{2}(\mathbb{R}^{n})$ and the set $S = \{x: f(x) \neq 0\}$ has finite measure, then for any measurable $E \subset \mathbb{R}^{n}$, $\int_{E}|\hat{f}|^{2} \leq \norm{f}_{L^{2}}^{2}m(S)m(E)$. 
	\end{prb}
	Let $f \in L^{2}(\mathbb{R}^{n})$ and assume that the set $S = \{x: f(x) \neq 0\}$ has finite measure. By Plancherel's Theorem, $\hat{f} \in L^{2}(\mathbb{R}^{n})$ so that for any measurable set $E \subset \mathbb{R}^{n}$, 
	\begin{equation*}
		\int_{E}|\hat{f}|^{2}\;d\xi \leq \int_{X}|\hat{f}|^{2}\;d\xi = \norm{\hat{f}}_{L^{2}}^{2} < \infty. 
	\end{equation*}
	Therefore, if $m(E) = \infty$, then the inequality is satisfied trivially. So assume $m(E) < \infty$. Then we can write, 
	\begin{align*}
		\begin{split}
			\int_{E}|\hat{f}|^{2}\;d\xi &\leq \int_{E}\left(\abs{\int_{S}f(x)e^{-2\pi i (x\cdot \xi)}\;dx}\right)^{2}\;d\xi \\
			&\leq \int_{E}\left(\int_{S}|f(x)|\;dx\right)^{2}\;d\xi = m(E) \cdot \left(\int_{s}|f(x)|\;dx\right)^{2} \\
			&\leq m(E) \cdot \left(\int_{S}|f(x)|^{2}\;dx\right)\left(\int_{S}1\;dx\right) \\
			&= \norm{f}_{L^{2}}^{2}m(S)m(E), 
		\end{split}
	\end{align*}
	...
	\begin{prb}{F-8-20}
		If $f \in L^{1}(\mathbb{R}^{n + m})$, define $Pf(x) = \int f(x,y)\;dy$. (Here $x \in \mathbb{R}^{n}$ and $y \in \mathbb{R}^{m}$.) Then $Pf \in L^{1}(\mathbb{R}^{n})$, $\norm{Pf}_{L^{1}} \leq \norm{f}_{L^{1}}$, and $(Pf)^{\wh{}}(\xi) = \wh{f}(\xi,0)$. 
	\end{prb}
	Let $f \in L^{1}(\mathbb{R}^{n + m})$, and define $Pf(x) = \int f(x,y)\;dy$, where $x \in \mathbb{R}^{n}$ and $y \in \mathbb{R}^{m}$. 
	\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
		\item To show that $Pf \in L^{1}(\mathbb{R}^{n})$, we must show that $\int |Pf(x)|\;dx < \infty$. Using the definition of $Pf(x)$, one has, 
		\begin{equation*}
			\int_{\mathbb{R}^{n}} |Pf(x)|\;dx = \int_{\mathbb{R}^{n}}\abs{\int_{\mathbb{R}^{m}}f(x,y)\;dy}dx \leq \int_{\mathbb{R}^{n}}\int_{\mathbb{R}^{m}}|f(x,y)|\;dydx. 
		\end{equation*} 
		But Tonelli's theorem tells us that since $|f(x,y)| \in L^{+}(\mathbb{R}^{n + m})$, 
		\begin{equation*}
			\int_{\mathbb{R}^{n}}\int_{\mathbb{R}^{m}}|f(x,y)|\;dydx = \int_{\mathbb{R}^{n + m}}|f(x,y)|\;d(x,y) \eqqcolon \norm{f}_{L^{1}(\mathbb{R}^{n + m})} < \infty. 
		\end{equation*}
		Thus, one has that 
		\begin{equation*}
			\norm{Pf}_{L^{1}(\mathbb{R}^{n})} = \int_{\mathbb{R}^{n}}|Pf(x)|\;dx \leq \norm{f}_{L^{1}(\mathbb{R}^{n + m})} < \infty 
		\end{equation*}
		so that $Pf \in L^{1}(\mathbb{R}^{n})$. 
		\item Now we recall the definition of the Fourier transform, and write 
		\begin{equation*}
			(Pf)^{\wh{}}(\xi) = \int_{\mathbb{R}^{n}}Pf(x)e^{-2\pi i (x \cdot \xi)}\;dx = \int_{\mathbb{R}^{n}}\int_{\mathbb{R}^{m}}f(x,y)e^{-2\pi i (x \cdot \xi)}\;dydx. 
		\end{equation*}
		Since $f \in L^{1}(\mathbb{R}^{n + m})$, it follows by Fubini's theorem that 
		\begin{equation*}
			\int_{\mathbb{R}^{n}}\int_{\mathbb{R}^{m}}f(x,y)e^{-2\pi i (x \cdot \xi)}\;dydx = \int_{\mathbb{R}^{n + m}}f(x,y)e^{-2\pi ((x,y) \cdot (\xi,0))}d(x,y) \eqqcolon \hat{f}(\xi,0). 
		\end{equation*}
		Therefore, we conclude that $(Pf)^{\wh{}}(\xi) = \wh{f}(\xi,0)$. 
	\end{enumerate}
	...
	\subsection{The Hahn-Banach and Baire Category Theorems}
	...
	\begin{prb}{R-RA-1}
		Let $\mathscr{X} = L^{2}([0,1])$ with the standard real $L^{2}$ norm, 
		\begin{equation*}
			\norm{f}_{L^{2}} \coloneqq \left(\int_{0}^{1}|f(x)|^{2}\;dx\right)^{1/2}. 
		\end{equation*}
		Let $\mathscr{S} = \{\chi_{(0,a]}: a \in (0,1]\}$, and $\mathscr{M} = \operatorname{span}{\mathscr{S}}$. Show that $\mathscr{M}$ is dense in $L^{2}([0,1])$. 
	\end{prb}
	Let $\mathscr{X} = L^{2}([0,1])$ equipped with the real $L^{2}$-norm; let $\mathscr{S}$ and $\mathscr{M}$ be the subspaces defined above. By the Hahn-Banach theorem, it suffices to show that every continuous linear functional $F \in \mathscr{X}^{\ast}$ satisfying $F|_{\mathscr{M}} = 0$ is the zero functional on $\mathscr{X}$. Therefore, let $F \in \mathscr{X}^{\ast}$ be a continuous linear functional on $\mathscr{X}$ such that $F|_{\mathscr{M}} = 0$. Since $\mathscr{X}$ is a Hilbert space, by the Riesz Representation Theorem for Hilbert spaces, there exists a unique $f \in \mathscr{X}$ such that for every $g \in \mathscr{X}$, $F(g) = \int_{0}^{1}f(t)g(t)\;dt$. By our hypothesis on $F$, it follows that for every $a \in (0,1]$, 
	\begin{equation*}
		0 = F(\chi_{(0,a]}) = \int_{0}^{a}f(t)\;dt. 
	\end{equation*}
	Thus if $0 < a < b \leq 1$, then it follows that $\int_{a}^{b}f(t)\;dt = 0$. This means that the integral of $f$ over every possible subinterval of $(0,1]$ is zero. Let $U \subset (0,1]$ be an arbitrary open set. Since $U$ can be written as the countable disjoint union $\bigcup_{j = 1}^{\infty}I_{j}$ of disjoint intervals, one has that 
	\begin{equation*}
		\int_{U}f(t)\;dt = \sum_{j = 1}^{\infty}\int_{I_{j}}f(t)\;dt = 0. 
	\end{equation*}
	Thus the integral of $f$ over any open subset of $((0,1])$ is zero. Now let $E = \{x: f(x) > 0\}$. By outer regularity, let $U$ be an open subset of $(0,1]$ containing $E$ such that $m(U\setminus E) < \epsilon^{2}$ for some $\epsilon > 0$. Then, 
	\begin{align*}
		\begin{split}
			\abs{\int_{E}f(t)\;dt} &= \abs{\int_{U}f(t)\;dt - \int_{U\setminus E}f(t)\;dt} = \abs{\int_{U\setminus E}f(t)\;dt} \\
			&\leq \int_{U \setminus E}|f(t)|\;dt \leq \norm{f}_{L^{2}}m(U \setminus E)^{1/2} < \norm{f}_{L^{2}} \cdot \epsilon.
		\end{split} 
	\end{align*}
	Since the left side is independent of $\epsilon$, taking $\epsilon \to 0$ shows that $\int_{E}f(t)\;dt = 0$. Since $f(t) > 0$ on $E$, we conclude that $m(E) = 0$. Likewise, if $F = \{x : f(x) < 0\}$, we can show that $m(F) = 0$. Therefore, $f \equiv 0$ a.e. on $(0,1]$. But this implies that for \textit{any} $g \in L^{2}([0,1])$, $F(g) = \int_{0}^{1}f(t)g(t)\;dt = 0$ so that $F$ is the zero functional. Therefore, we conclude by the Hahn-Banach theorem that $\mathscr{M}$ is dense in $L^{2}([0,1])$. 
	
	
	\begin{prb}{F-5-19}
		Let $\mathscr{X}$ be an infinite-dimensional normed vector space. \vspace{-0.25cm}
		\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
			\item There is a sequence $\{x_{j}\}$ in $\mathscr{X}$ such that $\norm{x_{j}} = 1$ for all $j$ and $\norm{x_{j} - x_{k}} \geq \frac{1}{2}$ for $j \neq k$. (Construct $x_{j}$ inductively, using Exercises 12b and 18.)
			\item $\mathscr{X}$ is not locally compact. 
		\end{enumerate}
	\end{prb}
	Let $\mathscr{X}$ be an infinite-dimensional normed vector space. \vspace{-0.25cm}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item Let $B_{1}$ denote a unit ball in $\mathscr{X}$, and let $x_{1} \in B_{1}$ be an arbitrary element with norm one. Let $\mathscr{M}_{1} = \operatorname{span}\{x_{1}\}$. Since $\mathscr{M}_{1}$ is finite-dimensional, it follows that it is closed in $\mathscr{X}$. Then by Exercise 12b, there exists some $x_{2} \in \mathscr{X}\setminus \mathscr{M}_{1}$ such that $\norm{x_{2}} = 1$ and $\norm{x_{2} - x_{1}} \geq \frac{1}{2}$. Now let $\mathscr{M}_{2} = \mathscr{M}_{1} + \mathbb{C}x_{2}$. Then by Exercise 18, $\mathscr{M}_{2}$ is closed. Thus applying Exercise 12b again, there exists some $x_{3} \in \mathscr{X}\setminus \mathscr{M}_{2}$ such that $\norm{x_{3} - x_{2}}, \norm{x_{3} - x_{1}} \geq \frac{1}{2}$. Now let $\mathscr{M}_{3} = \mathscr{M}_{2} + \mathbb{C}x_{3}$, which is closed by Exercise 18. Thus by induction, we construct an infinite sequence $\{x_{j}\}_{j = 1}^{\infty}$ of points in $\mathscr{X}$ such that the norm of each element is one, and $\norm{x_{j} - x_{k}} \geq \frac{1}{2}$ for all $j \neq k$. 
		\item By construction, the sequence $\{x_{j}\}_{j = 1}^{\infty}$ is bounded, but it has no convergent subsequence since the distance between \textit{any two} distinct points is at least $1/2$. Therefore, it follows that $\mathscr{X}$ cannot be locally compact. 
	\end{enumerate}
		
	\begin{prb}{F-5-22}
		Suppose that $\mathscr{X}$ and $\mathscr{Y}$ are normed vector spaces and $T \in L(\mathscr{X}, \mathscr{Y})$. \vspace{-0.25cm}
		\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
			\item Define $T^{\dagger}: \mathscr{Y}^{\ast} \to \mathscr{X}^{\ast}$ by $T^{\dagger}f = f \circ T$. Then $T^{\dagger} \in L(\mathscr{Y}^{\ast}, \mathscr{X}^{\ast})$ and $\norm{T^{\dagger}}  = \norm{T}$. $T^{\dagger}$ is called the \textbf{adjoint} or \textbf{transpose} of $T$. 
			\item Applying the construction in (a) twice, one obtains $T^{\dagger\dagger} \in L(\mathscr{X}^{\ast\ast}, Y^{\ast\ast})$. If $\mathscr{X}$ and $\mathscr{Y}$ are identified with their natural images $\wh{\mathscr{X}}$ and $\wh{\mathscr{Y}}$ in $\mathscr{X}^{\ast\ast}$ and $\mathscr{Y}^{\ast\ast}$, then $T^{\dagger\dagger}|\mathscr{X} = T$. 
			\item $T^{\dagger}$ is injective iff the range of $T$ is dense in $\mathscr{Y}$. 
			\item If the range of $T^{\dagger}$ is dense in $\mathscr{X}^{\ast}$, then $T$ is injective; the converse is true if $\mathscr{X}$ is reflexive. 
		\end{enumerate}
	\end{prb}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item Given $T \in L(\mathscr{Y}, \mathscr{X})$, define $T^{\dagger}: \mathscr{Y}^{\ast} \to \mathscr{X}^{\ast}$ as given above. That $T$ is linear follows immediately: for any $\alpha, \beta \in \mathbb{C}$, $f, g \in \mathscr{Y}^{\ast}$, and $x \in \mathscr{X}$, one has that 
		\begin{multline*}
			T^{\dagger}(\alpha f + \beta g)(x) = (\alpha f + \beta g) \circ T(x) = \alpha (f \circ T)(x) + \beta (g \circ T)(x) = \alpha T^{\dagger}f(x) + \beta T^{\dagger}g(x) \\ \implies T^{\dagger}(\alpha f + \beta g) = \alpha T^{\dagger}f + \beta T^{\dagger}g. 
		\end{multline*}
		Now, we will how that $T^{\dagger}$ is bounded. For any $f \in \mathscr{Y}^{\ast}$, 
		\begin{equation*}
			\norm{T^{\dagger}f}_{\mathscr{X}^{\ast}} = \norm{f \circ T}_{\mathscr{X}^{\ast}} = \sup_{\norm{x}_{\mathscr{X}}\leq 1}|(f \circ T)(x)| \leq \sup_{\norm{x}_{\mathscr{X}} \leq 1}\norm{f}_{\mathscr{Y}^{\ast}}\norm{T}_{L(\mathscr{X}, \mathscr{Y})}\norm{x}_{\mathscr{X}} = \norm{f}_{\mathscr{Y}^{\ast}}\norm{T}_{L(\mathscr{X}, \mathscr{Y})}. 
		\end{equation*}
		Thus, since $f \in \mathscr{Y}^{\ast}$ was arbitrary, we conclude that $T^{\ast}$ is bounded with operator norm at most $\norm{T}$. On the other hand, for every nonzero $x \in \mathscr{X}$ (so that $Tx \neq 0$), the Hahn-Banach theorem guarantees the existence of some linear functional $f \in\mathscr{Y}^{\ast}$ with $\norm{f}_{\mathscr{Y}^{\ast}} = 1$ and
		\begin{equation*}
			(T^{\dagger}f)(x) = (f \circ T)(x) = \norm{Tx}_{\mathscr{Y}^{\ast}}. 
		\end{equation*}
		In particular, one has that
		\begin{equation*}
			\norm{Tx}_{\mathscr{Y}} \leq \norm{f}_{\mathscr{Y}^{\ast}}\norm{T^{\dagger}}\norm{x}_{\mathscr{X}} = \norm{T^{\dagger}}\norm{x}_{\mathscr{X}}. 
		\end{equation*}
		Since this inequality holds for all $x \in \mathscr{X}$, $\norm{T} \leq \norm{T^{\dagger}}$. Therefore, $\norm{T} = \norm{T^{\dagger}}$. 
		\item Applying the construction in (a) twice, let $T^{\dagger\dagger} \in L(\mathscr{X}^{\ast\ast}, \mathscr{Y}^{\ast\ast})$; identify $\mathscr{X}$ and $\mathscr{Y}$ with their natural images $\wh{\mathscr{X}}$ and $\wh{\mathscr{Y}}$ in $\mathscr{X}^{\ast\ast}$ and $\mathscr{Y}^{\ast\ast}$, respectively, whereupon for each $\hat{x} \in \wh{\mathscr{X}}$, $\hat{y} \in \wh{\mathscr{Y}}$ and $f \in \mathscr{X}^{\ast}, g \in \mathscr{Y}^{\ast}$, $\hat{x}(f) \coloneqq f(x)$ and $\hat{y}(g) \coloneqq g(y)$. Fix an arbitrary $x \in \mathscr{X}$ (and its corresponding element $\wh{x} \in \wh{\mathscr{X}}$). For any $f \in \mathscr{Y}^{\ast}$, 
		\begin{align*}
			\begin{split}
				(T^{\dagger\dagger}\wh{x})(f) &= \wh{x} \circ T^{\dagger}f = \wh{x}(f \circ T) = f(Tx). 
			\end{split}
		\end{align*}
		On the other hand, since $Tx \in \mathscr{Y}$, $\wh{Tx} \in \wh{\mathscr{Y}}$ so that $\wh{Tx}(f) \coloneqq f(Tx)$. This means that 
		\begin{equation*}
			T^{\dagger\dagger}\wh{x} = \wh{Tx}. 
		\end{equation*}
		Since we identify $\wh{Tx}$ with $Tx$, $\wh{x}$ with $x$, and $x \in \mathscr{X}$ was arbitrary, we conclude that $T^{\dagger\dagger}|_{\mathscr{X}} = T$. 
		\item Let $\mathscr{M} \subset \mathscr{Y}$ denote the range of $T$ in $\mathscr{Y}$, and let $\overline{\mathscr{M}}$ denote its closure in $\mathscr{Y}$. First assume that $\mathscr{M}$ is dense in $\mathscr{Y}$. Then by the Hahn-Banach theorem, every continuous linear functional $f \in \mathscr{Y}^{\ast}$ satisfying $f|_{\mathscr{M}} = 0$ is the zero functional. Let $f, g \in \mathscr{Y}^{\ast}$ and assume that $T^{\dagger}f = T^{\dagger}g$, which is to say that for every $x \in \mathscr{X}$, $T^{\dagger}f(x) = T^{\dagger}g(x) \iff (f \circ T)(x) = (g \circ T)(x) \iff (f - g)(Tx) = 0$. Since $f - g \in \mathscr{Y}^{\ast}$ and the preceding relation holds for all $x \in \mathscr{X}$ (hence for all $y \in \mathscr{M}$), our hypothesis tells us that $f - g \equiv 0 \implies f =g$. Therefore, $T^{\dagger}$ is injective. Now assume $T^{\dagger}$ is injective. This means that for every $x \in \mathscr{X}$ (hence for every $y \in \mathscr{M}$), $f(Tx) = (T^{\dagger}f)(x) = 0$ iff $f = 0$. In other words, $f \in \mathscr{Y}^{\ast}$ satisfying $f|_{\mathscr{M}} = 0$ implies $f$ is the zero functional. Thus by the Hahn-Banach Theorem, this implies that $\mathscr{M}$ is dense in $\mathscr{Y}$. 
		\item Now let $\mathscr{M} \subset \mathscr{X}^{\ast}$ denote the range of $T^{\dagger}$. Assume that $\mathscr{M}$ is dense in $\mathscr{X}^{\ast}$. By the Hahn-Banach theorem, every continuous linear functional $\chi \in \mathscr{X}^{\ast\ast}$ satisfying $\chi|_{\mathscr{M}} = 0$ must be the zero functional. By (b), it suffices to show that $T^{\dagger\dagger}|_{\wh{\mathscr{X}}}$ is injective, where $\wh{\mathscr{X}}$ is the natural image of $\mathscr{X}$ in $\mathscr{X}^{\ast\ast}$. Let $\hat{x}_{1}, \hat{x}_{2} \in \wh{\mathscr{X}}$ and assume that $T^{\dagger\dagger}\hat{x}_{1} = T^{\dagger\dagger}\hat{x}_{2}$. This means that for any $f \in \mathscr{Y}^{\ast}$, $T^{\dagger\dagger}(\hat{x}_{1} - \hat{x}_{2})(f) = 0 \iff (\hat{x}_{1} - \hat{x}_{2})(T^{\dagger}f) = 0$. Since this is true for all $f \in \mathscr{Y}^{\ast}$ (hence for all $\ti{f} \in \mathscr{M}$), our hypothesis tells us that $\hat{x}_{1} - \hat{x}_{2} \equiv 0 \iff \hat{x}_{1} = \hat{x}_{2}$. Therefore, $T^{\dagger\dagger}|_{\wh{\mathscr{X}}}$ is injective, whereupon $T$ is injective.  Now we will show that if $\mathscr{X}$ is reflexive (i.e., $\mathscr{X} = \mathscr{X}^{\ast\ast}$), then the converse result is true. Assume $T$ is injective. By the Hahn-Banach theorem, it suffices to show that every continuous linear functional $\chi \in \mathscr{X}^{\ast\ast}$ satisfying $\chi|_{\mathscr{M}} = 0$ is the zero functional. Let $\chi \in \mathscr{X}^{\ast\ast}$ be such a functional: $\chi(T^{\dagger}f) = 0$ for all $f \in \mathscr{Y}^{\ast}$. Since $\mathscr{X}$ is reflexive, $\chi$ corresponds to an element $x \in \mathscr{X}$ under the standard identification of $\mathscr{X}$ and $\mathscr{X}^{\ast\ast}$ so that $0 = \chi(T^{\dagger}f) = T^{\dagger}f(x) = f(Tx)$. Since this is true for all continuous linear functionals $f \in \mathscr{Y}^{\ast}$, we conclude that $Tx =0$. But since $T$ is injective, $x = 0$ so that $\chi = 0$. Therefore, by the Hahn-Banach theorem, we conclude that $\mathscr{M}$ is dense in $\mathscr{X}^{\ast}$. 
	\end{enumerate}
	
	
	\begin{prb}{F-5-32}
		Let $\norm{\empspace}_{1}$ and $\norm{\empspace}_{2}$ be norms on the vector space $\mathscr{X}$ such that $\norm{\empspace}_{1} \leq \norm{\empspace}_{2}$. If $\mathscr{X}$ is complete with respect to both norms, then the norms are equivalent. 
	\end{prb}
	Let $(X, \norm{\empspace}_{1})$ and $(X, \norm{\empspace}_{2})$ be complete normed vector spaces such that for any $x \in X$, $\norm{x}_{1} \leq \norm{x}_{2}$. To show that the two norms are equivalent, it suffices to show that there exists some $C > 0$ such that $\norm{\empspace}_{2} \leq C \norm{\empspace}_{1}$. Consider the linear operator $\iota: (X, \norm{\empspace}_{1}) \to (X, \norm{\empspace}_{2})$ defined by $\iota(x) = x$ for every $x \in X$. Since $\iota$ is bijective (as the identity map on $X$), let $\jmath: (X, \norm{\empspace}_{2}) \to (X, \norm{\empspace}_{1})$ be its inverse. Then $\jmath$ is also bijective and linear. Moreover, $\jmath$ is continuous on $(X, \norm{\empspace}_{2})$: for any $\epsilon > 0$, $\norm{x - y}_{2} < \epsilon$ implies 
	\begin{equation*}
		\norm{\jmath(x) - \jmath(y)}_{1} = \norm{x - y}_{1} \leq \norm{x - y}_{2} < \epsilon. 
	\end{equation*}
	Thus $\jmath \in L(\mathscr{X}, \mathscr{X})$. By the Bounded Inverse Theorem, it follows that $\iota$ is bounded; i.e., there exists some $C > 0$ such that for all $x \in X$, $\norm{\iota(x)}_{2} \leq C\norm{x}_{1} \iff \norm{x}_{2} \leq C\norm{x}_{1}$. Therefore, since we have shown that $\norm{\empspace}_{1} \leq \norm{\empspace}_{2} \leq C\norm{\empspace}_{1}$, we conclude that the two norms are equivalent. 
	
	\begin{prb}{F-5-37}
		Let $\mathscr{X}$ and $\mathscr{Y}$ be Banach spaces. If $T: \mathscr{X} \to \mathscr{Y}$ is a linear map such that $f \circ T \in \mathscr{X}^{\ast}$ for every $f \in \mathscr{Y}^{\ast}$, then $T$ is bounded. 
	\end{prb}
	Let $\mathscr{X}$ and $\mathscr{Y}$ be Banach spaces, $T: \mathscr{X} \to \mathscr{Y}$ be a linear map such that for every $f \in \mathscr{Y}^{\ast}$, $f \circ T \in \mathscr{X}^{\ast}$. Let $\{x_{n}\}_{n = 1}^{\infty} \subset \mathscr{X}$ such that $x_{n} \to x \in \mathscr{X}$ and assume that $Tx_{n} \to y \in \mathscr{Y}$. Since $f \circ T$ is continuous linear functional, one has that $(f \circ T)(x_{n}) \to (f \circ T)(x) = f(Tx)$. On the other hand, since $f$ is continuous, $(f \circ T)(x_{n}) = f(Tx_{n}) \to f(y)$. Thus $f(y) = f(Tx)$, which implies that for every $f \in \mathscr{Y}^{\ast}$, $f(y - Tx) =0$. Assume to the contrary that $y - Tx \neq 0$. Then define the subspace $\braket{y - Tx}$. Define the continuous linear functional $g: \braket{y - Tx} \to \mathbb{C}$ by $g(\alpha(y - Tx)) = \alpha$. Since $\braket{y - Tx}$ is a finite-dimensional normed vector space, $g$ must be continuous; thus $g$ is a continuous linear functional on the subspace $\braket{y - Tx}$. By the Hahn-Banach theorem, $g$ extends to a continuous linear functional $\hat{g}$ on $\mathscr{Y}^{\ast}$ such that $\hat{g}(y - Tx) = g(y - Tx) = 1$, which is a contradiction! Therefore, we must have $y - Tx = 0 \implies y = Tx$. Hence, by the Closed Graph Theorem, we conclude that $T$ is a bounded linear map. 
	
	\begin{prb}{F-5-38}
		Let $\mathscr{X}$ and $\mathscr{Y}$ be Banach spaces, and let $\{T_{n}\}$ be a sequence in $L(\mathscr{X}, \mathscr{Y})$ such that $\lim T_{n}x$ exists for every $x \in \mathscr{X}$. Let $Tx = \lim T_{n}x$; then $T \in L(\mathscr{X}, \mathscr{Y})$. 
	\end{prb}
	Let $\mathscr{X}$ and $\mathscr{Y}$ be Banach spaces, $\{T_{n}\}$ a sequence in $L(\mathscr{X}, \mathscr{Y})$ such that $\lim T_{n}x$ exists for every $x \in \mathscr{X}$, and define the operator $T: \mathscr{X} \to \mathscr{Y}$ pointwise by $Tx = \lim T_{n}x$ for every $x \in \mathscr{X}$. First, we will show that $T$ is linear. Let $\alpha, \beta \in K$ (the base field of $\mathscr{X}, \mathscr{Y}$), and $x, \ti{x} \in \mathscr{X}$. Then 
	\begin{align*}
		\begin{split}
			T(\alpha x + \beta \ti{x}) = \lim_{n \to \infty}T_{n}(\alpha x + \beta \ti{x}) = \lim_{n \to \infty}[\alpha T_{n}x + \beta T_{n}\ti{x}] = \alpha Tx + \beta T\ti{x}, 
		\end{split} 
	\end{align*}
	where the second equality follows from the fact that each $T_{n}$ is linear. Now, we must show that $T$ is bounded. Since for each $x \in \mathscr{X}$, $T_{n}x$ converges, it follows that $\{T_{n}x\}$ is bounded; i.e., $\sup_{n}\norm{T_{n}x}_{\mathscr Y} < \infty$; let $M = \sup_{n}\norm{T_{n}x}_{\mathscr Y}$. Then for any $x \in \mathscr{X}$ with $\norm{x}_{\mathscr{X}} = 1$, it follows that $\norm{T_{n}x}_{\mathscr{Y}} \leq M\norm{x}_{\mathscr{X}} = M$. Therefore, for any $x \in \mathscr{X}$ with norm one, $\norm{Tx}_{Y} \leq M$. Since 
	\begin{equation*}
		\norm{T} = \sup\{\norm{Tx}_{\mathscr{Y}}: x \in \mathscr{X}, \norm{x}_{\mathscr{X}} = 1\}, 
	\end{equation*}
	we conclude that $\norm{T} \leq M < \infty$ so that $T$ is bounded (continuous). Therefore, $T \in L(\mathscr{X}, \mathscr{Y})$. 
	
	
	\begin{prb}{F-5-39}
		Let $\mathscr{X}, \mathscr{Y}, \mathscr{Z}$ be Banach spaces and let $B: \mathscr{X} \times \mathscr{Y} \to \mathscr{Z}$ be a separately continuous bilinear map; that is, $B(x, \cdot) \in L(\mathscr{Y}, \mathscr{Z})$ for each $x \in \mathscr{X}$ and $B(\cdot, y) \in L(\mathscr{X}, \mathscr{Z})$ for each $y \in \mathscr{Y}$. Then $B$ is jointly continuous, that is, continuous from $\mathscr{X} \times \mathscr{Y}$ to $\mathscr{Z}$. (Reduce the problem to proving that $\norm{B(x, y)} \leq C\norm{x}\norm{y}$ for some $C > 0$.)
	\end{prb}
	Let $\mathscr{X}, \mathscr{Y}, \mathscr{Z}$ be Banach spaces, and let $B: \mathscr{X} \times \mathscr{Y} \to\mathscr{Z}$ be a separately continuous bilinear map. First we start by proving the hint: $B$ is jointly continuous iff for every $(x, y) \in \mathscr{X} \times \mathscr{Y}$, there exists some $C > 0$ such that $\norm{B(x, y)} \leq C\norm{x}\norm{y}$. 
	
	($\Longrightarrow$) Assume $B$ is jointly continuous so that it is continuous at $(0,0)$. This means there exists a $\delta > 0$ so that $\norm{x} \leq \delta$ and $\norm{y} \leq \delta$ implies $\norm{B(x,y)} \leq 1$. Now let $x \in \mathscr{X}$ and $y\in \mathscr{Y}$ be arbitrary points, and define 
	\begin{equation*}
		\hat{x} = \delta \frac{x}{\norm{x}}, \qquad \hat{y} = \delta \frac{y}{\norm{y}} \implies \norm{\hat{x}}, \norm{\hat{y}} = \delta.  
	\end{equation*}
	This implies 
	\begin{align*}
		\begin{split}
			\norm{B(x, y)} &= \norm{B(\hat{x}\delta^{-1}\norm{x}, \hat{y}\delta^{1}\norm{y})} \\
			&= \delta^{-2}\norm{x}\norm{y}\norm{B(\hat{x}, \hat{y})} \leq \delta^{-2}\norm{x}\norm{y}. 
		\end{split}
	\end{align*}
	Taking $C = \delta^{-2} >0$ completes the proof that joint continuity implies boundedness. 
	
	($\Longleftarrow$) Now assume that for every $(x, y) \in \mathscr{X} \times \mathscr{Y}$, there exists some $C > 0$ such that $\norm{B(x, y)} \leq C\norm{x}\norm{y}$. Fix some point $(x_{0}, y_{0}) \in \mathscr{X} \times \mathscr{Y}$. Then 
	\begin{align*}
		\begin{split}
			\norm{B(x, y) - B(x_{0}, y_{0})} &\leq \norm{B(x - x_{0}, y)} + \norm{B(x_{0}, y- y_{0})} \\
			&\leq C\left(\norm{x - x_{0}}\norm{y} + \norm{x_{0}}\norm{y- y_{0}}\right)
		\end{split}
	\end{align*}
	Thus in the limit $(x, y) \to (x_{0}, y_{0})$, we conclude that $\norm{B(x, y) - B(x_{0}, y_{0})} \to 0$ so that $B$ is jointly continuous. 
	
	Now we will show that separate continuity implies joint continuity. First define $S = \{x \in X: \norm{x} \leq 1\}$ to be the closed unit ball in $X$. Continuity in $\mathscr{X}$ implies that for any fixed $y \in \mathscr{Y}$, $\norm{B(x, y)} \leq C_{y} \norm{x} \leq C_{y}$, where $C_{y} > 0$ is some fixed constant that depends only on $y$. Therefore, the set of linear operators, $\{T_{x} \coloneqq B(x, y): \mathscr{Y} \to \mathscr{Z} \mid x \in S\}$ is pointwise bounded for every $y\in \mathscr{Y}$. The Uniform Boundedness Principle then implies that this bound is uniform; i.e., there exists some $0 < C < \infty$ such that for every $y \in \mathscr{Y}$, $\norm{B(x, y)} \leq C\norm{y}$ when $\norm{x} \leq 1$. Let $x \in \mathscr{X}$ be an arbitrary nonzero element, and define $\hat{x} = x/\norm{x}$. Then by bilinearity, one has that 
	\begin{equation*}
		\norm{B(x,y)} = \norm{x}\norm{B(\hat{x}, y)} \leq C\norm{x}\norm{y}. 
	\end{equation*}
	The inequality follows trivially for $x = 0$. Thus, we conclude that $B$ is jointly continuous. 
	...
	\begin{prb}{F-8-5}
		If $s: \mathbb{R}^{n} \times \mathbb{R}^{n}$ is defined by $s(x, y) = x - y$, then $s^{-1}(E)$ is Lebesgue measurable whenever $E$ is Lebesgue measurable. (For $n = 1$, draw a picture of $s^{-1}(E) \subset \mathbb{R}^{2}$. It should be clear that after rotation through an angle $\pi/4$, $s^{-1}(E)$ becomes $F \times \mathbb{R}$ where $F = \{x: \sqrt{2}x \in E\}$, and Theorem 2.44 can be applied. The same idea works in higher dimensions.)
	\end{prb}
	Let $s: \mathbb{R}^{n}\times \mathbb{R}^{n}$ be defined by $s(x, y) = x - y$. We begin by showing that $s(x,y)$ is continuous everywhere in $\mathbb{R}^{n} \times \mathbb{R}^{n}$. Let $\epsilon > 0$, and pick $\delta = \epsilon/2$. Then for any $(x_{0}, y_{0}) \in \mathbb{R}^{n} \times \mathbb{R}^{n}$, $|(x,y) - (x_{0}, y_{0})| < \delta$ implies 
	\begin{equation*}
		|s(x,y) - s(x_{0},y_{0})| = |x - y - x_{0} + y_{0}| \leq |x - x_{0}| + |y - y_{0}| \leq 2\delta = \epsilon. 
	\end{equation*}
	Now let $E \subset \mathbb{R}^{n}$ be a Lebesgue measurable set; then there exists a Borel set $B$ and a null set $N$ such that $E \Delta B \subset N$, where $\Delta$ denotes the symmetric difference. One notes that $s^{-1}(E) \Delta s^{-1}(B) \subset s^{-1}(N)$; since $B$ is Borel measurable and the preimage of any Borel measurable set under a continuous map is Borel measurable, $s^{-1}(B)$ is Borel measurable. Thus it suffices to show that $s^{-1}(N)$ is a null set. Let $N_{0} \subset \mathbb{R}^{n}$ be a Borel set such that $N \subset N_{0}$ and $m_{n}(N_{0}) = 0$, where $m_{n}$ denotes the $n$-dimensional Lebesgue measure on $\mathbb{R}^{n}$. Then since $s$ is continuous and $N_{0}$ is Borel, $s^{-1}(N_{0})$ is a Borel subset of $\mathbb{R}^{2n}$. Thus since $\mathscr{B}(\mathbb{R}^{2n}) = \mathscr{B}(\mathbb{R}^{n})\otimes \mathscr{B}(\mathbb{R}^{n})$, where $\mathscr{B}$ denotes the Borel $\sigma$-algebra (see e.g., Chapter 1 of Folland), $\chi_{S^{-1}(N_{0})}(x,y) \coloneqq \chi_{N_{0}}(x - y)$ is product-measurable. Thus applying Tonelli's theorem to $\chi_{N_{0}}(x - y)$, one obtains 
	\begin{align*}
		\begin{split} 
		m_{2n}(s^{-1}(N_{0})) &= \int_{\mathbb{R}^{n}}\int_{\mathbb{R}^{n}}\chi_{N_{0}}(x - y)\;dxdy = \int_{\mathbb{R}^{n}}\left(\int_{\mathbb{R}^{n}}\tau_{y}\chi_{N_{0}}(x)\;dx\right)\;dy \\
		&= \int_{\mathbb{R}^{n}}m_{n}(N_{0} - y)\;dy \\
		&= \int_{\mathbb{R}^{n}}m_{n}(N_{0})\;dy = 0, 
		\end{split} 
	\end{align*}
	where in going to the final line, we used the translation invariance of the Lebesgue measure. Therefore, $s^{-1}(N_{0})$ is a null set. Since the Lebesgue measure is complete and $s^{-1}(N) \subset s^{-1}(N_{0})$, we conclude that $m_{2n}(s^{-1}(N)) = 0$ so that $s^{-1}(N)$ is also a null set. Since $s^{-1}(E)$ differs from a Borel set by a null set, we conclude that $s^{-1}(E)$ is Lebesgue measurable. 
	...
	\begin{prb}{2012-J-II-2}
		Let $f$ be a continuous, complex-valued function on the closed disk $\overline{\Delta} =\{z \in \mathbb{C}: |z| \leq 1\}$ which is holomorphic on the interior. Assume  that $f$ restricted to the boundary $\partial \overline{\Delta}$ only takes values on the curve $\{x + iy \in \mathbb{C}: x, y \in \mathbb{R}, xy = 1\}$. Show that $f$ is constant on the whole disk. 
	\end{prb}
	Let $f$ be a continuous, complex-valued function on the closed disk $\overline{\Delta}$, holomorphic on the interior, and such that $f(\partial \overline{\Delta})$ only takes values on the curve $\{x + iy \in \mathbb{C}: x,y \in \mathbb{R}, xy = 1\}$.Let $g(z) = f(z)^{2}$, which is a harmonic function. In particular, if for some $z \in \partial \overline{\Delta}$, $f(z)= x + iy$, then one has $g(z) = (x^{2} - y^{2}) + 2ixy = (x^{2} - y^{2}) + 2i$. Since $g$ is harmonic, so is its imaginary part $v(z)$. Thus, by the maximum/minimum principles for harmonic functions, one has that 
	\begin{equation*}
		\max_{|z| \leq 1}v(z) = \max_{|z| = 1}v(z) = 2 \qquad \text{and} \qquad \min_{|z| \leq 1}v(z) = \min_{|z| = 1}v(z) = 2, 
	\end{equation*}
	so that the imaginary part of $g$ is constant over the whole disk $\overline{\Delta}$. But since $\overline{\mathbb{D}}$ is connected, the Cauchy-Riemann equations imply that $g(z)$, and hence $f(z)^{2}$, is constant. Let $f(z)^{2}\equiv c$ on $\overline{\Delta}$. Then for every $z \in \overline{\Delta}$, $f(z) \in \{\pm \sqrt{c}\}$. But since $f$ is continuous over the whole disk, $f(z) \equiv \sqrt{c}$ (or $f(z) \equiv -\sqrt{c}$). Thus $f$ is constant on the whole disk. 
	...
	\begin{equation*}
		\oint_{\Gamma_{R}}f(z)\;dz = 2\pi i \cdot \left(-\frac{1}{4\sqrt{2}} -\frac{i}{4\sqrt{2}}+ \frac{1}{4\sqrt{2}} - \frac{i}{4\sqrt{2}}\right) = \frac{\pi}{\sqrt{2}}. 
	\end{equation*}
	Therefore, we conclude that 
	\begin{equation*}
	...
	\begin{enumerate}[resume = algebra]
		...
		\item The transitive subgroups of $S_{4}$ are $\mathbb{Z}_{4}$, $\sf{V}_{4}$ (normal), $D_{8}$, $A_{4}$ (normal), and $S_{4}$.  The transitive subgroups of $S_{3}$ are $A_{3}$ (normal) and $S_{3}$. 
	...
	\begin{enumerate}
		...
		\item (Alternating Sum of Dimensions for Exact Sequences) Let $0 \xrightarrow{f_{0}} V_{1} \xrightarrow{f_{1}} V_{2} \xrightarrow{f_{2}} \dotsm \xrightarrow{f_{n - 1}} V_{n} \xrightarrow{f_{n}} 0$ be a (long) exact sequence. Then 
		\begin{equation*}
			\sum_{k = 1}^{n}(-1)^{k}\dim{V_{k}} = 0. 
		\end{equation*}
		\expl{
		Consider the long exact sequence  
		\begin{equation*}
			0 \xrightarrow{f_{0}} V_{1} \xrightarrow{f_{1}} V_{2} \xrightarrow{f_{2}} \dotsm \xrightarrow{f_{n-1}} V_{n} \xrightarrow{f_{n}} 0. 
		\end{equation*}
		The rank-nullity theorem tells us that for each $i = 1, \ldots, n$, 
		\begin{equation*}
			\dim{V_{i}} = \dim{\operatorname{im}{f_{i}}} + \dim{\operatorname{ker}{f_{i}}}. 
		\end{equation*}
		Therefore, 
		\begin{equation*}
			\sum_{k = 1}^{n}(-1)^{k}\dim{V_{k}} = \sum_{k = 1}^{n}(-1)^{k}\dim{\operatorname{im}{f_{k}}} + \sum_{k = 1}^{n}(-1)^{k}\dim{\ker{f_{k}}}. 
		\end{equation*}
		By exactness, one has that $\operatorname{im}{f_{k}} = \ker{f_{k + 1}}$ for each $k = 0, \ldots, n$ so that $\dim{\operatorname{im}{f_{k}}} = \dim{\ker{f_{k + 1}}}$ for each $k = 0, \ldots, n$. Since $(-1)^{k + 1} = -(-1)^{k}$, this implies that 
		\begin{equation*}
			\sum_{k = 1}^{n}(-1)^{k}\dim{V_{k}} = (-1)^{n}\dim{\operatorname{im}{f_{n}}} - \dim{\ker{f_{1}}} = 0 - \dim{\operatorname{im}{f_{0}}} = 0.  
		\end{equation*}
		Thus the proof concludes. 
	}
	\item (Poincar\'e Duality) Let $M$ be a compact orientable smooth $n$-manifold. Then for all $0 \leq p \leq n$, $H^{p}_{\text{dR}}(M) = H^{n -p}_{\text{dR}}(M)$. In particular, if $n$ is odd, then $\chi(M) = 0$, where $\chi$ denotes the Euler characteristic. 
	\end{enumerate}
	...
	\begin{enumerate}[start = 29]
		...
		\item Let $M$ be a smooth $n$-manifold, and $\theta$ a nowhere vanishing $1$-form on $M$. Let $D \coloneqq \ker{\theta}$. Then $D$ is a codimension $1$ distribution ($\operatorname{rank}(D) = n - 1$) on $M$. 
		\item $(\theta \wedge d\theta)(X,Y,Z) \propto \theta(X)d\theta(Y,Z) + \theta(Y)d\theta(Z, X) + \theta(Z)d\theta(X, Y)$. 
		\item (Few Properties of the Lie Derivative) We have the following properties for the Lie derivative. \vspace{-0.25cm}
		\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
			\item For $a, b \in \mathbb{R}$, $\mathscr{L}_{X}(a\omega + b \eta) = a\mathscr{L}_{X}\omega + b\mathscr{L}_{X}\eta$. 
			\item $\mathscr{L}_{[X, Y]}\omega = \mathscr{L}_{X}\mathscr{L}_{Y}\omega - \mathscr{L}_{Y}\mathscr{L}_{X}\omega$. 
			\item (\textsc{Cartan's Formula}) $\mathscr{L}_{Y}\alpha = i_{Y}d\alpha + d(i_{Y}\alpha)$, where $i_{Y}$ denotes the interior derivative with respect to the vector field $Y$.
			\item $d\mathscr{L}_{X}\omega = \mathscr{L}_{X}(d\omega)$. 
			\item $\mathscr{L}_{fX}\omega = f\mathscr{L}_{X}\omega + df\wedge i_{X}\omega$. 
			\item If $X$, $Y$ are vector fields,  then $\mathscr{L}_{X}Y = [X, Y]$. 
			\item $[\mathscr{L}_{X}, \mathscr{L}_{Y}] = \mathscr{L}_{[X,Y]}$. 
		\end{enumerate}
	\item If $\omega$ is a 1-form, then $d\theta(X,Y)= X(\theta(Y)) - Y(\theta(X))- \theta([X,Y])$.
	\item The interior derivative of a $k$-form $\theta$, denoted by $i_{V}\theta$, a $(k - 1)$-form. 
	\end{enumerate}
	
	\subsubsection{Real Analysis}
	\paragraph{Lebesgue Measure Theory}
\begin{enumerate}[series=real_analysis]
...
\end{enumerate}
\paragraph{Generic Convergence Results}
\begin{enumerate}[resume=real_analysis]
	\item Let $\{a_{n}\}_{n = 1}^{\infty} \subset \mathbb{R}$ be a sequence of real numbers. Then $a_{n}$ converges to $L \in \mathbb{R}$ if and only if for each subsequence, there exists a subsubsequence $\{a_{n_{j_{k}}}\}_{k = 1}^{\infty}$ that converges to $L$. 
	\item Let $\{f_{n}\}_{n = 1}^{\infty}$ and $f$ be measurable functions on a measure space $(X, \mu)$. $f_{n} \to f$ in measure if for each $\epsilon > 0$, 
	\begin{equation*}
		\mu(\{x \in X \mid |f_{n}(x) - f(x)| \geq \epsilon\}) \xrightarrow{n \to \infty} 0. 
	\end{equation*}
	\item Let $1 \leq p \leq \infty$, $(X, \mu)$ a measure space, and let $\{f_{n}\}_{n = 1}^{\infty}, f \in L^{p}(X,\mu)$. Then $f_{n} \to f$ in $L^{p}$ if $\norm{f_{n} - f}_{L^{p}} \to 0$. 
	\item Let $(X, \mu)$ be a measure space, and let $\{f_{n}\}_{n = 1}^{\infty}, f$ be measurable functions. If $f_{n} \to f$ in $L^{p}$ ($1 \leq p \leq \infty$), then $f_{n} \to f$ in measure. \newline 
	\begin{solutions}
		First assume $1 \leq p < \infty$, and let $f_{n} \to f$ in $L^{p}$, and fix $\epsilon > 0$. Let $E_{\epsilon} = \{x \in X\mid |f_{n}(x) - f(x)|\geq \epsilon\}$. Then 
		\begin{equation*}
			\left(\int_{X}|f_{n}(x) - f(x)|^{p}\;d\mu\right)^{1/p} \geq \left(\int_{E_{\epsilon}}|f_{n}(x) - f(x)|^{p}\;d\mu\right)^{1/p} \geq \left(\int_{E_{\epsilon}}\epsilon^{p}\;d\mu\right)^{1/p} = \epsilon \cdot \mu(E_{\epsilon})^{1/p}. 
		\end{equation*}
		In particular, this implies 
		\begin{equation*}
			\mu(E_{\epsilon})^{1/p} = \frac{1}{\epsilon} \cdot\norm{f_{n} - f}_{L^{p}} \implies \limsup_{n \to \infty}\mu(E_{\epsilon})^{1/p} = \frac{1}{\epsilon} \cdot \limsup_{n \to \infty}\norm{f_{n} - f}_{L^{p}} = 0, 
		\end{equation*}
		so that $\lim_{n \to \infty}\mu(E_{\epsilon}) = 0$. Since $\epsilon > 0$ was arbitrary, we conclude that $f_{n} \to f$ in measure. Now assume $p = \infty$. Fix $\epsilon > 0$. Then there exists a positive integer $N$ such that for all $n \geq N$, $\norm{f_{n} - f}_{L^{\infty}} < \epsilon$. By definition of the $L^{\infty}$ norm, for a.e. $x \in X$, $|f_{n}(x) - f(x)| \leq \norm{f_{n} - f}_{L^{\infty}} < \epsilon$. This means that $\mu(\{x \in X\mid |f_{n}(x) -f (x)| \geq \epsilon) = 0$ for all $n \geq N$ so that $f_{n} \to f$ in measure. 
	\end{solutions}
	\item If $f_{n} \to f$ in measure, there exists a subsequence $\{f_{n_{j}}\}$ that converges to $f$ pointwise a.e. 
	\item If $f_{n} \to f$ in $L^{1}$, there exists a subsequence $\{f_{n_{j}}\}$ that converges to $f$ pointwise a.e. 
	\item (Egoroff's Theorem) Suppose that $\mu(X) < \infty$, and $f_{1}, f_{2}, \ldots,$ and $f$ are measurable complex-valued functions on $X$ such that $f_{n} \to f$ a.e. Then for every $\epsilon > 0$ there exists $E \subset X$ such that $\mu(E) < \epsilon$ and $f_{n} \to f$ uniformly on $E^{c}$. \newline 
	\begin{solutions}
		Without loss of generality, we may assume that $f_{n} \to f$ everywhere on $X$. For $k, n\in \mathbb{N}$, let 
		\begin{equation*}
			E_{n}(k) = \bigcup_{m = n}^{\infty}\{x \mid |f_{m}(x) - f(x)| \geq k^{-1}\}. 
		\end{equation*}
		Then for fixed $k$, $E_{n}(k)$ decreases as $n$ increases, and $\bigcap_{n =1 }^{\infty}E_{n}(k) = \varnothing$. Thus since $\mu(X) < \infty$, continuity of limits concludes that $\mu(E_{n}(k)) \to 0$ as $n \to \infty$. Given $\epsilon > 0$ and $k \in \mathbb{N}$, choose $n_{k}$ so large that $\mu(E_{n_{k}}(k)) < \epsilon 2^{-k}$ and let $E = \bigcup_{k =1}^{\infty}E_{n_{k}}(k)$. Then $\mu(E_{k}) < \epsilon$ and $|f_{n}(x) - f(x)| < k^{-1}$ for $n > n_{k}$ and $x \notin E$. Thus $f_{n} \to f$ uniformly on $E^{c}$.
	\end{solutions}
	\item (Lusin's Theorem) If $f: [a, b] \to \mathbb{C}$ is Lebesgue measurable and $\epsilon > 0$, there is a compact set $E \subset [a, b]$ such that $\mu(E^{c}) < \epsilon$ and $f|E$ is continuous. 
	\item (Fubini-Tonelli) Suppose that $(X, \mathscr{M}, \mu)$ and $(Y, \mathscr{N},\nu)$ are $\sigma$-finite measure spaces. \vspace{-0.25cm}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item (Tonelli) If $f \in L^{+}(X \times Y)$, then the functions $g(x) = \int f_{x}\;d\nu$ and $h(y) = \int f^{y}\;d\mu$ are in $L^{+}(X)$ and $L^{+}(Y)$, respectively, and 
		\begin{equation*}
			\int f\;d(\mu \times \nu) = \int \left[\int f(x,y)\;d\nu(y)\right]\;d\mu(x) = \int \left[\int f(x,y)\;d\mu(x)\right]\;d\nu(y). 
		\end{equation*}
		\item (Fubini) If $f \in L^{1}(\mu \times \nu)$, then $f_{x} \in L^{1}(\nu)$ for a.e. $x$ and $f^{y} \in L^{1}(\mu)$ for a.e. $y$, and the above equation holds. 
	\end{enumerate}
	\item (Arzel\`a-Ascoli) The following results are both instances of the \textit{Arzel\`a-Ascoli Theorem}. \vspace{-0.25cm}
	\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
		\item If a sequence $\{f_{n}\}_{n = 1}^{\infty} \subset C(X)$ is bounded and equicontinuous, then it has a uniformly convergent subsequence. 
		\item A subset $\mathscr{F} \subset C(X)$ is compact if and only if it is closed, bounded, and equicontinuous. 
	\end{enumerate}
\end{enumerate}
\paragraph{Dual Spaces}
\begin{enumerate}[resume = real_analysis]
	\item (Dual $L^{p}$ Spaces) Let $1 < p < \infty$, and let $p, q$ be conjugate exponents. For each $\phi \in (L^{p})^{\ast}$, there exists $g \in L^{q}$ such that $\phi(f) = \int fg$ for all $f \in L^{p}$, and hence $L^q$ is isometrically isomorphic to $(L^{p})^{\ast}$. The same conclusion holds for $p = 1$ provided $\mu$ is $\sigma$-finite. Thus for $1 < p < \infty$, $L^{p}$ is reflexive. 
	\item (Chebyshev's Inequality) If $f \in L^{p}$ ($0 < p < \infty$), then for any $\alpha > 0$, 
	\begin{equation*}
		\mu(\{x: |f(x)| > \alpha\}) \leq \left[\frac{\norm{f}_{p}}{\alpha}\right]^{p}. 
	\end{equation*}
	\begin{solutions}
		Let $E_{\alpha} = \{x: |f(x)| > \alpha\}$. Then 
		\begin{equation*}
			\norm{f}_{p}^{p} = \int |f|^{p} \geq \int_{E_{\alpha}}|f|^{p} \geq \alpha^{p}\mu(E_{\alpha}). 
		\end{equation*}
	\end{solutions}
	\item (Dual Space of $C_{0}(X)$) Let $X$ be a LCH (locally compact Hausdorff) space, and let $M(X)$ denote the space of all signed Radon measures\footnote{Borel measures that are finite on all compact sets, outer regular on all Borel sets, and inner regular on all open sets.}. Then the dual space of $C_{0}(X)$ is isometrically isomorphic to $M(X)$ in the sense that for every bounded linear operator $I: C_{0}(X) \to \mathbb{R}$, there exists a signed Radon measure $\mu \in M(X)$ such that $I(f) = \int f\;d\mu$ for all $f \in C_{0}(X)$. 
	\item (Dual Space of $C(X)$) Let $X$ be a compact Hausdorff space. Then $C(X)^{\ast}$ is isometrically isomorphic to $M(X)$. 
	\item (Weak$^{\ast}$/Vague Convergence) $\mu_{\alpha}\to \mu$ in $M(X)$ iff $\int f\;d\mu_{\alpha} \to \int f\;d\mu$ for all $f \in C_{0}(X)$. 
	\item (Riesz Representation Theorem; Hilbert Spaces) Let $\mathscr{H}$ be a Hilbert space. If $f \in \mathscr{H}^{\ast}$, there exists a unique $y \in \mathscr{H}$ such that $f(x) = \braket{x,y}$ for all $x \in \mathscr{X}$. 
\end{enumerate}
	
	\paragraph{Hahn-Banach Theorem, Baire Category Theorem, and Consequences}
	\begin{enumerate}[resume = real_analysis]
		\item (Hahn-Banach Theorem) Let $\mathscr{X}$ be a real vector space, $p$ a sublinear functional on $\mathscr{X}$, $\mathscr{M}$ a subspace of $\mathscr{X}$, and $f$ a linear functional on $\mathscr{M}$ such that $f(x) \leq p(x)$ for all $x \in \mathscr{M}$. Then there exists a linear functional $F$ on $\mathscr{X}$ such that $F(x) \leq p(x)$ for all $x \in \mathscr{X}$ and $F|\mathscr{M} = f$. In other words, every bounded linear functional on a subspace can be extended to a bounded linear functional on the whole space. 
		\item (Few Immediate Consequences of the Hahn-Banach Theorem) Let $\mathscr{X}$ be a normed vector space. \vspace{-0.25cm}
		\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
			\item If $\mathscr{M}$ is a closed subspace of $\mathscr{X}$ and $x \in \mathscr{X}\setminus \mathscr{M}$, there exists a continuous linear functional, $f \in \mathscr{X}^{\ast}$ such that $f(x) \neq 0$ and $f|\mathscr{M} = 0$. In particular, if $\delta = \inf_{y \in \mathscr{M}}\norm{x - y}$, then one can choose $\norm{f}= 1$ and $f(x) = \delta$. 
			\item If $x \in \mathscr{X}\setminus \{0\}$, there exists $f \in \mathscr{X}^{\ast}$ such that $\norm{f}= 1$ and $f(x) = \norm{x}$. 
			\item The bounded linear functionals on $\mathscr{X}$ separate points, which is to say that if $x \neq y$, then $f(x) \neq f(y)$. 
			\item If $x \in \mathscr{X}$, define $\wh{x}: \mathscr{X}^{\ast} \to \mathbb{C}$ by $\wh{x}(f)= f(x)$. Then the map $x \mapsto \wh{x}$ is a linear isometry from $\mathscr{X}$ into $\mathscr{X}^{\ast\ast}$. 
		\end{enumerate} 
		\item Let $\mathscr{X}$ be a normed vector space and $\mathscr{M} \subseteq \mathscr{X}$ a subspace. $\mathscr{M}$ is dense in $\mathscr{X}$ if and only if every $f \in \mathscr{X}^{\ast}$ satisfying $f|_{\mathscr{M}} = 0$ is the zero functional ($f \equiv 0$). \newline 
		\begin{solutions}
			($\Longleftarrow$)  Let $\mathscr{X}$ be a normed vector space and $\mathscr{M} \subseteq \mathscr{X}$ a subspace. One notes that the closure $\overline{\mathscr{M}}$ is a closed subspace of $\mathscr{X}$. So assume $\overline{\mathscr{M}} \neq \mathscr{X}$; i.e., assume there exists some $x_{0} \in \mathscr{X}\setminus \overline{\mathscr{M}}$. Then by the Hahn-Banach Theorem, there exists a continuous linear functional $f \in \mathscr{X}^{\ast}$ such that $f(x_{0}) \neq 0$ and $f|_{\mathscr{M}} = 0$. Therefore, by the contrapositive, it follows that if every $f \in \mathscr{X}^{\ast}$ satisfying $f|_{\mathscr{M}} = 0$ is the zero functional, then $\mathscr{M}$ is dense in $\mathscr{X}$.
			
			($\Longrightarrow$) Now assume $\overline{\mathscr{M}} = \mathscr{X}$, and let $f \in \mathscr{X}^{\ast}$ be a \textit{continuous} linear functional satisfying $f|_{\mathscr{M}} =0$. Let $x \in \overline{\mathscr{M}}$. since $\mathscr{M}$ is a normed linear subspace (hence a metrizable space), there exists a sequence $\{x_{n}\} \subset \mathscr{M}$ such that $x_{n} \to x$. By continuity of $f$, $0 = f(x_{n}) \to f(x)$ so that $f(x) = 0$. Thus $f \equiv 0$ on $\overline{\mathscr{M}} = \mathscr{X}$.
		\end{solutions}
		\item (Open Mapping Theorem) Let $\mathscr{X}$ and $\mathscr{Y}$ be Banach spaces. If $T \in L(\mathscr{X}, \mathscr{Y})$ is surjective, then $T$ is open. 
		\item (Bounded Inverse Theorem) Let $\mathscr{X}$ and $\mathscr{Y}$ be Banach spaces. If $T \in L(\mathscr{X}, \mathscr{Y})$ is bijective, then $T^{-1} \in L(\mathscr{X}, \mathscr{Y})$. 
		\item (Closed Graph Theorem) If $\mathscr{X}$ and $\mathscr{Y}$ are Banach spaces and $T: \mathscr{X} \to\mathscr{Y}$ is a closed linear map, then $T$ is bounded. 
		\item (Uniform Boundedness Principle) Suppose that $\mathscr{X}$ and $\mathscr{Y}$ are normed vector spaces and $\mathscr{A}$ is a subset of $L(\mathscr{X}, \mathscr{Y})$. \vspace{-0.25cm}
		\begin{enumerate}[itemsep =-2pt,label = (\alph{*})]
			\item $\sup_{T \in \mathscr{A}}\norm{Tx} < \infty$ for all $x$ in a nonmeager subset of $\mathscr{X}$ implies that $\sup_{T\in \mathscr{A}}\norm{T} < \infty$. 
			\item If $\mathscr{X}$ is a \textit{Banach} space, $\sup_{T \in \mathscr{A}}\norm{Tx} < \infty$ for all $x\in\mathscr{X}$ implies that $\sup_{T\in \mathscr{A}}\norm{T} < \infty$. 
		\end{enumerate}
	\end{enumerate}
	
	
	\paragraph{Fourier Analysis and Sobolev Theory} 
	\begin{enumerate}[resume=real_analysis]
		\item The \textit{Schwartz space} $\mathscr{S} \subset C^{\infty}$ is the space defined by 
		\begin{equation*}
			\mathscr{S} = \{f \in C^{\infty}: \sup_{x}(1 + |x|)^{N}|\partial^{\alpha}f(x)| < \infty, \hspace{2pt} \forall N \in \mathbb{N}_{0}, \forall \alpha\}, 
		\end{equation*}
		where $\alpha$ is a multiindex. 
		\item If $f \in \mathscr{S}$, then $\partial^{\alpha}f \in L^{p}$ for all $\alpha$ and all $p \in [1, \infty]$.  \newline 
		\begin{solutions}
			Assume $f \in \mathscr{S}$ and fix a multiindex $\alpha$ and $p \in [1,\infty]$. Let $n$ denote the dimension of the space. Then $(1 + |x|)^{-N} \in L^{p}$ for $N > n/p$. Since $f \in \mathscr{S}$, for each $N$ and every $x$, $|\partial^{\alpha}f(x)| \leq C_{N}(1 + |x|)^{-N}$, where $C_{N} > 0$ is some finite constant.  Thus, one has that $|\partial^{\alpha}f|$ is bounded above by a $p$-integrable function so that $\partial^{\alpha}f$ is itself $p$-integrable. 
		\end{solutions} 
		
		\item The Schwartz space $\mathscr{S}$ is complete under the topology induced by the norm $\norm{\empspace}_{(N, \alpha)} \coloneqq \sup_{x \in \mathbb{R}^{n}}(1 + |x|)^{N}|\partial^{\alpha}\empspace|$. \newline 
		\begin{solutions}
			Let $\{f_{k}\}$ be a Cauchy sequence in $\mathscr{S}$. Then $\norm{f_{j} - f_{k}}_{(N,\alpha)} \to 0$ for all $N, \alpha$. In particular, fixing $N = 0$, one has that for each $\alpha$, the sequence $\{\partial^{\alpha}f_{k}\}$ converges uniformly to a function $g_{\alpha}$. Let $e_{j}$ be the basis vector with a 1 in the $j$th position and zero elsewhere. By the Fundamental Theorem of Calculus, one has that 
			\begin{equation*}
				f_{k}(x + te_{j}) - f_{k}(x) = \int_{0}^{t}\partial_{j}f_{k}(x + se_{j})\;ds. 
			\end{equation*}
			Taking $k \to \infty$ and using uniform convergence to justify passing the limit inside the integral, one has that 
			\begin{equation*}
				g_{0}(x + te_{j}) - g_{0}(x) = \int_{0}^{t}g_{e_{j}}(x + se_{j})\;ds. 
			\end{equation*}
			Using the Fundamental Theorem of Calculus, it follows that $g_{e_{j}} = \partial_{j}g_{0}$. Thus applying induction on $\alpha$, one concludes that $g_{\alpha} = \partial^{\alpha}g_{0}$. Therefore, $\norm{f_{k} - g_{0}}_{(N,\alpha)} \to 0$ for all $\alpha$. 
		\end{solutions}
		\item Let $f \in C^{\infty}$. Then $f \in \mathscr{S}$ iff $x^{\alpha}\partial^{\beta}f$ is bounded for all multiindices $\alpha,\beta$ iff $\partial^{\alpha}(x^{\beta}f)$ is bounded for all multiindices $\alpha,\beta$. 
		\item (Young's Inequality) Suppose $1 \leq p, q, r \leq \infty$ and $p^{-1} + q^{-1} = r^{-1} + 1$. If $f \in L^{p}$, $g \in L^{q}$, then $f \ast g \in L^{r}$ and $\norm{f \ast g}_{L^{r}} \leq \norm{f}_{L^{p}}\norm{g}_{L^{q}}$. 
		\item If $f, g \in \mathscr{S}$, then $f\ast g \in \mathscr{S}$.
		\item $C_{c}^{\infty}$ (and hence $\mathscr{S}$) is dense in $L^{p}$ ($1 \leq p < \infty$) and in $C_{0}$. 
		\item ($C^{\infty}$ Urysohn Lemma) If $K \subset \mathbb{R}^{n}$ is compact and $U$ is an open set containing $K$, there exists $f \in C_{c}^{\infty}$ such that $0 \leq f \leq 1$, $f = 1$ on $K$, and $\operatorname{supp}(f) \subset U$. 
		\item (Few Important Properties of the Fourier Transform) Suppose $f, g \in L^{1}(\mathbb{R}^{n})$. Then \vspace{-0.25cm}
		\begin{enumerate}[itemsep =-2pt,label = (\arabic{*})]
			\item If $T$ is an invertible linear transformation of $\mathbb{R}^{n}$ and $S = (T^\ast)^{-1}$ is its inverse transpose, then $(f \circ T)^{\wh{}} = |\det{T}|^{-1} \hat{f} \circ S$. 
			\item $(f \ast g)^{\wh{}} = \hat{f}\hat{g}$. 
			\item If $x^{\alpha}f \in L^{1}$ for $|\alpha|\leq k$, then $\hat{f} \in C^{k}$ and $\partial^{\alpha}\hat{f} = [(-2\pi i x)^{\alpha}f]^{\wh{}}$. 
			\item If $f \in C^{k}$, $\partial^{\alpha}f \in L^{1}$ for $|\alpha| \leq k$, and $\partial^{\alpha}f \in C_{0}$ for $|\alpha| \leq k- 1$, then $(\partial^{\alpha}f)^{\wh{}}(\xi) = (2\pi i \xi)^{\alpha}\hat{f}(\xi)$. 
			\item (Riemann-Lebesgue Lemma) $\mathscr{F}(L^{1}(\mathbb{R}^{n})) \subset C_{0}(\mathbb{R}^{n})$. 
			\item (Plancherel's Theorem) $\mathscr{F}(L^{2}(\mathbb{R}^{n})) = L^{2}(\mathbb{R}^{n})$. 
			\item The Fourier transform maps the Schwartz class $\mathscr{S}$ continuously onto itself. 
			\item If $f(x)= e^{-\pi a |x|^{2}}$ where $a > 0$, then $\hat{f}(\xi) = a^{-n/2}e^{-\pi |\xi|^{2}/a}$. 
			\item $\int \hat{f}g = \int f \hat{g}$. 
			\item (Fourier Inversion Theorem) If $f, \hat{f} \in L^{1}$, then $f$ agrees almost everywhere with a continuous function $f_{0}$, and $(\hat{f})^{\check{}} = (\check{f})^{\wh{}} = f_{0}$. 
			\item (Hausdorff-Young Inequality) Suppose that $1 \leq p \leq 2$ and $q$ is the conjugate exponent to $p$. If $f \in L^{p}(\mathbb{R}^{n})$, then $\hat{f} \in L^{q}(\mathbb{R}^{n})$ and $\norm{\hat{f}}_{L^q} \leq \norm{f}_{L^{p}}$. 
		\end{enumerate}
		\end{enumerate}
