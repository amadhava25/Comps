...
f2		\begin{equation*}
	...
	($\Longleftarrow$) Assume that $R$ is an integral domain that satisfies the ascending chain condition for principal ideals and further assume that every irreducible element is prime. Assume to the contrary that $R$ is not a UFD; i.e., assume to the contrary that there exists some nonzero noninvertible element $x_0 \in R$ that does not factor into the product of finitely many irreducible elements. Define the principal ideal $I_{0} = Rx_{0}$. By hypothesis, $x_{0}$ must then split into nonzero nonunit factors (since $a$ itself is not irreducible), of which at least one (denoted $x_1$) also does not split into a finite product of irreducible elements. Let $I_{1} = Rx_{1}$. Thus repeating the argument for $x_1$, we obtain yet another such element $x_2$, and thus the ideal $I_{2} = Rx_{2}$. In this manner, we obtain the infinite sequences $\{x_{j}\}_{j = 0}^{\infty}$ and $\{I_{j}\}_{j = 0}^{\infty}$ of elements of $R$ that do not split into the factor of finitely many irreducible elements and a \textit{strictly} ascending chain of principal ideals, respectively. But by the ascending chain condition for principal ideals, there exists some $N > 0$ such that for all $n \geq N$, $I_{n} = I_{N} \iff Rx_{n} = Rx_{N}$. This means that our procedure of obtaining a further element of $R$ that does not split into a finite product of irreducible elements must terminate after finitely many iterations; i.e. this contradicts our hypothesis that the infinite sequence of principle ideals is strictly ascending. Thus by contradiction, every nonzero nonunit element of $R$ must split into a product of finitely many irreducible elements. Now we will show that such a factorization is unique. 
