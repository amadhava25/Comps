...
f2		\begin{equation*}
	...
	\end{prb}
	Let $f(z) = e^{z} + 1$ and $h(z) = 6z^{3}$. Then on the unit circle $\partial\{|z| < 1\}$, one has that 
	\begin{equation*}
		|f(z)| \leq |e^{z}| + 1 = e^{\operatorname{Re}{z}} + 1\leq e + 1 < 6 = |h(z)|. 
	\end{equation*}
	Hence, by Rouch\'e's theorem, $h(z)$ and $h(z) + f(z) = g(z)$ must have the same number of zeros (counted with multiplicity) in the unit disk $\{|z| < 1\}$. Since $h(z)$ has precisely three roots (counted with multiplicity) in the unit disk, we conclude that $g(z)$ also has three roots in the unit disk up to multiplicity. 
	
	\begin{prb}{2018-A-I-3}
		Show that if $c > 1$, then the function $$f(z) = ze^{c - z} - 1$$ has precisely one root in $\Delta = \{|z| < 1\}$, and this root is real and positive. 
	\end{prb}
	Let $c > 1$, and define the function $f(z) = ze^{c - z} - 1$ on $\mathbb{C}$. Let $g(z)$ be the constant function $-1$, and let $h(z)$ be the function $ze^{c - z}$. Since the exponential is never zero, $h(z)$ has precisely one root in $\Delta = \{|z| < 1\}$, namely $z = 0$. On $\partial \Delta$, one observes that since $c > 1$ and $|ze^{c- z}| = |z|e^{c - \operatorname{Re}{z}} = e^{c - \operatorname{Re}{z}} > 1$ since $\operatorname{Re}{z} \leq 1$,  
	\begin{equation*}
		|g(z)| = 1 < |h(z)|. 
	\end{equation*}
	Thus by Rouch\'e's Theorem, one has that $h(z)$ and $h(z) + g(z) = f(z)$ have the same number of roots in $\Delta$. Therefore, $f(z)$ has precisely one root in $\Delta$. Now consider the function $f_{\mathbb{R}}(x)$, which is the restriction of $f(z)$ to the open unit interval $(-1,1)$. By elementary calculus, $xe^{c - x} - 1$ is continuous on the whole interval, and thus on the interval $(0,1)$. Moreover, $f_{\mathbb{R}}(0) = -1$ while $f_{\mathbb{R}}(1) = e^{c - 1} - 1 > 0$ since $c - 1 > 0$ implies $e^{c - 1} > 1$. Hence, by the Intermediate Value Theorem, there exists some $a \in (0,1)$ such that $f(a) = f_{\mathbb{R}}(a) = 0$. Since we showed $f$ has only one root, we conclude that this root is real and positive. 
	...
	\end{prb}
	Let $P(z) = z^{n} + a_{1}z^{n-1} + \dotsm + a_{n}$. If there exists some $|z| = 1$ such that $|P(z)| > 1$ we are done. So assume that this property does not hold (i.e., assume that for every $|z| = 1$, $|P(z)| \leq 1$). We must show that each coefficient $a_{1}, \ldots, a_{n}$ is zero. Consider the polynomial $g(z) = 1 + a_{1}z + a_{2}z + \dotsm + a_{n}z^{n}$. Notice that $g(0) = 1$, and for every $|z| = 1$, one has 
	\begin{equation*}
		|g(z)| = |z|^{n} \cdot |P(1/z)| \leq 1. 
	\end{equation*}
	
	\subsection{Residue Theorem and Complex Integration}
	\begin{prb}{2018-J-I-2}
		...
		\end{equation*}
	\end{prb}
	We start by defining the complex-valued function, 
	\begin{equation*}
		f(z) = \frac{z + 1}{z^{4} + 2z^{2} + 5}. 
	\end{equation*}
	We note that the denominator of $f(z)$ has zeros at the points, 
	\begin{equation*}
		z = \pm \sqrt{-1 - 2i}, \pm \sqrt{-1 + 2i}. 
	\end{equation*}
	Of these roots, we note that only $z_{1} = \sqrt{-1 + 2i}$ and $z_{2} = -\sqrt{-1 - 2i}$ are contained in the upper half plane. Thus for some $R > 0$, we consider the semicircle contour $\gamma_{R}$ whose diameter lies in the real interval $[-R, R]$, and whose radius is $R$; let $\gamma_{1, R}$ denote the curved portion of $\gamma$, and let $\gamma_{2,R}$ denote the real interval portion. Figure \ref{fig:2018JI2} depicts this contour. 
	\begin{figure}[h!]
		\centering
		\begin{tikzpicture}
	...
	First, we must show that the integral of $f(z)$ over this contour converges to the original real integral in the limit $R \to \infty$. For sufficiently large $R$ and $|z| = R$, one has that 
	\begin{equation*}
		\abs{\frac{z + 1}{z^{4} + 2z^{2} + 5}} \leq \frac{R + 1}{R^{4} - 2R^{2} - 5}. 
	\end{equation*}
	This implies that 
	\begin{equation*}
		\int_{\gamma_{1,R}}f(z)\;dz \leq \pi R \cdot \frac{R + 1}{R^{4} - 2R^{2} - 5} \xrightarrow{R \to \infty} 0. 
	\end{equation*}
	Thus the contribution along the semicircle portion vanishes in the limit $R \to \infty$. On the other, the integral of $f(z)$ over $[-R,R]$ converges to the desired real integral in the limit $R \to \infty$. Thus, the original integral is given by the limit $\lim_{R\to \infty}\int_{\gamma_{R}}f(z)\;dz$. Fix some arbitrary $R > 0$. Since $\gamma_{R}$ encloses $z_{1}$ and $z_{2}$, we must compute the residue of $f$ at these poles. A simple calculation yields, 
	\begin{equation*}
		\operatorname{Res}_{f}(z_{1}) = -\frac{1}{8}\left(i +  \frac{\sqrt{1 + 2i}}{\sqrt{5}}\right) \quad \text{and}\quad \operatorname{Res}_{f}(z_{2}) = \frac{1}{8}\left(i + \frac{\sqrt{1 - 2i}}{\sqrt{5}}\right). 
	\end{equation*}
	Thus this implies that 
	\begin{align*}
		\begin{split}
		\int_{\gamma_{R}}f(z)\;dz &= 2\pi i \cdot \left[\frac{\sqrt{5}}{40} \cdot \left(-\sqrt{1 + 2i} + \sqrt{1 - 2i}\right)\right] \\
		&= \frac{\sqrt{5}\pi i}{20} \cdot \left[-\sqrt{\frac{1 + \sqrt{5}}{2}} - i\sqrt{\frac{2}{1 + \sqrt{5}}} + \sqrt{\frac{1 + \sqrt{5}}{2}} - i\sqrt{\frac{2}{1 + \sqrt{5}}}\right] \\
		&= \frac{\sqrt{5}\pi}{10} \cdot \sqrt{\frac{2}{1 + \sqrt{5}}} =\pi \cdot  \sqrt{\frac{10}{100 \cdot (1 + \sqrt{5})}} = \pi \cdot \sqrt{\frac{1}{10 \cdot (1 + \sqrt{5})}} \\
		&= \pi \cdot \sqrt{\frac{1 - \sqrt{5}}{10 \cdot (1 - 5)}} = \pi \cdot \sqrt{\frac{\sqrt{5} - 1}{40}} = \frac{\pi}{2}\cdot \sqrt{\frac{\sqrt{5} - 1}{10}}. 
		\end{split}
	\end{align*}
	Thus, we conclude that 
	\begin{equation*}
		\int_{-\infty}^{\infty}\frac{x + 1}{x{4} + 2x^{2} + 5}\;dx = \frac{\pi}{2} \cdot \sqrt{\frac{\sqrt{5} - 1}{10}}. 
	\end{equation*}
	\newpage 
	\section{Notes}
